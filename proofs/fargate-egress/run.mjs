...
const publicEnvironmentNames = Object.freeze([
  "AWS_M019_ISOLATED_TEST",
  "AWS_M019_KUBECONFIG",
  "AWS_M019_KUBE_CONTEXT",
  "AWS_M019_CLUSTER_NAME",
  "AWS_M019_REGION",
  "AWS_M019_FARGATE_PROFILE",
  "AWS_M019_PROFILE_NAMESPACE_PREFIX",
  "AWS_M019_PROFILE_LABEL_KEY",
  "AWS_M019_PROFILE_LABEL_VALUE",
  "AWS_M019_PROXY_URL",
  "AWS_M019_DIRECT_URL",
  "AWS_M019_CANARY_TOKEN",
  "AWS_M019_POD_SECURITY_GROUP_ID",
  "AWS_M019_CLUSTER_SECURITY_GROUP_ID",
  "AWS_M019_PROXY_SECURITY_GROUP_ID",
  "AWS_M019_VPC_ID",
  "AWS_M019_DNS_CIDR",
  "AWS_M019_ACCESS_KEY_ID",
  "AWS_M019_SECRET_ACCESS_KEY",
]);
const attestation = "I_UNDERSTAND_THIS_APPLIES_A_DISPOSABLE_EKS_EGRESS_POLICY";
...
export function validateProofEnvironment(environment) {
  ...
  if (result.AWS_M019_ISOLATED_TEST !== attestation ||
      result.AWS_M019_PROFILE_NAMESPACE_PREFIX !== "zasp-m019-" ||
      result.AWS_M019_PROFILE_LABEL_KEY !== "zasp.agentsec.dev/fargate" ||
      result.AWS_M019_PROFILE_LABEL_VALUE !== "true") reject("configuration");
  const sessionToken = environment.AWS_M019_SESSION_TOKEN;
  if (sessionToken !== undefined) {
    if (typeof sessionToken !== "string" || sessionToken.length === 0 || sessionToken.length > 4_096 || /[\0\r\n]/u.test(sessionToken)) reject("configuration");
    result.AWS_M019_SESSION_TOKEN = sessionToken;
  }
  return result;
}
...
export async function orchestrate({
  ...
  try {
    ...
    const executable = join(identity.path, "fargate-egress-proof");
    ...
    const proofEnvironment = Object.assign(Object.create(null), publicEnvironment, { ZASP_M019_KUBECTL_EXECUTABLE: kubectl });
    ...
    if (proof?.code === 0 && proof?.signal === null && proof?.stdout === `${successLine}\n` && proof?.stderr === "") {
      ...
      const match = proof?.code !== 0 && proof?.signal === null && proof?.stdout === "" && typeof proof?.stderr === "string"
        ? /^EKS Fargate egress proof failed: (configuration|provider|scheduling|canary|ownership|cleanup|deadline|panic) rejected\.\n$/u.exec(proof.stderr)
        : null;
...
export function fixedFailureLine(error) {
  ...
  return `EKS Fargate egress proof failed: ${category} rejected.`;
}
