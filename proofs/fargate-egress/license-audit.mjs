...
function reject() { throw new Error("Fargate egress adapter license audit rejected"); }
...
export async function fingerprintURL(url, options = {}) {
  ...
  let reader;
  try {
    const response = await (options.fetchImpl ?? fetch)(url, { signal: controller.signal, redirect: "error", headers: { Accept: "application/octet-stream", "Accept-Encoding": "identity", "User-Agent": "zasp-fargate-egress-license-audit/1" } });
...
export async function runMain(output = process.stdout, error = process.stderr, dependencies = {}) {
  try {
    ...
    output.write(`EKS Fargate egress adapter license audit passed: components=${result.components} artifacts=${result.artifacts} prohibited=${result.prohibited}.\n`);
    return 0;
  } catch {
    error.write("EKS Fargate egress adapter license audit failed.\n");
    return 1;
  }
}
