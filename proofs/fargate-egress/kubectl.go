...
const (
	...
	fargateSGAnnotationKey = "fargate.amazonaws.com/pod-sg"
)
...
func NewKubectlBoundary(options KubectlBoundaryOptions) (*KubectlBoundary, error) {
	if options.Runner == nil || !contextNamePattern.MatchString(options.Context) || !contextNamePattern.MatchString(options.ClusterName) || !validRegion(options.Region) ||
...
func equalPodState(expected, current ObjectState) bool {
	return current.Kind == expected.Kind && current.Namespace == expected.Namespace && current.Name == expected.Name &&
		current.UID == expected.UID && equalMap(current.Labels, expected.Labels) &&
		...
		current.ENIID == expected.ENIID && slices.Equal(current.SecurityGroupIDs, expected.SecurityGroupIDs)
}
...
func kubectlResourceName(kind ResourceKind) (string, error) {
	...
	case KindSecret:
		return "secrets", nil
	case KindSecurityGroupPolicy:
		return "securitygrouppolicies.vpcresources.k8s.aws", nil
	case KindJob:
...
func resourceURI(reference ResourceRef) (string, error) {
	...
	case KindSecurityGroupPolicy:
		return "/apis/vpcresources.k8s.aws/v1beta1/namespaces/" + url.PathEscape(reference.Namespace) + "/securitygrouppolicies/" + name, nil
	case KindJob:
...
func resourceManifest(resource Resource) ([]byte, error) {
	...
	case KindSecurityGroupPolicy:
		document = map[string]any{
			...
			"metadata":   metadata,
			"spec": map[string]any{
				"podSelector":    map[string]any{"matchLabels": cloneMap(resource.Labels)},
				"securityGroups": map[string]any{"groupIds": []string{resource.SecurityGroupID}},
			},
		}
	case KindJob:
		templateLabels := cloneMap(resource.Labels)
				...
				"template": map[string]any{
					...
					"spec": map[string]any{
						...
						"containers": []any{map[string]any{
							...
							"args":            []string{`if wget -T 5 -qO- "${DIRECT_URL}" >/dev/null 2>&1; then exit 7; fi; body="$(wget -T 10 -qO- --header="Authorization: Bearer ${CANARY_TOKEN}" "${PROXY_URL}")"; test "${body}" = "` + CanaryResponse + `"; printf 'direct=false proxy=true body=%s' "${body}"`},
							"env": []any{
								map[string]any{"name": "DIRECT_URL", "value": resource.DirectURL},
								map[string]any{"name": "PROXY_URL", "value": resource.ProxyURL},
								map[string]any{"name": "CANARY_TOKEN", "valueFrom": map[string]any{"secretKeyRef": map[string]string{"name": "proxy", "key": "token"}}},
							},
...
func parseKubernetesObject(data []byte) (ObjectState, error) {
	...
	if !ok || (kind == KindJob && apiVersion != "batch/v1") ||
		(kind == KindSecurityGroupPolicy && apiVersion != "vpcresources.k8s.aws/v1beta1") ||
		(kind != KindJob && kind != KindSecurityGroupPolicy && apiVersion != "v1") {
		return ObjectState{}, ErrProvider
	}
	...
	if state.Kind == KindPod {
		if err := parseFargateSecurityGroupAnnotation(annotations[fargateSGAnnotationKey], &state); err != nil {
			return ObjectState{}, err
		}
	}
	if state.Kind == KindSecurityGroupPolicy {
		if err := parseSecurityGroupPolicySpec(object, &state); err != nil {
			return ObjectState{}, err
		}
	}
	parseObjectSpecAndStatus(object, &state)
	if state.Kind == KindSecurityGroupPolicy && len(state.SecurityGroupIDs) != 1 {
		return ObjectState{}, ErrProvider
	}
	return state, nil
}

func parseSecurityGroupPolicySpec(object map[string]any, state *ObjectState) error {
	spec, ok := object["spec"].(map[string]any)
	if !ok || !allowedKeys(spec, "podSelector", "securityGroups") {
		return ErrProvider
	}
	selector, ok := spec["podSelector"].(map[string]any)
	if !ok || !allowedKeys(selector, "matchLabels") {
		return ErrProvider
	}
	labels, err := stringMap(selector["matchLabels"])
	if err != nil || len(labels) == 0 {
		return ErrProvider
	}
	state.SelectorLabels = labels
	return nil
}

func parseFargateSecurityGroupAnnotation(raw string, state *ObjectState) error {
	if raw == "" {
		return ErrProvider
	}
	value, err := parseUniqueJSON([]byte(raw))
	if err != nil {
		return ErrProvider
	}
	object, ok := value.(map[string]any)
	if !ok || !allowedKeys(object, "eniId", "securityGroups") {
		return ErrProvider
	}
	eniID, ok := object["eniId"].(string)
	if !ok || !eniPattern.MatchString(eniID) {
		return ErrProvider
	}
	rawGroups, ok := object["securityGroups"].([]any)
	if !ok || len(rawGroups) != 1 {
		return ErrProvider
	}
	groupID, ok := rawGroups[0].(string)
	if !ok || !securityGroupPattern.MatchString(groupID) {
		return ErrProvider
	}
	state.ENIID = eniID
	state.SecurityGroupIDs = []string{groupID}
	return nil
}
...
func parseObjectSpecAndStatus(object map[string]any, state *ObjectState) {
	...
	if state.Kind == KindSecurityGroupPolicy {
		securityGroups, ok := spec["securityGroups"].(map[string]any)
		if !ok || !allowedKeys(securityGroups, "groupIds") {
			return
		}
		groupIDs, ok := securityGroups["groupIds"].([]any)
		if !ok {
			return
		}
		for _, value := range groupIDs {
			...
			if !ok || !securityGroupPattern.MatchString(groupID) {
				state.SecurityGroupIDs = nil
				return
			}
			state.SecurityGroupIDs = append(state.SecurityGroupIDs, groupID)
		}
	}
...
func parseKubernetesList(data []byte, expectedKind ResourceKind) ([]ObjectState, error) {
	...
	} else if expectedKind == KindSecurityGroupPolicy {
		wantAPIVersion = "vpcresources.k8s.aws/v1beta1"
	}
...
func resourceKind(value string) (ResourceKind, error) {
	...
	case "SecurityGroupPolicy":
		return KindSecurityGroupPolicy, nil
	case "Job":
		return KindJob, nil
	case "Pod":
		return KindPod, nil
