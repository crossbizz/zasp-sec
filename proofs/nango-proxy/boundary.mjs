...
export function validateProxyMarker(value) {
  if (typeof value !== "string" || !markerPattern.test(value)) throw new TypeError("proxy marker is invalid");
  return value;
}

export async function createProxyWorkspace({
  marker,
  tempParent = tmpdir(),
  proofSourcePath,
  ...
  const validatedMarker = validateProxyMarker(marker);
  if (!canonicalAbsolute(tempParent) || !canonicalAbsolute(proofSourcePath) || typeof randomSource !== "function" || typeof runCommand !== "function" || !plainObject(overrides)) {
    throw new TypeError("proxy workspace input is invalid");
  }
  ...
  if (!passwordPattern.test(password) || !validEncryptionKey(encryptionKey) || !providerKeyPattern.test(providerKey)) {
    throw new TypeError("proxy workspace secret is invalid");
  }
  ...
  try {
    ...
    if (candidate !== undefined) {
      try {
        ...
        if (!sameIdentityExceptMode(current, retained)) throw new TypeError("proxy workspace identity changed");
        await io.remove(candidate, { recursive: true, force: false, maxRetries: 0 });
        await requireMissing(candidate, io);
      } catch (cleanupError) {
        throw new TypeError("proxy workspace creation cleanup failed", { cause: cleanupError });
      }
    }
    throw error;
  }
...
export async function reproveProxyWorkspace(workspace, overrides = {}) {
  ...
  for (const [name, mode] of [["caKey", 0o400], ["caCertificate", 0o444], ["caSerial", 0o444], ["fixtureCertificate", 0o444], ["fixtureKey", 0o400], ["fixtureRequest", 0o444], ["san", 0o444]]) {
    ...
    if (!sameFileIdentity(current, expected)) throw new TypeError("proxy TLS identity changed");
  }
  await requireExactEntries(tlsDirectory.path, tlsEntries, io);
  return workspace;
}

export async function removeProxyWorkspace(workspace, overrides = {}) {
  const io = buildIo(overrides);
  try {
    await reproveProxyWorkspace(workspace, io);
    await io.remove(workspace.root.path, { recursive: true, force: false, maxRetries: 0 });
    await requireMissing(workspace.root.path, io);
  } catch (error) {
    throw new TypeError("proxy workspace cleanup failed", { cause: error });
  }
}

export function validateProxyTemporaryPrefixEntries(entries) {
  if (!Array.isArray(entries) || entries.some((entry) => typeof entry !== "string")) throw new TypeError("proxy temporary entries are invalid");
  if (entries.some((entry) => entry.startsWith(globalPrefix))) throw new TypeError("stale proxy temporary root exists");
  return true;
}
...
function tlsCommands(paths, marker) {
  return [
    ...
    ["req", "-x509", "-new", "-nodes", "-key", paths.caKey, "-sha256", "-days", "1", "-subj", `/CN=zasp-m0-15-ca-${marker}`, "-out", paths.caCertificate],
...
async function inspectOwnedRoot(candidate, parent, prefix, io) {
  if (!canonicalAbsolute(candidate) || !canonicalAbsolute(parent)) throw new TypeError("proxy root path is invalid");
  const name = basename(candidate);
  if (dirname(candidate) !== parent || !name.startsWith(prefix) || !suffixPattern.test(name.slice(prefix.length))) throw new TypeError("proxy root name is invalid");
...
async function inspectDirectory(path, parent, name, io) {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()) throw new TypeError("proxy directory type is invalid");
  const [parentCanonical, canonical] = await Promise.all([io.realpath(parent), io.realpath(path)]);
  if (dirname(path) !== parent || basename(path) !== name || dirname(canonical) !== parentCanonical || basename(canonical) !== name) throw new TypeError("proxy directory boundary is invalid");
...
async function inspectFile(path, parent, name, mode, io) {
  const stat = await io.lstat(path);
  if (!stat.isFile() || stat.isSymbolicLink() || dirname(path) !== parent || basename(path) !== name) throw new TypeError("proxy TLS file boundary is invalid");
  const canonical = await io.realpath(path);
  const parentCanonical = await io.realpath(parent);
  if (dirname(canonical) !== parentCanonical || basename(canonical) !== name) throw new TypeError("proxy TLS canonical boundary is invalid");
  requireMode(stat.mode, mode);
  const content = await io.readFile(path);
  if (!Buffer.isBuffer(content) || content.byteLength === 0 || content.byteLength > 32_768) throw new TypeError("proxy TLS file is invalid");
...
async function inspectParent(parent, io) {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()) throw new TypeError("proxy parent is invalid");
  return io.realpath(parent);
}

async function recheckIdentity(expected, inspect, mode) {
  const current = await inspect();
  if (!sameIdentityExceptMode(current, expected)) throw new TypeError("proxy directory identity changed");
  requireMode(current.mode, mode);
  return current;
}

function requireSame(current, expected, mode) {
  if (!sameDirectoryIdentity(current, expected)) throw new TypeError("proxy directory identity changed");
  requireMode(current.mode, mode);
}
...
async function requireExactEntries(path, expected, io) {
  const entries = await io.readdir(path);
  if (!Array.isArray(entries) || entries.some((entry) => typeof entry !== "string") || !sameEntries(entries, expected)) throw new TypeError("proxy directory entries are invalid");
}
...
async function requireMissing(path, io) {
  ...
  throw new TypeError("proxy workspace still exists");
}

function expectWorkspace(value) {
  if (!plainObject(value) || !plainObject(value.root) || !plainObject(value.dockerConfig) || !plainObject(value.tls) || value.prefix !== `${globalPrefix}${value.marker}-` || !providerKeyPattern.test(value.providerKey ?? "")) throw new TypeError("proxy workspace is invalid");
  validateProxyMarker(value.marker);
}
...
function requireMode(mode, expected) { if ((mode & 0o777) !== expected) throw new TypeError("proxy file mode is invalid"); }
function integer(value) { if (!Number.isInteger(value)) throw new TypeError("proxy identity is invalid"); return value; }
...
function buildIo(overrides) {
  if (!plainObject(overrides)) throw new TypeError("proxy workspace I/O is invalid");
  const io = { ...defaultIo, ...overrides };
  for (const key of ["chmod", "lstat", "makeTemp", "mkdir", "readFile", "readdir", "realpath", "remove", "writeFile"]) if (typeof io[key] !== "function") throw new TypeError("proxy workspace I/O is invalid");
  return io;
}
