...
const expectedEventId = "11111111-1111-4111-8111-111111111111";
const expectedEventAction = "item.usage";
...
export class Failure extends Error {
  constructor(category = "proxy", options) {
    super(`${category} rejected`, options);
    this.category = category;
  }
}

export async function runProxyConnection(input, dependencies = {}) {
  try {
    validateInput(input);
    ...
    const event = await proxyEvent(input, apiKey, connectionId, request, dependencies.signal);

    const result = { organizationId: input.organizationId, integrationKey: input.integrationKey, connectionId, event };
...
export async function runMain(input, dependencies = {}) {
  ...
  try {
    ...
    const result = await runProxyConnection(input, dependencies);
    stdout.write(`${JSON.stringify(result)}\n`);
    return 0;
  } catch {
    try { stderr?.write?.("Nango proxy wrapper failed.\n"); } catch { /* fixed boundary */ }
    return 1;
  }
}
...
export function configurationFromEnvironment(environment) {
  ...
  const forbidden = environment.NANGO_PROXY_FORBIDDEN_VALUES;
  ...
  const input = {
    baseUrl: environment.NANGO_PROXY_BASE_URL,
    environment: environment.NANGO_PROXY_ENVIRONMENT,
    organizationId: environment.NANGO_PROXY_ORGANIZATION_ID,
    endUserId: environment.NANGO_PROXY_END_USER_ID,
    integrationKey: environment.NANGO_PROXY_INTEGRATION_KEY,
    providerKey: environment.NANGO_PROXY_PROVIDER_KEY,
    forbiddenValues,
  };
  validateInput(input);
...
async function readApiKey(input, request, signal) {
  ...
  if (
    !positiveInteger(key.id) || !boundedString(key.display_name, 1, 255) || !Array.isArray(key.scopes) ||
    key.scopes.length !== 1 || key.scopes[0] !== "environment:*" || !uuidV4Pattern.test(key.secret ?? "") ||
    (key.last_used_at !== null && !validTimestamp(key.last_used_at)) || !validTimestamp(key.created_at) || !validTimestamp(key.updated_at)
...
async function createIntegration(input, apiKey, request, signal) {
  ...
  if (
    !positiveInteger(value.id) || value.unique_key !== input.integrationKey || value.provider !== "1password-events" ||
    value.oauth_client_id !== null || value.oauth_client_secret !== null || value.oauth_scopes !== null || !positiveInteger(value.environment_id) ||
    value.app_link !== null || value.custom !== null || !Array.isArray(value.missing_fields) || value.missing_fields.length !== 0 ||
...
async function readConnection(input, apiKey, request, sleep, maximumPollAttempts, signal) {
  ...
  for (let attempt = 0; attempt < maximumPollAttempts; attempt += 1) {
    ...
    if (attempt + 1 < maximumPollAttempts) {
      try { await sleep(100); } catch { throw failure("proxy"); }
    }
  }
  throw failure("provider");
}
...
function validateConnection(input, connection) {
  ...
  if (
    !positiveInteger(connection.id) || !uuidV4Pattern.test(connection.connection_id ?? "") || connection.provider_config_key !== input.integrationKey ||
    connection.provider !== "1password-events" || !Array.isArray(connection.errors) || connection.errors.length !== 0 ||
    connection.endUser.id !== input.endUserId || connection.endUser.display_name !== null || connection.endUser.email !== null ||
    connection.endUser.tags.origin !== "nango_dashboard" || connection.endUser.organization.id !== input.organizationId ||
    connection.endUser.organization.display_name !== null || connection.tags.end_user_id !== input.endUserId ||
    connection.tags.organization_id !== input.organizationId || connection.tags.origin !== "nango_dashboard" ||
    !Array.isArray(connection.pausedSyncs) || connection.pausedSyncs.length !== 0 ||
    !validConnectionTimestamp(connection.created_at) || !validConnectionTimestamp(connection.updated_at)
  ) throw failure("provider");
  return connection.connection_id;
}

async function proxyEvent(input, apiKey, connectionId, request, signal) {
  const response = await requestJson(request, {
    method: "GET",
    url: `${input.baseUrl}/proxy/api/v2/events?limit=1`,
    headers: {
  ...
  exactKeys(response, ["cursor", "items"]);
  if (response.cursor !== null || !Array.isArray(response.items) || response.items.length !== 1) throw failure("provider");
  const [item] = response.items;
  exactKeys(item, ["action", "uuid"]);
  if (item.uuid !== expectedEventId || item.action !== expectedEventAction) throw failure("normalization");
  return { id: item.uuid, action: item.action };
}
...
async function requestRaw(request, specification) {
  ...
  assertActive(specification.signal);
  if (
    !plainObject(response) || !Number.isInteger(response.status) || response.status < 100 || response.status > 599 ||
    !plainObject(response.headers) || !Buffer.isBuffer(response.body) || response.body.byteLength > maximumBodyBytes
  ) throw failure("provider");
  return response;
}
...
export async function boundedRequest(specification) {
  ...
  try {
    if (specification.signal?.aborted === true) throw failure("proxy");
...
function validateInput(input) {
  ...
  if (
    base.protocol !== "http:" || base.port !== "3003" || base.pathname !== "/" || base.search !== "" || base.hash !== "" ||
    !/^[a-z0-9][a-z0-9-]{0,62}$/.test(base.hostname) || input.environment !== "dev" ||
    !organizationPattern.test(input.organizationId) || !endUserPattern.test(input.endUserId) ||
    !integrationPattern.test(input.integrationKey) || !providerKeyPattern.test(input.providerKey ?? "") ||
    !Array.isArray(input.forbiddenValues) || input.forbiddenValues.length < 1 || input.forbiddenValues.length > 16 ||
    input.forbiddenValues.some((value) => !boundedString(value, 1, 4_096))
...
function assertActive(signal) { if (signal?.aborted === true) throw failure("proxy"); }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  catch { process.stderr.write("Nango proxy wrapper failed.\n"); process.exitCode = 1; }
}
