...
const verificationBody = Buffer.from('{"features":[],"items":[]}');
const proxyBody = Buffer.from('{"cursor":null,"items":[{"action":"item.usage","uuid":"11111111-1111-4111-8111-111111111111"}]}');
...
export function createFixtureProvider(configuration, dependencies = {}) {
  ...
  const handle = async (request) => {
    try {
      if (
        !plainObject(request) || typeof request.method !== "string" || typeof request.host !== "string" ||
        typeof request.url !== "string" || !plainObject(request.headers) || !Buffer.isBuffer(request.body) ||
        request.bodyComplete !== true || request.body.byteLength > maximumRequestBytes
      ) return rejected();
      if (
        request.host !== validated.hostname || request.method !== "GET" || request.body.byteLength !== 0 ||
        !exactHeader(request.headers, "accept", "application/json, text/plain, */*") ||
        !exactHeader(request.headers, "authorization", `Bearer ${validated.providerKey}`)
      ) return rejected();

      if (!verificationUsed && !proxyUsed && request.url === "/api/v2/auth/introspect") {
        verificationUsed = true;
        return accepted(verificationBody);
      }
      if (verificationUsed && !proxyUsed && request.url === "/api/v2/events?limit=1") {
        proxyUsed = true;
        return accepted(proxyBody);
      }
      return rejected();
    } catch {
      return rejected();
  ...
  const ensureServer = () => {
    ...
    if (
      !Buffer.isBuffer(key) || !Buffer.isBuffer(certificate) || key.byteLength === 0 || certificate.byteLength === 0 ||
      key.byteLength > 32_768 || certificate.byteLength > 32_768
  ...
  return Object.freeze({
    handle,
    state: () => ({ verificationUsed, proxyUsed }),
...
export async function runMain(configuration, dependencies = {}) {
  ...
  try {
    ...
    stdout.write("Nango proxy fixture ready.\n");
    return 0;
  } catch {
    try { stderr?.write?.("Nango proxy fixture failed.\n"); } catch { /* fixed boundary */ }
    return 1;
  }
}
...
export function configurationFromEnvironment(environment) {
  ...
  return {
    hostname: environment.NANGO_PROXY_HOSTNAME,
    providerKey: environment.NANGO_PROXY_PROVIDER_KEY,
    tlsKeyPath: environment.NANGO_PROXY_TLS_KEY_PATH,
    tlsCertificatePath: environment.NANGO_PROXY_TLS_CERTIFICATE_PATH,
  };
}
...
function accepted(body) { return { status: 200, headers: { "content-type": "application/json", "cache-control": "no-store" }, body: Buffer.from(body) }; }
...
function exactHeader(headers, name, expected) { const matches = Object.entries(headers).filter(([key]) => key.toLowerCase() === name); return matches.length === 1 && typeof matches[0][1] === "string" && safeEqual(matches[0][1], expected); }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  catch { process.stderr.write("Nango proxy fixture failed.\n"); process.exitCode = 1; }
}
