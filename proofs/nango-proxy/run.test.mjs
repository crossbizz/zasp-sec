...
import {
  ...
  parseWrapperOutput,
  runMain,
  validateProxyContainerIdentity,
} from "./run.mjs";
import { removeProxyWorkspace } from "./boundary.mjs";
import { PROXY_PINS, buildProxyRuntimeSpec } from "./manifest.mjs";
...
test("binds the fixed success line and strict mutation taxonomy", () => {
  assert.equal(SUCCESS_LINE, "Nango proxy proof passed: get=true response=true product_state_safe=true cleanup=true.");
...
test("builds the exact M0-15 isolated schema inspect and mutation commands", () => {
  const spec = specification();
...
test("accepts only the exact normalized proxy wrapper artifact", () => {
  const exact = JSON.stringify({ organizationId: `org_${marker}`, integrationKey: `zasp-m0-15-${marker}-1password-events`, connectionId: "00000000-0000-4000-8000-0000000000b4", event: { id: "11111111-1111-4111-8111-111111111111", action: "item.usage" } });
  ...
  for (const value of [
    ...
    JSON.stringify({ ...JSON.parse(exact), event: { id: "11111111-1111-4111-8111-111111111111", action: "other" } }),
    JSON.stringify({ ...JSON.parse(exact), event: { id: "11111111-1111-4111-8111-111111111111", action: "item.usage", extra: true } }),
    "not-json\n",
...
test("binds complete fixture container security, environment, mounts, and inactive network state", () => {
  ...
  assert.equal(validateProxyContainerIdentity(document, options), true);
  ...
  assert.equal(validateProxyContainerIdentity(running, { ...options, expectedState: "running" }), true);
  ...
  assert.equal(validateProxyContainerIdentity(exited, { ...options, expectedState: undefined, cleanup: true }), true);
  for (const mutate of [
    ...
    mutate(changed);
    assert.throws(() => validateProxyContainerIdentity(changed, options));
  }
});
...
test("runs the exact dependency graph and reverse cleanup", async () => {
  ...
  assert.deepEqual(await orchestrate(runtime, { mainTimeoutMs: 2_000, cleanupTimeoutMs: 2_000 }), {
    proxy: true, reference: true, productStateSafe: true, cleanup: true,
  });
...
test("continues cleanup and gives it precedence", async () => {
  const calls = [];
  const runtime = {
    initialize: async () => { throw Object.assign(new Error("main"), { category: "proxy" }); },
...
test("constructs the concrete runtime only from fixed dependencies", () => {
  assert.doesNotThrow(() => new DockerNangoProxyRuntime({
    pathValue: "/safe/bin",
    platform: "linux/arm64",
...
test("concrete runtime reconciles ambiguous lifecycle mutations and coherent cleanup", async () => {
  const parent = await mkdtemp(join(tmpdir(), "zasp-m0-15-runtime-test-"));
  ...
  const imageFor = (role) => {
    ...
    return {
      Id: nangoImage ? PROXY_PINS.nango.configDigest : `sha256:${"1".repeat(64)}`,
  ...
  const command = async (commandName, arguments_, options) => {
    ...
    if (arguments_[0] === "logs") {
      if (arguments_[1] === identifiers.fixture) return result({ stdout: "Nango proxy fixture ready.\n" });
      if (arguments_[1] === identifiers.wrapper) return result({ stdout: `${JSON.stringify({ organizationId: `org_${marker}`, integrationKey: `zasp-m0-15-${marker}-1password-events`, connectionId: "00000000-0000-4000-8000-0000000000b4", event: { id: "11111111-1111-4111-8111-111111111111", action: "item.usage" } })}\n` });
    }
  ...
  try {
    runtime = new DockerNangoProxyRuntime({
      pathValue: "/safe/bin",
      platform: "linux/arm64",
      tempParent: parent,
      markerSource: () => marker,
    ...
    if (runtime?.workspace !== undefined) await removeProxyWorkspace(runtime.workspace).catch(() => {});
