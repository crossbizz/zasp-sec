...
import { validateProxyMarker } from "./boundary.mjs";

export { validateProxyMarker };

export const PROXY_PROOF_LABEL = "m0-15";
export const PROXY_PINS = PINS;
...
export function buildProxyRuntimeSpec(input) {
  validateInput(input);
  const marker = validateProxyMarker(input.marker);
  const prefix = `zasp-m0-15-${marker}`;
  const expectedRootPrefix = `${prefix}-`;
  const rootName = basename(input.workspaceRoot);
  if (!rootName.startsWith(expectedRootPrefix) || !/^[A-Za-z0-9]{6}$/.test(rootName.slice(expectedRootPrefix.length))) throw new TypeError("proxy workspace root is invalid");
  ...
  const labels = (role) => ({ "zasp.dev/proof": PROXY_PROOF_LABEL, "zasp.dev/run": marker, "zasp.dev/role": role });
  ...
  return deepFreeze({
    ...
    nango: {
      ...
      environment: {
        ...
        NANGO_DB_APPLICATION_NAME: "zasp-m0-15-proof",
    ...
    wrapper: {
      ...
      environment: {
        NODE_EXTRA_CA_CERTS: "/proof/tls/ca.crt",
        NANGO_PROXY_BASE_URL: `http://${nangoContainerName}:3003`,
        NANGO_PROXY_END_USER_ID: `user_${marker}`,
        NANGO_PROXY_ENVIRONMENT: "dev",
        NANGO_PROXY_FORBIDDEN_VALUES: [input.password, input.encryptionKey].join(","),
        NANGO_PROXY_INTEGRATION_KEY: `${prefix}-1password-events`,
        NANGO_PROXY_ORGANIZATION_ID: `org_${marker}`,
        NANGO_PROXY_PROVIDER_KEY: input.providerKey,
      },
      entrypoint: ["/usr/local/bin/node"],
      command: ["/proofs/nango-proxy/product_wrapper.mjs"],
      readOnlyRootfs: true,
      tmpfs: { "/tmp": "rw,noexec,nosuid,nodev,size=16m" },
      mounts: [
        { source: input.proofSourcePath, target: "/proofs", readOnly: true },
...
function validateInput(input) {
  if (!plainObject(input)) throw new TypeError("proxy runtime input is invalid");
  ...
  if (!sameArray(actual, expected)) throw new TypeError("proxy runtime input is invalid");
  validateProxyMarker(input.marker);
  if (!supportedPlatforms.has(input.platform) || !passwordPattern.test(input.password) || !validEncryptionKey(input.encryptionKey) || !providerKeyPattern.test(input.providerKey)) throw new TypeError("proxy runtime secret is invalid");
  for (const path of [input.workspaceRoot, input.dockerConfigPath, input.caCertificatePath, input.fixtureCertificatePath, input.fixtureKeyPath, input.proofSourcePath]) {
    if (typeof path !== "string" || !isAbsolute(path) || resolve(path) !== path || path.includes("\0")) throw new TypeError("proxy runtime path is invalid");
  }
}
