...
export const SUCCESS_LINE = "Nango OAuth proof passed: oauth=true reference=true product_state_safe=true cleanup=true.";
...
export function buildContainerCreateArguments(specification, role) {
  expectSpecification(specification);
  if (!specification.roles.includes(role)) throw new TypeError("OAuth role is invalid");
...
export function parseWrapperOutput(value) {
  ...
  if (
    ...
    parsed.integrationKey !== `zasp-m0-14b-${parsed.organizationId.slice(4)}-github`
...
export async function orchestrate(runtime, options = {}) {
  ...
  return { oauth: true, reference: true, productStateSafe: true, cleanup: true };
}

export async function runMain(dependencies = {}) {
  const writeLine = dependencies.writeLine ?? ((line) => process.stdout.write(`${line}\n`));
  try {
    const runtime = dependencies.runtime ?? new DockerNangoOAuthRuntime({
...
export class DockerNangoOAuthRuntime {
  constructor({
    pathValue,
    platform,
    tempParent = tmpdir(),
  ...
  async #initialize(signal) {
    ...
    assertActive(signal);
    validateOAuthTemporaryPrefixEntries(await this.filesystem.readdir(this.tempParent));
  ...
  async startFixture(signal) {
    ...
    for (let attempt = 0; attempt < 40; attempt += 1) {
      ...
      if (result.status === 0 && result.signal === null && result.stdout === "Nango OAuth fixture ready.\n" && result.stderr === "") return;
  ...
  async #startWrapper(signal) {
    ...
    if (classification === "definitive") throw oauthError();
    if (result.status === 0 && result.signal === null && result.stderr === "") this.wrapperOutput = Buffer.from(result.stdout);
      ...
      if (logs.status !== 0 || logs.signal !== null || logs.stderr !== "") throw oauthError();
  ...
  async verifyReady(signal) {
    ...
    if (result.organizationId !== this.specification.wrapper.environment.NANGO_OAUTH_ORGANIZATION_ID || result.integrationKey !== this.specification.wrapper.environment.NANGO_OAUTH_INTEGRATION_KEY) throw normalizationError();
  ...
  async requireFinalAbsence(signal) {
    ...
    if (globalAbsence && this.workspace !== undefined) {
      try { await removeOAuthWorkspace(this.workspace); this.workspace = undefined; }
      catch (error) { firstError ??= error; }
    }
    try { validateOAuthTemporaryPrefixEntries(await this.filesystem.readdir(this.tempParent)); }
    catch (error) { firstError ??= error; }
    if (firstError !== undefined) throw cleanupErrorWithCause(firstError);
  }
  ...
  async #resolveImage(expected, signal) {
    ...
    if (
      ...
      (["nango", "fixture", "wrapper"].includes(expected.role) && document.Id !== OAUTH_PINS.nango.configDigest)
    ) throw providerError();
    return document;
  }
  ...
  async #reconcileStopped(role, signal) {
    for (let attempt = 0; attempt < 30; attempt += 1) {
      ...
      if (!["created", "running"].includes(document.State.Status)) throw oauthError();
      if (attempt < 29) await delay(100, signal);
    }
    throw oauthError();
  }
  ...
  async #verifyContainer(role, signal, expectedState, cleanup = false) {
    ...
    validateOAuthContainerIdentity(document, {
      resource,
  ...
  async #requireGlobalAbsence(signal) {
    const prefix = "zasp-m0-14b-";
    const commands = [
      ["ps", "-a", "--no-trunc", "--filter", `name=^/${prefix}`, "--format", "{{.ID}}"],
      ["ps", "-a", "--no-trunc", "--filter", `label=zasp.dev/proof=${OAUTH_PROOF_LABEL}`, "--format", "{{.ID}}"],
      ["network", "ls", "--no-trunc", "--filter", `name=^${prefix}`, "--format", "{{.ID}}"],
      ["network", "ls", "--no-trunc", "--filter", `label=zasp.dev/proof=${OAUTH_PROOF_LABEL}`, "--format", "{{.ID}}"],
    ];
  ...
  async #docker(arguments_, timeoutMs, signal) {
    assertActive(signal);
    if (this.workspace !== undefined) await reproveOAuthWorkspace(this.workspace);
...
export function validateOAuthContainerIdentity(document, { resource, expected, image, networkName, networkId, expectedState, cleanup = false }) {
...
function expectSpecification(value) {
  if (!plainObject(value) || value.prefix !== `zasp-m0-14b-${value.marker}` || value.network?.labels?.["zasp.dev/proof"] !== OAUTH_PROOF_LABEL || !Array.isArray(value.roles)) throw new TypeError("OAuth runtime specification is invalid");
}
...
function allowedCategory(category) {
  return new Set(["configuration", "provider", "oauth", "normalization", "ownership", "cleanup", "operation"]).has(category) ? category : "operation";
}
...
function oauthError() { return Object.assign(new Error("oauth rejected"), { category: "oauth" }); }
