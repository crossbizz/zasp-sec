...
import {
  ...
  removeApiKeyWorkspace,
  reproveApiKeyWorkspace,
  validateApiKeyTemporaryPrefixEntries,
} from "./boundary.mjs";
import {
  API_KEY_PROOF_LABEL,
  API_KEY_PINS,
  buildApiKeyRuntimeSpec,
  validateApiKeyMarker,
} from "./manifest.mjs";

export { classifyMutationResult };

export const SUCCESS_LINE = "Nango API key proof passed: api_key=true reference=true product_state_safe=true cleanup=true.";
...
export function buildContainerCreateArguments(specification, role) {
  expectSpecification(specification);
  if (!specification.roles.includes(role)) throw new TypeError("API-key role is invalid");
...
export function parseWrapperOutput(value) {
  ...
  if (
    !plainObject(parsed) || !sameKeys(parsed, ["connectionId", "integrationKey", "organizationId"]) ||
    ...
    parsed.integrationKey !== `zasp-m0-14c-${parsed.organizationId.slice(4)}-1password-events`
...
export async function orchestrate(runtime, options = {}) {
  ...
  return { apiKey: true, reference: true, productStateSafe: true, cleanup: true };
}

export async function runMain(dependencies = {}) {
  const writeLine = dependencies.writeLine ?? ((line) => process.stdout.write(`${line}\n`));
  try {
    const runtime = dependencies.runtime ?? new DockerNangoApiKeyRuntime({
...
export class DockerNangoApiKeyRuntime {
  constructor({
    pathValue,
    platform,
    tempParent = tmpdir(),
  ...
  async #initialize(signal) {
    ...
    assertActive(signal);
    validateApiKeyTemporaryPrefixEntries(await this.filesystem.readdir(this.tempParent));
    const marker = validateApiKeyMarker(this.markerSource());
    const proofSourcePath = resolve(fileURLToPath(new URL("..", import.meta.url)));
    this.workspace = await createApiKeyWorkspace({ marker, tempParent: this.tempParent, proofSourcePath });
    this.environment = { PATH: this.pathValue, DOCKER_CONFIG: this.workspace.dockerConfig.path };
    this.specification = buildApiKeyRuntimeSpec({ ...this.workspace.runtimeInput, platform: this.platform });
    assertActive(signal);
  }
  ...
  async startFixture(signal) {
    ...
    for (let attempt = 0; attempt < 40; attempt += 1) {
      ...
      if (result.status === 0 && result.signal === null && result.stdout === "Nango API-key fixture ready.\n" && result.stderr === "") return;
  ...
  async #startWrapper(signal) {
    ...
    if (classification === "definitive") throw apiKeyError();
    if (result.status === 0 && result.signal === null && result.stderr === "") this.wrapperOutput = Buffer.from(result.stdout);
      ...
      if (logs.status !== 0 || logs.signal !== null || logs.stderr !== "") throw apiKeyError();
  ...
  async verifyReady(signal) {
    ...
    if (result.organizationId !== this.specification.wrapper.environment.NANGO_API_KEY_ORGANIZATION_ID || result.integrationKey !== this.specification.wrapper.environment.NANGO_API_KEY_INTEGRATION_KEY) throw normalizationError();
  ...
  async requireFinalAbsence(signal) {
    ...
    if (globalAbsence && this.workspace !== undefined) {
      try { await removeApiKeyWorkspace(this.workspace); this.workspace = undefined; }
      catch (error) { firstError ??= error; }
    }
    try { validateApiKeyTemporaryPrefixEntries(await this.filesystem.readdir(this.tempParent)); }
    catch (error) { firstError ??= error; }
    if (firstError !== undefined) throw cleanupErrorWithCause(firstError);
  }
  ...
  async #resolveImage(expected, signal) {
    ...
    if (
      ...
      (["nango", "fixture", "wrapper"].includes(expected.role) && document.Id !== API_KEY_PINS.nango.configDigest)
    ) throw providerError();
    return document;
  }
  ...
  async #reconcileStopped(role, signal) {
    for (let attempt = 0; attempt < 30; attempt += 1) {
      ...
      if (!["created", "running"].includes(document.State.Status)) throw apiKeyError();
      if (attempt < 29) await delay(100, signal);
    }
    throw apiKeyError();
  }
  ...
  async #verifyContainer(role, signal, expectedState, cleanup = false) {
    ...
    validateApiKeyContainerIdentity(document, {
      resource,
  ...
  async #requireGlobalAbsence(signal) {
    const prefix = "zasp-m0-14c-";
    const commands = [
      ["ps", "-a", "--no-trunc", "--filter", `name=^/${prefix}`, "--format", "{{.ID}}"],
      ["ps", "-a", "--no-trunc", "--filter", `label=zasp.dev/proof=${API_KEY_PROOF_LABEL}`, "--format", "{{.ID}}"],
      ["network", "ls", "--no-trunc", "--filter", `name=^${prefix}`, "--format", "{{.ID}}"],
      ["network", "ls", "--no-trunc", "--filter", `label=zasp.dev/proof=${API_KEY_PROOF_LABEL}`, "--format", "{{.ID}}"],
    ];
  ...
  async #docker(arguments_, timeoutMs, signal) {
    assertActive(signal);
    if (this.workspace !== undefined) await reproveApiKeyWorkspace(this.workspace);
...
export function validateApiKeyContainerIdentity(document, { resource, expected, image, networkName, networkId, expectedState, cleanup = false }) {
...
function expectSpecification(value) {
  if (!plainObject(value) || value.prefix !== `zasp-m0-14c-${value.marker}` || value.network?.labels?.["zasp.dev/proof"] !== API_KEY_PROOF_LABEL || !Array.isArray(value.roles)) throw new TypeError("API-key runtime specification is invalid");
}
...
function allowedCategory(category) {
  return new Set(["configuration", "provider", "api_key", "normalization", "ownership", "cleanup", "operation"]).has(category) ? category : "operation";
}
...
function apiKeyError() { return Object.assign(new Error("API key rejected"), { category: "api_key" }); }
