...
async function withParent(callback) {
  const parent = await mkdtemp(join(tmpdir(), "zasp-m0-14c-boundary-parent-"));
...
test("creates an exact owned workspace with one synthetic provider key and TLS SAN", async () => {
  await withParent(async (parent) => {
    const workspace = await createApiKeyWorkspace({
      marker,
      tempParent: parent,
    ...
    assert.match(basename(workspace.root.path), /^zasp-m0-14c-0123456789abcdef-[A-Za-z0-9]{6}$/);
...
test("accepts real bounded OpenSSL output and binds every generated file", async () => {
  await withParent(async (parent) => {
    const workspace = await createApiKeyWorkspace({ marker, tempParent: parent, proofSourcePath: join(process.cwd(), "proofs") });
    assert.deepEqual(await reproveApiKeyWorkspace(workspace), workspace);
    await writeFile(join(workspace.tls.directory.path, "unexpected"), "foreign");
    await assert.rejects(() => reproveApiKeyWorkspace(workspace));
  });
});

test("rejects TLS content, mode, Docker-config, and root identity drift", async () => {
  await withParent(async (parent) => {
    const workspace = await createApiKeyWorkspace({ marker, tempParent: parent, proofSourcePath: join(process.cwd(), "proofs"), randomSource: deterministicRandom, runCommand: fakeOpenSSL });
...
test("cleans only an exact retained candidate after creation failure", async () => {
  await withParent(async (parent) => {
    await assert.rejects(() => createApiKeyWorkspace({
      marker,
      tempParent: parent,
      proofSourcePath: join(process.cwd(), "proofs"),
      randomSource: deterministicRandom,
    ...
    assert.deepEqual((await readdir(parent)).filter((name) => name.startsWith("zasp-m0-14c-")), []);
  });
});
...
test("skips destructive cleanup after workspace replacement", async () => {
  await withParent(async (parent) => {
    ...
    await assert.rejects(() => removeApiKeyWorkspace(workspace));
    assert.equal((await lstat(workspace.root.path)).isDirectory(), true);
  });
});

test("rejects stale API-key roots across markers and invalid marker grammar", () => {
  assert.equal(validateApiKeyMarker(marker), marker);
  assert.equal(validateApiKeyTemporaryPrefixEntries(["unrelated"]), true);
  for (const entries of [["zasp-m0-14c-ffffffffffffffff-ABC123"], ["zasp-m0-14c-invalid"], "bad", [1]]) {
    assert.throws(() => validateApiKeyTemporaryPrefixEntries(entries));
  }
  for (const value of ["", "A".repeat(16), "a".repeat(15), "a".repeat(17), 1]) {
    assert.throws(() => validateApiKeyMarker(value));
  }
});
