...
describe("M1-19 product telemetry contract", () => {
  ...
  it("starts only M1-19 and preserves its prerequisite and blockers", async () => {
    ...
    expect(readme).toMatch(/M1-19\s+is\s+In progress/);
    ...
    expect(active.filter(([task]) => task === "M1-19")).toHaveLength(1);
