...
describe("M1-26 UI API coverage CI", () => {
  ...
  it("preserves three planned forward references while allowing later UI wiring", async () => {
    const map = await readFile(resolve(repositoryRoot, "docs/product/ui-api-map.yaml"), "utf8");
    expect(map.match(/availability: planned/g)).toHaveLength(3);
  ...
  it("documents the exact local coverage commands and honest current result", async () => {
    ...
    expect(section).toContain("UI/API coverage passed: planned=3 api_available=63 available=20 public=83 internal=0.");
