...
describe("M1-26 UI API coverage CI", () => {
  ...
  it("records the deliberately hidden provider and job forward references", async () => {
    ...
    expect(map.match(/availability: planned/g) ?? []).toHaveLength(42);
    expect(map.match(/availability: available/g)).toHaveLength(91);
  ...
  it("documents the exact local coverage commands and honest current result", async () => {
    ...
    expect(section).toContain("UI/API coverage passed: planned=42 api_available=7 available=91 public=98 internal=0.");
