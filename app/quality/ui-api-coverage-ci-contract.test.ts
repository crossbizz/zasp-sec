...
describe("M1-26 UI API coverage CI", () => {
  ...
  it("records the deliberately hidden provider and job forward references", async () => {
    ...
    expect(map.match(/availability: planned/g) ?? []).toHaveLength(52);
    expect(map.match(/availability: available/g)).toHaveLength(64);
    expect(map.match(/availability: api_available/g)).toHaveLength(20);
  ...
  it("documents the exact local coverage commands and honest current result", async () => {
    ...
    expect(section).toContain("UI/API coverage passed: planned=52 api_available=20 available=64 public=84 internal=0.");
