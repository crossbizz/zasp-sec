...
describe("M1-28c event-ingest health wiring", () => {
  ...
  it("starts only M1-28c after M1-28b and preserves exact blockers", async () => {
    ...
    expect(readme).toContain("M1-28d remains Pending");
    ...
    expect(complete.filter(([task]) => task === "M1-28c")).toHaveLength(0);
    expect([...active, ...complete].filter(([task]) => task === "M1-28d")).toHaveLength(0);
