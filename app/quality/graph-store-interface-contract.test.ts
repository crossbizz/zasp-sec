...
describe("M1-15 GraphStore interface contract", () => {
  ...
  it("completes only M1-15 and preserves prior completion and blockers", async () => {
    ...
    expect(milestones.find(([milestone]) => milestone === "M1")).toEqual(["M1", "68", "46", "1", "21", "0"]);
    ...
    expect(active.filter(([task]) => task === "M1-16")).toHaveLength(1);
    expect(complete.filter(([task]) => task === "M1-16")).toHaveLength(0);
  ...
  it("exposes only the hermetic product contract without Neo4j or raw queries", async () => {
    ...
    expect(section).toContain("M1-16 is In progress");
