...
describe("M6 gate and M7 session-foundation completion batch", () => {
  ...
  it("records exactly 25 reviewed tasks as Complete", async () => {
    ...
    for (const value of ["| Pending | 0 |", "| In progress | 316 |", "| Complete | 409 |", "| Blocked | 3 |", "`0/316/409/3`", "| M6 | 36 | 0 | 0 | 36 | 0 |", "| M7 | 62 | 0 | 56 | 6 | 0 |"]) expect(tracker).toContain(value);
    ...
    expect(prose).toContain("M7-01 through M7-06 are Complete");
    expect(prose).toContain("M7-07 through M7-40 remain batched as In progress");
  });
});
