...
describe("M3 gate and M4 reconciliation accelerated batch", () => {
  ...
  it("moves exactly the 22-task batch to In progress without a provider claim", async () => {
    ...
    expect(tracker).toContain("| M3 | 75 | 0 | 11 | 64 | 0 |");
    expect(tracker).toMatch(/^\| M4 \| 82 \| \d+ \| \d+ \| 0 \| 0 \|$/m);
    const active = tracker.match(/## In progress[\s\S]*?## Complete/)?.[0] ?? "";
    const complete = tracker.match(/## Complete[\s\S]*?## Blocked/)?.[0] ?? "";
    for (const task of ["M3-48d", "M3-48e", "M3-48f", "M3-48g", "M3-48h"]) expect(complete.match(new RegExp(`^\\| ${task} \\|`, "gm"))).toHaveLength(1);
    const tasks = ["M3-48", "M3-49", "M3-50", "M3-51", "M3-52a", "M3-52b", "M3-52c", "M3-52d", "M3-52e", "M3-52", "M4-01a", "M4-01b", "M4-01c", "M4-01d", "M4-01e", "M4-01f", "M4-01"];
