...
describe("M7 gate and Security Agent foundation batch", () => {
  ...
  it("records the M7 gate complete and Security Agent foundation active", () => {
    const tracker = read("docs/internal/implementation_status_v1.5.md");
    for (const value of ["| Pending | 0 |", "| In progress | 260 |", "| Complete | 465 |", "| Blocked | 3 |", "`0/260/465/3`", "| M7 | 62 | 0 | 0 | 62 | 0 |", "| M7A | 113 | 0 | 113 | 0 | 0 |"]) expect(tracker).toContain(value);
    ...
    for (const task of ["M7A-01", "M7A-02", "M7A-03", "M7A-04", "M7A-05", "M7A-06", "M7A-07", "M7A-08", "M7A-09", "M7A-10", "M7A-11", "M7A-12", "M7A-13", "M7A-14", "M7A-15", "M7A-16", "M7A-17"]) expect(active.match(new RegExp(`^\\| ${task} \\|`, "gm"))).toHaveLength(1);
