...
function assertM015Active(tracker: string, readme: string, riskRegister: string) {
  ...
  expect([...activeRows, ...completeRows].filter(([task]) => task === "M0-16")).toHaveLength(0);
...
function assertM015Complete(tracker: string, readme: string, riskRegister: string) {
  ...
  expect(activeRows).toHaveLength(1);
  expect(activeRows[0]?.[0]).toBe("M0-20");
...
function activeFixture(tracker: string, readme: string) {
  ...
  return {
    ...
    tracker: tracker
      .replace("| Pending | 704 |", "| Pending | 709 |")
      .replace("| Complete | 19 |", "| Complete | 16 |")
      .replace("| Blocked | 3 |", "| Blocked | 1 |")
      .replace("| M0 | 27 | 3 | 1 | 19 | 3 |", "| M0 | 27 | 8 | 1 | 16 | 1 |")
      .replace("`704/1/19/3`", "`709/1/16/1`")
