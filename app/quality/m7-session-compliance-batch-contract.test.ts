...
describe("M7 sessions, compliance, and data-controls batch", () => {
  ...
  it("records the first six tasks complete and the remaining local M7 slice active", () => {
    const tracker = read("docs/internal/implementation_status_v1.5.md");
    for (const value of ["| Pending | 0 |", "| In progress | 316 |", "| Complete | 409 |", "| Blocked | 3 |", "`0/316/409/3`", "| M7 | 62 | 0 | 56 | 6 | 0 |"]) expect(tracker).toContain(value);
    const active = tracker.match(/## In progress[\s\S]*?## Complete/)?.[0] ?? "";
    const complete = tracker.match(/## Complete[\s\S]*?## Blocked/)?.[0] ?? "";
    for (const task of ["M7-01","M7-02","M7-03","M7-04","M7-05","M7-06"]) expect(complete.match(new RegExp(`^\\| ${task} \\|`, "gm"))).toHaveLength(1);
    for (const task of ["M7-07a","M7-07b","M7-07c","M7-07","M7-08","M7-09","M7-10","M7-11","M7-12","M7-13","M7-14","M7-15a","M7-15b","M7-15c","M7-15d","M7-15","M7-16","M7-17","M7-18"]) expect(active.match(new RegExp(`^\\| ${task} \\|`, "gm"))).toHaveLength(1);
