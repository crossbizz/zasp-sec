...
export function ProductionPoliciesView({ canWrite }: { canWrite: boolean }) {
  ...
  return <div className="page"><PageHeader title="Policies" description="Durable scoped runtime controls with version checks and audit receipts." />
    <FeedbackLine value={feedback} /><QueryBoundary status={query.status} label="policies" onRetry={query.retry} disabled={mutation.hasAmbiguousAttempt} />
    {mutation.hasAmbiguousAttempt && <p role="alert">The response was lost. The exact operation and idempotency key are retained. <Button disabled={busy} onClick={retryMutation}>Retry retained policy operation</Button></p>}
    {canWrite && <Card title="Create policy"><div className="form-grid"><Field label="Policy ID" value={policyID} disabled={mutation.hasAmbiguousAttempt} pattern="policy-[a-z0-9-]+" onChange={(event) => setPolicyID(event.target.value)} /><Field label="Name" value={name} disabled={mutation.hasAmbiguousAttempt} maxLength={128} onChange={(event) => setName(event.target.value)} /></div><Button variant="primary" disabled={busy || mutation.hasAmbiguousAttempt || !policyID || !name} onClick={create}>Create policy</Button></Card>}
    <Card title="Authorized policies">{query.data?.length ? <div className="connection-list">{query.data.map((policy) => <button type="button" key={policy.id} disabled={busy || mutation.hasAmbiguousAttempt} onClick={() => open(policy.id)} aria-label={`Open ${policy.name}`}><strong>{policy.name}</strong><span>{policy.action}</span><span>{policy.rollout}</span></button>)}</div> : query.status === "empty" ? <EmptyState title="No policies" description="Create the first scoped runtime policy when write access is enabled." /> : null}</Card>
    {selected && <Card title={`Policy detail · ${selected.value.id}`}><p><Badge tone="info">{selected.value.rollout}</Badge> Version {selected.version}</p><Field label="Policy name" value={editName} disabled={!canWrite || busy || mutation.hasAmbiguousAttempt} onChange={(event) => setEditName(event.target.value)} /><p>{selected.value.conditions.map((condition) => `${condition.field} ${condition.operator} ${condition.value}`).join(", ")}</p>{canWrite && <div className="button-row"><Button disabled={busy || mutation.hasAmbiguousAttempt} onClick={update}>Save changes</Button>{selected.value.rollout === "draft" && <Button disabled={busy || mutation.hasAmbiguousAttempt} onClick={() => rollout("monitor")}>Roll to monitor</Button>}{selected.value.rollout === "monitor" && <Button variant="primary" disabled={busy || mutation.hasAmbiguousAttempt} onClick={() => rollout("enforced")}>Enforce policy</Button>}{(selected.value.rollout === "monitor" || selected.value.rollout === "enforced") && <Button disabled={busy || mutation.hasAmbiguousAttempt} onClick={disable}>Disable policy</Button>}</div>}</Card>}
  </div>;
}
...
export function ProductionIntegrationsView({ canWrite }: { canWrite: boolean }) {
  ...
  return <div className="page"><PageHeader title="Integrations" description="Durable local connector configuration. Provider authorization and sync controls appear only when a real adapter exists." /><FeedbackLine value={feedback} />{mutation.hasAmbiguousAttempt && <p role="alert">The response was lost. The exact operation and idempotency key are retained. <Button disabled={busy} onClick={retryMutation}>Retry retained integration operation</Button></p>}
    <QueryBoundary status={integrations.status} label="integrations" onRetry={integrations.retry} disabled={mutation.hasAmbiguousAttempt} /><Card title="Configured integrations">{integrations.data?.length ? <div className="connection-list">{integrations.data.map((value) => <button type="button" key={value.id} disabled={busy || mutation.hasAmbiguousAttempt} aria-label={`Open ${value.name}`} onClick={() => open(value.id)}><strong>{value.name}</strong><span>{value.connector_key}</span><span>{value.status}</span></button>)}</div> : integrations.status === "empty" ? <EmptyState title="No integrations" description="Choose a supported connector catalog entry to save its scoped configuration." /> : null}</Card>
    <QueryBoundary status={catalog.status} label="integration catalog" onRetry={catalog.retry} disabled={mutation.hasAmbiguousAttempt} />{canWrite && <Card title="Connector catalog"><div className="connector-grid">{catalog.data?.map((value) => <article key={value.key} className="connector-card"><h3>{value.provider}</h3><p>{value.description}</p><Badge tone="info">{value.auth_mode}</Badge><Button disabled={busy || mutation.hasAmbiguousAttempt} onClick={() => choose(value)}>Configure {value.provider}</Button></article>)}</div></Card>}
    <Modal open={manifest !== null} title={`Configure ${manifest?.provider ?? "integration"}`} closeDisabled={mutation.hasAmbiguousAttempt} onClose={() => setManifest(null)} footer={<><Button disabled={mutation.hasAmbiguousAttempt} onClick={() => setManifest(null)}>Cancel</Button><Button variant="primary" disabled={busy || mutation.hasAmbiguousAttempt || !name || manifest?.setup_schema.some((field) => field.required && !configuration[field.key])} onClick={create}>Save integration</Button></>}>{manifest && <div className="form-stack"><p>{manifest.access_guidance}</p><Field label="Integration name" value={name} disabled={mutation.hasAmbiguousAttempt} onChange={(event) => setName(event.target.value)} />{manifest.setup_schema.map((field) => <Field key={field.key} label={field.label} hint={field.description} value={configuration[field.key] ?? ""} disabled={mutation.hasAmbiguousAttempt} onChange={(event) => setConfiguration((current) => ({ ...current, [field.key]: event.target.value }))} />)}</div>}</Modal>
    <Modal open={selected !== null} title={selected?.value.name ?? "Integration"} closeDisabled={mutation.hasAmbiguousAttempt} onClose={() => setSelected(null)} footer={<><Button disabled={mutation.hasAmbiguousAttempt} onClick={() => setSelected(null)}>Close</Button>{canWrite && <><Button disabled={busy || mutation.hasAmbiguousAttempt} onClick={update}>Save changes</Button><Button variant="danger" disabled={busy || mutation.hasAmbiguousAttempt} onClick={remove}>Delete integration</Button></>}</>}>{selected && <div className="form-stack"><p><Badge tone="info">{selected.value.status}</Badge> Version {selected.version}</p><Field label="Integration name" value={name} disabled={!canWrite || mutation.hasAmbiguousAttempt} onChange={(event) => setName(event.target.value)} />{Object.entries(configuration).map(([key, value]) => <Field key={key} label={key.replaceAll("_", " ")} value={value} disabled={!canWrite || mutation.hasAmbiguousAttempt} onChange={(event) => setConfiguration((current) => ({ ...current, [key]: event.target.value }))} />)}<p>Provider authorization and sync are capability-hidden for this deployment.</p></div>}</Modal>
  </div>;
}
