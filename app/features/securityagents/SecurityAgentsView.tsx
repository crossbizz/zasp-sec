...
export type SecurityAgentsAPI = {
  ...
  createSecurityAgent(value: SecurityAgentInput): Promise<SecurityAgentDefinition>;
  ...
  listSecurityAgentRuns(signal?: AbortSignal): Promise<SecurityAgentRunPage>;
  getSecurityAgentRun(id: string): Promise<SecurityAgentRunDetail>;
  cancelSecurityAgentRun(id: string): Promise<SecurityAgentRun>;
  listSecurityAgentApprovals(signal?: AbortSignal): Promise<SecurityAgentApprovalPage>;
...
export function createSecurityAgentsAPI(client: APIClient = createAPIClient()): SecurityAgentsAPI {
  const version = async (path: "/api/v1/security-agents/{id}" | "/api/v1/security-agent-runs/{id}" | "/api/v1/security-agent-approvals/{id}", id: string) => {
    const result = await client.GET(path, { params: { path: { id } } });
    if (result.error) requireData(result);
    const value = result.response.headers.get("ETag");
    if (!value || !/^"[1-9][0-9]*"$/.test(value)) throw new Error("Security Agent API omitted resource version");
    return value;
  };
  return {
    async listSecurityAgentTemplates(signal) { return requireData<{ items: readonly SecurityAgentTemplate[] }>(await client.GET("/api/v1/security-agent-templates", { signal })).items; },
    async listSecurityActions(signal) { return requireData<{ items: readonly SecurityAction[] }>(await client.GET("/api/v1/security-actions", { signal })).items; },
    async listSecurityAgents(signal) { return requireData<SecurityAgentPage>(await client.GET("/api/v1/security-agents", { params: { query: { limit: 50 } }, signal })); },
    async createSecurityAgent(value) { return requireData<SecurityAgentDefinition>(await client.POST("/api/v1/security-agents", { params: { header: { "Idempotency-Key": workflowIdempotencyKey() } }, body: value })); },
    ...
    async updateSecurityAgent(id, value) { return requireData<SecurityAgentDefinition>(await client.PATCH("/api/v1/security-agents/{id}", { params: { path: { id }, header: { "Idempotency-Key": workflowIdempotencyKey(), "If-Match": await version("/api/v1/security-agents/{id}", id) } }, body: value })); },
    async deleteSecurityAgent(id) { const response = await client.DELETE("/api/v1/security-agents/{id}", { params: { path: { id }, header: { "Idempotency-Key": workflowIdempotencyKey(), "If-Match": await version("/api/v1/security-agents/{id}", id) } } }); if (response.error) throw new Error("Security Agent API rejected"); },
    async simulateSecurityAgent(id, value) { return requireData<SecurityAgentSimulation>(await client.POST("/api/v1/security-agents/{id}/simulate", { params: { path: { id }, header: { "Idempotency-Key": workflowIdempotencyKey(), "If-Match": await version("/api/v1/security-agents/{id}", id) } }, body: value })); },
    async runSecurityAgent(id, value) { return requireData<SecurityAgentRun>(await client.POST("/api/v1/security-agents/{id}/runs", { params: { path: { id }, header: { "Idempotency-Key": workflowIdempotencyKey() } }, body: value })); },
    async listSecurityAgentRuns(signal) { return requireData<SecurityAgentRunPage>(await client.GET("/api/v1/security-agent-runs", { params: { query: { limit: 50 } }, signal })); },
    ...
    async cancelSecurityAgentRun(id) { return requireData<SecurityAgentRun>(await client.POST("/api/v1/security-agent-runs/{id}/cancel", { params: { path: { id }, header: { "Idempotency-Key": workflowIdempotencyKey(), "If-Match": await version("/api/v1/security-agent-runs/{id}", id) } } })); },
    async listSecurityAgentApprovals(signal) { return requireData<SecurityAgentApprovalPage>(await client.GET("/api/v1/security-agent-approvals", { signal })); },
    ...
    async decideSecurityAgentApproval(id, value) { return requireData<SecurityAgentApproval>(await client.POST("/api/v1/security-agent-approvals/{id}/decision", { params: { path: { id }, header: { "Idempotency-Key": workflowIdempotencyKey(), "If-Match": await version("/api/v1/security-agent-approvals/{id}", id), "X-Zasp-Fresh-Auth": "confirmed" } }, body: value })); },
  };
}
...
type SecurityAgentTab = "agents" | "runs" | "approvals" | "builder";
const isSecurityAgentTab = (value: string): value is SecurityAgentTab => ["agents", "runs", "approvals", "builder"].includes(value);
...
async function loadSecurityAgentSnapshot(api: SecurityAgentsAPI, signal?: AbortSignal): Promise<SecurityAgentSnapshot> {
  const [agentPage, templates, actions, runPage, approvalPage] = await Promise.all([api.listSecurityAgents(signal), api.listSecurityAgentTemplates(signal), api.listSecurityActions(signal), api.listSecurityAgentRuns(signal), api.listSecurityAgentApprovals(signal)]);
  return { agents: agentPage.items, templates, actions, runs: runPage.items, approvals: approvalPage.items };
}

function Builder({ templates, actions, agents, api, environmentID, initialEvidenceID, onCreated }: { templates: readonly SecurityAgentTemplate[]; actions: readonly SecurityAction[]; agents: readonly SecurityAgentDefinition[]; api: SecurityAgentsAPI; environmentID: string; initialEvidenceID: string; onCreated(agent: SecurityAgentDefinition): void }) {
  ...
  const save = async () => {
    const created = await api.createSecurityAgent({
      name, trigger_kind: trigger, trigger_source: triggerSources[trigger], environment_ids: [environment], autonomy,
  ...
  const simulate = async () => {
    ...
    if (!agent) return;
    setSimulation(await api.simulateSecurityAgent(agent.id, { goal, environment_id: environment, evidence_ids: [evidenceID] }));
  };
  ...
  return <Card className="security-agent-builder">
    <div className="wizard-layout">
      ...
      <section className="wizard-main">
        ...
        {stage === "Goal / Scope" && <div className="form-stack"><Field label="Short goal" value={goal} maxLength={1024} onChange={(event) => setGoal(event.target.value)} /><Field label="Authorized environment" value={environment} readOnly /><small>Changing the goal never changes the allowed-action list. Add or remove actions explicitly.</small></div>}
        ...
        {stage === "Simulate" && <div className="form-stack"><p>Simulation matches scoped evidence, proposes a plan, and evaluates authorization without action side effects.</p><Field label="Evidence ID" value={evidenceID} onChange={(event) => setEvidenceID(event.target.value)} /><Button variant="primary" disabled={!name || !verificationCompatible} onClick={() => void save()}>Save Security Agent</Button><Button disabled={!(createdAgent || agents.length) || !verification || !evidenceID} onClick={() => void simulate()}>Simulate with selected evidence</Button>{simulation && <div className="review-summary"><div><span>Matched evidence</span><strong>{simulation.matched_evidence_ids.length}</strong></div><div><span>Proposed plan</span><strong>{simulation.summary}</strong></div><div><span>Approval points</span><strong>{simulation.steps.filter((value) => value.approval_required).length}</strong></div><div><span>Side effects</span><strong>{simulation.side_effects}</strong></div>{simulation.steps.map((value) => <div key={value.index}><span>{value.action}</span><strong>authorization: {value.authorization}</strong></div>)}</div>}</div>}
      </section>
...
function AgentDetail({ agent, runs, actions, api, environmentID, canRun, onRun, onClose }: { agent: SecurityAgentDefinition; runs: readonly SecurityAgentRun[]; actions: readonly SecurityAction[]; api: SecurityAgentsAPI; environmentID: string; canRun: boolean; onRun(run: SecurityAgentRun): void; onClose: () => void }) {
  const allowed = actions.filter((value) => agent.allowed_actions.includes(value.key));
  const [evidenceID, setEvidenceID] = useState("");
  const [busy, setBusy] = useState(false);
  const start = async () => { setBusy(true); try { onRun(await api.runSecurityAgent(agent.id, { environment_id: environmentID, trigger_kind: "finding", trigger_id: evidenceID })); } finally { setBusy(false); } };
  return <Drawer open title={agent.name} onClose={onClose}><div className="detail-content"><h3>Definition</h3><p>{agent.trigger_kind} · {agent.trigger_source} · version {agent.definition_version}</p><h3>Allowed actions</h3>{allowed.map((value) => <div key={value.key}><strong>{value.key}</strong> · {value.risk_class} · {value.approval_floor}</div>)}<h3>Limits</h3><p>{agent.max_steps} steps · {agent.max_duration_seconds}s runtime · {agent.temporary_policy_seconds}s temporary policy · {agent.ai_token_budget} AI tokens · concurrency {agent.concurrency_limit} · {agent.autonomy}</p><h3>Verification</h3><p>{agent.verification_kind}</p>{canRun && <><h3>Start a bounded run</h3><Field label="Run evidence ID" value={evidenceID} onChange={(event) => setEvidenceID(event.target.value)} /><Button disabled={busy || !evidenceID} onClick={() => void start()}>Start bounded run</Button></>}<h3>Recent runs</h3>{runs.filter((value) => value.agent_id === agent.id).slice(0, 5).map((value) => <div key={value.id}>{value.id} · {value.state}</div>)}<small>Model and provider internals are available only in protected audit metadata.</small></div></Drawer>;
}

function RunDetail({ detail, api, canRun, onChange, onClose }: { detail: SecurityAgentRunDetail; api: SecurityAgentsAPI; canRun: boolean; onChange(value: SecurityAgentRunDetail): void; onClose: () => void }) {
  ...
  const refresh = async () => onChange(await api.getSecurityAgentRun(detail.run.id));
  const cancel = async () => onChange({ ...detail, run: await api.cancelSecurityAgentRun(detail.run.id) });
  const cancellable = !["contained", "remediated", "failed", "inconclusive", "cancelled"].includes(detail.run.state);
  return <Drawer open title={`Run ${detail.run.id}`} onClose={onClose}><div className="detail-content"><p><Badge tone="info">{detail.run.state}</Badge></p><div className="button-row"><Button onClick={() => void refresh()}>Refresh run</Button>{canRun && cancellable && <Button onClick={() => void cancel()}>Cancel run</Button>}</div><h3>Trigger / evidence</h3>{detail.evidence_ids.map((value) => <a key={value} href={`/violations?evidence_id=${encodeURIComponent(value)}#security-agent-run-${detail.run.id}`}>Scoped activity link · {value}</a>)}<h3>AI rationale</h3><p>{plan.summary || "Bounded planner rationale unavailable"}</p><h3>Deterministic authorization</h3><Badge tone="info">{detail.authorization}</Badge><h3>Ordered plan</h3>{(plan.steps || []).map((value) => <div key={value.index}><strong>{value.index + 1}. {value.action}</strong><code>{JSON.stringify(value.parameters || { protected: "[REDACTED]" })}</code></div>)}<h3>Action results</h3>{execution.map((value, index) => <div key={value.id || index}><strong>{value.action_key || `Step ${index + 1}`}</strong> · {value.state || "recorded"}<p>Protected arguments: [REDACTED]</p><p>Rollback / TTL: bounded by action metadata</p><p>Verification: {detail.verification}</p></div>)}{execution.length === 0 && <p>No provider action result is claimed while this run is awaiting authorization.</p>}<h3>Approvals</h3>{detail.approvals.map((value) => <div key={value.id}>{value.id} · {value.state}</div>)}</div></Drawer>;
}

function ApprovalDetail({ approval, api, canApprove, onClose }: { approval: SecurityAgentApproval; api: SecurityAgentsAPI; canApprove: boolean; onClose: () => void }) {
  ...
  return <Drawer open title={`Approval ${value.id}`} onClose={onClose}><div className="detail-content"><h3>Reason and evidence</h3>{(value.evidence_summary || []).map((item) => <a key={item} href={`/violations?evidence_id=${encodeURIComponent(item)}#security-agent-run-${value.run_id}`}>{item}</a>)}<h3>Expected side effect</h3><p>{value.expected_effect || "Bounded registered action"}</p><h3>Risk and reversibility</h3><p>{value.reversible ? "Reversible" : "Not reversible"} · TTL {value.ttl_seconds || 0}s</p><h3>Requester / run context</h3><a href={`/protect/security-agents?tab=runs&run_id=${encodeURIComponent(value.run_id)}`}>Scoped activity link · {value.run_id}</a><p>Fresh authentication is required before every sensitive decision.</p>{canApprove ? <div className="button-row"><Button onClick={() => void decide("approved")}>Approve</Button><Button onClick={() => void decide("rejected")}>Deny</Button><Button onClick={() => void decide("cancelled")}>Cancel</Button></div> : <p>Approval decisions are not enabled for your role.</p>}</div></Drawer>;
}

export function SecurityAgentsView({ api = defaultSecurityAgentsAPI, initialTab = "agents", environmentID = "production", initialEvidenceID = "evidence-selected", canWrite = true, canRun = true, canApprove = true, initialSnapshot, autoLoad = true }: { api?: SecurityAgentsAPI; initialTab?: SecurityAgentTab; environmentID?: string; initialEvidenceID?: string; canWrite?: boolean; canRun?: boolean; canApprove?: boolean; initialSnapshot?: SecurityAgentSnapshot; autoLoad?: boolean }) {
  ...
  useEffect(() => { if (!autoLoad) return; let active = true; const controller = new AbortController(); void loadSecurityAgentSnapshot(api, controller.signal).then((snapshot) => { if (active) { setAgents(snapshot.agents); setTemplates(snapshot.templates); setActions(snapshot.actions); setRuns(snapshot.runs); setApprovals(snapshot.approvals); } }, () => { if (active) setError(true); }); return () => { active = false; controller.abort(); }; }, [api, autoLoad]);
  ...
  const tabs = [{ id: "agents", label: "Security Agents", count: agents.length }, { id: "runs", label: "Runs", count: runs.length }, { id: "approvals", label: "Approvals", count: approvals.filter((value) => value.state === "pending").length }, ...(canWrite ? [{ id: "builder", label: "Builder" }] : [])];
  return <div className="page"><PageHeader title="Security agents" description="Bounded automated response definitions, approvals, and verified outcomes." actions={canWrite ? <Button variant="primary" onClick={() => setTab("builder")}>Create Security Agent</Button> : undefined} /><Tabs tabs={tabs} active={tab} onChange={(value) => { if (isSecurityAgentTab(value) && (value !== "builder" || canWrite)) setTab(value); }} />{error && <div role="alert" className="form-error">Security Agent API unavailable</div>}{tab === "agents" ? agentsView : tab === "runs" ? runsView : tab === "approvals" ? approvalsView : canWrite ? <Builder templates={templates} actions={actions} agents={agents} api={api} environmentID={environmentID} initialEvidenceID={initialEvidenceID} onCreated={(agent) => setAgents((values) => [...values, agent])} /> : agentsView}{selectedAgent && <AgentDetail agent={selectedAgent} runs={runs} actions={actions} api={api} environmentID={environmentID} canRun={canRun} onRun={(run) => setRuns((values) => [run, ...values])} onClose={() => setSelectedAgent(null)} />}{selectedRun && <RunDetail detail={selectedRun} api={api} canRun={canRun} onChange={(detail) => { setSelectedRun(detail); setRuns((values) => values.map((run) => run.id === detail.run.id ? detail.run : run)); }} onClose={() => setSelectedRun(null)} />}{selectedApproval && <ApprovalDetail approval={selectedApproval} api={api} canApprove={canApprove} onClose={() => setSelectedApproval(null)} />}</div>;
}

export function ProductionSecurityAgentsView({ initialTab = "agents", environmentID }: { initialTab?: SecurityAgentTab; environmentID: string }) {
  const { client } = useAPI();
  ...
  const query = useAPIQuery("workflow:security-agents", load);
  ...
  if (query.status === "error") return <p role="alert">Security Agent workflows are unavailable. <Button onClick={() => void query.retry()}>Retry</Button></p>;
  if (!query.data) return null;
  return <SecurityAgentsView key={environmentID} api={api} initialSnapshot={query.data} autoLoad={false} initialTab={initialTab} environmentID={environmentID} initialEvidenceID="" canWrite={session.hasCapability("security-agents.write")} canRun={session.hasCapability("security-agents.run")} canApprove={session.hasCapability("security-agents.approve")} />;
}
