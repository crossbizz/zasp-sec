...
function Builder({ templates, api, environmentID, mutation, onCreated }: { templates: readonly SecurityAgentTemplate[]; api: SecurityAgentsAPI; environmentID: string; mutation: SecurityAgentCreateMutation; onCreated(value: SecurityAgentDefinition): void }) {
  ...
  return <Card title="Create a definition from a locally supported template"><div className="form-stack">
    <Select label="Definition template" value={templateID} disabled={mutation.hasAmbiguousAttempt} onChange={(event) => { if (!mutation.hasAmbiguousAttempt) setTemplateID(event.target.value); }}>{templates.map((value) => <option key={value.id} value={value.id}>{value.name}</option>)}</Select>
    ...
    <div className="form-grid"><Field label="Step limit" type="number" min={1} max={maximums.steps} value={steps} disabled={mutation.hasAmbiguousAttempt} onChange={(event) => setSteps(bounded(event.target.value, maximums.steps))} /><Field label="Runtime seconds" type="number" min={1} max={maximums.runtime} value={runtime} disabled={mutation.hasAmbiguousAttempt} onChange={(event) => setRuntime(bounded(event.target.value, maximums.runtime))} /><Field label="Temporary-policy seconds" type="number" min={1} max={maximums.temporaryPolicy} value={temporaryPolicy} disabled={mutation.hasAmbiguousAttempt} onChange={(event) => setTemporaryPolicy(bounded(event.target.value, maximums.temporaryPolicy))} /><Field label="AI token budget" type="number" min={1} max={maximums.aiTokens} value={aiTokens} disabled={mutation.hasAmbiguousAttempt} onChange={(event) => setAITokens(bounded(event.target.value, maximums.aiTokens))} /><Field label="Concurrency" type="number" min={1} max={maximums.concurrency} value={concurrency} disabled={mutation.hasAmbiguousAttempt} onChange={(event) => setConcurrency(bounded(event.target.value, maximums.concurrency))} /></div>
...
export function SecurityAgentsView({ api = defaultSecurityAgentsAPI, environmentID = "production", canWrite = true, initialSnapshot, autoLoad = true }: { api?: SecurityAgentsAPI; environmentID?: string; canWrite?: boolean; initialSnapshot?: SecurityAgentSnapshot; autoLoad?: boolean }) {
  ...
  const mutationLocked = createMutation.hasAmbiguousAttempt || detailMutation.hasAmbiguousAttempt;
