  ...
  return <section id="scope-onboarding" className="page identity-access">
    ...
    <div className="identity-grid">
      ...
      <Card title={<h2>Environment onboarding</h2>}>
        {workspaceId && environments.length === 0 ? <EmptyState title="No authorized environments" /> : workspaceId && <Select label="Authorized environment" value={environmentId} onChange={(event) => { const id = event.target.value; setEnvironmentId(id); setEnvironmentName(environments.find((item) => item.id === id)?.name ?? ""); void changeScope({ workspaceId, environmentId: id }); }}>{environments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select>}
