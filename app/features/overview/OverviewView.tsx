...
export function OverviewView({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  return <div className="page">
    ...
    <MetricGrid metrics={[
      ...
      { label: "Protected interactions", value: "99.84%", note: `${blocked} blocked in this sample`, tone: "success", onClick: () => onNavigate("/guardrails/dashboard") },
    ...
    <div className="overview-grid">
      ...
      <Card title="Recommended actions" className="recommendations"><div className="action-list">{[
        ...
        { icon: <ShieldCheck />, title: "Enforce the secure code guardrail", note: "7 components remain in review mode", path: "/guardrails/policies" },
      ...
      <Card title="Recent changes"><div className="change-feed">{state.changes.slice(0, 4).map((change) => <button key={change.id} onClick={() => onNavigate("/discovery/recent-changes")}><Clock3 size={15} /><span><strong>{change.description}</strong><small>{change.time}</small></span><Badge tone={change.risk}>{change.risk}</Badge></button>)}</div></Card>
    </div>
  </div>;
}
