...
function RunOutcomeGroups({ runs, definitions, onOpen }: { runs: readonly TestRun[]; definitions: readonly TestDefinition[]; onOpen(id: string): void }) {
  if (runs.length === 0) return <p>No Red Team runs in this scope.</p>;
  return <>{groupRedTeamRuns(runs).map(group => <section key={group.id} aria-label={group.label}>
    <h3>{group.label} ({group.runs.length})</h3><p>{group.description}</p>
    <div className="table-scroll"><table className="data-table"><thead><tr><th>Run</th><th>Test</th><th>Status</th><th>Verdict</th><th>Attempt</th><th>Queued</th></tr></thead><tbody>{group.runs.map(run => <tr key={run.id}>
      <td><button className="row-title" aria-label={`Open run ${run.id}`} onClick={() => onOpen(run.id)}>{run.id}</button></td>
      <td>{definitions.find(item => item.id === run.definition_id)?.name ?? run.definition_id}</td>
      <td><Badge tone={runTone(run)}>{run.cancel_requested && !["cancelled", "complete", "failed"].includes(run.status) ? "cancelling" : run.status}</Badge></td>
      <td>{run.verdict ?? "Not established"}</td><td>{run.attempt}</td><td>{run.queued_at}</td>
    </tr>)}</tbody></table></div>
  </section>)}</>;
}
