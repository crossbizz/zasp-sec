...
variable "database_principals" {
  ...
  validation {
    condition = length(distinct(values(var.database_principals))) == 20 && alltrue([
      for principal in values(var.database_principals) : can(regex("^[a-z][a-z0-9_]{2,62}$", principal))
    ])
    error_message = "database_principals must contain twenty distinct bounded PostgreSQL login names."
  }
}
