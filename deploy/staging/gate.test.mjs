...
const deployment = {
  ...
  jobIdentities: [
    { name: "agentsec-schema-v48", serviceAccount: "agentsec-migration", roleArn: "arn:aws:iam::123456789012:role/zasp-production-migration" },
...
test("staging deployment binds thirty-two deployments and every init/runner/canary identity to one account", () => {
  ...
  assert.throws(() => buildStagingDeployment({ ...deployment, jobIdentities: deployment.jobIdentities.map((identity) => identity.name === "agentsec-schema-v48" ? { ...identity, roleArn: "arn:aws:iam::210987654321:role/zasp-production-migration" } : identity) }), /rejected/);
});
