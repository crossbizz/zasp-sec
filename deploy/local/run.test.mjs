...
const exactProductKindConfig = Object.freeze({
  apiVersion: "kind.x-k8s.io/v1alpha4",
  kind: "Cluster",
  networking: { apiServerAddress: "127.0.0.1" },
  nodes: [{ role: "control-plane" }],
});
const exactGraphKindConfig = Object.freeze({
  apiVersion: "kind.x-k8s.io/v1alpha4",
  kind: "Cluster",
  networking: { apiServerAddress: "127.0.0.1" },
...
test("concrete lifecycle admits only an exact-owned empty workspace", async () => {
  ...
  assert.equal(files.get(`${root}/kind.json`), `${JSON.stringify(exactProductKindConfig)}\n`);
...
function createConcreteFixture(command, profile = undefined) {
  const proof = profile?.proof ?? "m1-30a";
  const root = `/safe/tmp/zasp-${proof}-0123456789abcdef-runtime-owned`;
  ...
  return {
    ...
    system: new LocalProductSystem(input, dependencies, profile),
  };
}
...
test("the graph profile alone requires exact kubelet PID authority after kind creation", async () => {
  const cases = [
    ...
    ["drifted", kubeletPidProjection.replace("512", "511"), true],
