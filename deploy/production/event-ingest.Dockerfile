...
COPY services/event-ingest ./event-ingest
WORKDIR /src/event-ingest
RUN go mod download && test -n "$VERSION" && \
    CGO_ENABLED=0 GOOS=linux go build -trimpath -ldflags="-s -w -X main.buildVersion=$VERSION" -o /out/event-ingest . && \
