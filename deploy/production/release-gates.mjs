...
export async function verifyReleaseSources() {
  ...
  const requiredDocs = ["production-deployment.md", "backup-restore-rollback.md", "recovery-runbook.md", "observability-and-canaries.md", "authentication-and-support.md", "supported-workflows.md"];
  const docs = await Promise.all(requiredDocs.map((name) => source(`docs/operations/${name}`)));
  for (const phrase of ["schema v27", "Do not run `agentsec-migrate down`", "correlation ID", "must not ask", "not supported production workflows", "recovery hold", "cleanup_failed"]) {
  ...
  const sensitiveSources = await Promise.all([
    ...
    source("deploy/staging/product/templates/nango.yaml"), source("deploy/staging/product/templates/otel-collector.yaml"), source("deploy/production/redteam-worker.Dockerfile"), source("deploy/staging/product/templates/red-team.yaml"),
    source("deploy/staging/product/templates/attack-lab.yaml"), source("deploy/production/attack-lab-runner.Dockerfile"),
  ]);
  ...
  for (const contract of ["system:serviceaccount:agentsec:agentsec-api", "system:serviceaccount:agentsec:zasp-discovery-worker", "system:serviceaccount:agentsec:zasp-discovery-scheduler", "system:serviceaccount:agentsec:zasp-security-agent", "system:serviceaccount:agentsec:zasp-security-agent-action", "system:serviceaccount:agentsec:zasp-policy-deployment", "system:serviceaccount:agentsec:zasp-outbox-publisher", "system:serviceaccount:agentsec:zasp-recovery-backup-outbox", "system:serviceaccount:agentsec:zasp-recovery-restore-outbox", "system:serviceaccount:agentsec:zasp-recovery-backup", "system:serviceaccount:agentsec:zasp-recovery-restore", "system:serviceaccount:agentsec:zasp-red-team-outbox", "system:serviceaccount:agentsec:zasp-red-team-worker", "system:serviceaccount:agentsec:zasp-red-team-adapter", "system:serviceaccount:agentsec:zasp-attack-lab-outbox", "system:serviceaccount:agentsec:zasp-attack-lab-controller", "system:serviceaccount:agentsec:zasp-attack-lab-proxy", "system:serviceaccount:agentsec:zasp-projection-risk", "system:serviceaccount:agentsec:zasp-projection-graph", "system:serviceaccount:agentsec:zasp-projection-search", "system:serviceaccount:agentsec:agentsec-projection-graph-init", "system:serviceaccount:agentsec:agentsec-projection-search-init", "system:serviceaccount:agentsec:agentsec-migration", "system:serviceaccount:agentsec:agentsec-canary-secret-sync", "system:serviceaccount:agentsec:zasp-runtime-ingest", "system:serviceaccount:agentsec:zasp-gateway-control", "system:serviceaccount:agentsec:zasp-runtime-outbox", "system:serviceaccount:agentsec:zasp-runtime-coordinator", "system:serviceaccount:agentsec:zasp-runtime-archive", "system:serviceaccount:agentsec:zasp-runtime-index", "system:serviceaccount:agentsec:zasp-runtime-correlation", "system:serviceaccount:agentsec:zasp-runtime-projection", "system:serviceaccount:agentsec:zasp-runtime-complete", "secretsmanager:GetSecretValue", "secretsmanager:DescribeSecret", "sqs:SendMessage", "sqs:ReceiveMessage", "sqs:ChangeMessageVisibility", "s3:PutObject", "recovery-backup-jobs", "recovery-restore-jobs", "discovery-jobs", "red-team-tests", "attack-lab-jobs", "runtime-events", "red_team_evidence", "attack_lab_evidence", "zasp-inventory-v1/_mapping", "zasp-runtime-events-v1", "_zasp_schema_v1", "neo4j/auth/runtime", "neo4j/auth/schema", "canary-read-token", "token-reveal-key", "stytch-secret", "stytch-webhook-secret", "gateway-policy-signing-private-key", "attack-lab-egress-signing-key", "attack-lab-proxy-tls-certificate", "attack-lab-proxy-tls-private-key", "zasp-recovery/neon/project-api-key", "postgres-api-dsn", "postgres-security-agent-worker-dsn", "postgres-security-agent-action-worker-dsn", "postgres-policy-deployment-worker-dsn", "postgres-worker-dsn", "postgres-outbox-worker-dsn", "postgres-recovery-outbox-worker-dsn", "postgres-recovery-worker-dsn", "postgres-red-team-outbox-dsn", "postgres-red-team-worker-dsn", "postgres-red-team-adapter-dsn", "postgres-attack-lab-controller-dsn", "postgres-attack-lab-outbox-dsn", "postgres-attack-lab-proxy-dsn", "postgres-runtime-ingest-dsn", "postgres-runtime-coordinator-dsn", "postgres-runtime-archive-dsn", "postgres-runtime-index-dsn", "postgres-runtime-correlation-dsn", "postgres-runtime-projection-dsn", "postgres-gateway-control-dsn", "postgres-scheduler-dsn", "postgres-projection-risk-dsn", "postgres-projection-graph-dsn", "postgres-projection-search-dsn", "postgres-migration-dsn"]) {
    if (!terraform.includes(contract)) throw new Error("secret identity gate rejected");
  }
  const [workloads, services, redTeamWorkloads, attackLabWorkloads, runtimeWorkloads, projectionInit, resilience, nango, collector] = await Promise.all([
    source("deploy/staging/product/templates/workloads.yaml"), source("deploy/staging/product/templates/services.yaml"), source("deploy/staging/product/templates/red-team.yaml"), source("deploy/staging/product/templates/attack-lab.yaml"), source("deploy/staging/product/templates/runtime.yaml"), source("deploy/staging/product/templates/projection-init.yaml"), source("deploy/staging/product/templates/resilience.yaml"), source("deploy/staging/product/templates/nango.yaml"), source("deploy/staging/product/templates/otel-collector.yaml"),
  ]);
  for (const contract of ["ZASP_WORKER_MODE", "security-agent", "security-agent-action", "policy-deployment", "zasp_security_agent_worker", "zasp_security_agent_action_worker", "zasp_policy_deployment_worker", "ZASP_GATEWAY_SIGNING_KEY_ID", "ZASP_GATEWAY_SIGNING_PRIVATE_KEY_FILE", "projection-risk", "projection-graph", "projection-search", "recovery-outbox", "ZASP_RECOVERY_OPERATION_KIND", "ZASP_RECOVERY_SIGNING_KMS_KEY_ARN", "ZASP_RECOVERY_KUBERNETES_ENDPOINT", "ZASP_NEO4J_EXPECTED_PRINCIPAL", "ZASP_NEO4J_EXPECTED_ROLE"]) if (!workloads.includes(contract)) throw new Error("worker deployment gate rejected");
  ...
  for (const contract of ["memory_limiter", "transform/redact_content", "set(log.body", "set(span.name", "redaction", "batch", "nop", "otlphttp/remote", "sending_queue", "queue_size: 256", "max_elapsed_time: 60s", "otel-collector-private", "ServiceMonitor", "PrometheusRule", "ZaspOTelCollectorQueueLoss", "port: 8888"]) if (!collector.includes(contract)) throw new Error("Collector deployment gate rejected");
  for (const contract of [
    "ZaspDiscoverySchedulerUnavailable", "ZaspDiscoveryWorkerUnavailable", "ZaspSecurityAgentUnavailable", "ZaspSecurityAgentActionUnavailable", "ZaspOutboxPublisherUnavailable",
    ...
    "ZaspProjectionBacklogAge", "ZaspWorkerLeaseLoss", "ZaspWorkerExhaustion", "ZaspTask4WorkerCapacityExhausted", "ZaspRedTeamOutboxUnavailable", "ZaspRedTeamWorkerUnavailable", "ZaspRedTeamAdapterUnavailable", "ZaspRedTeamDependencyNotReady", "ZaspAttackLabOutboxUnavailable", "ZaspAttackLabControllerUnavailable", "ZaspAttackLabProxyUnavailable", "ZaspAttackLabDependencyNotReady",
    "ZaspRecoveryUnavailable", "ZaspRecoveryDriverNotReady", "ZaspRecoveryQueueAge", "ZaspRecoveryHoldTooOld", "ZaspRecoveryExhaustion", "ZaspRecoveryLeaseLoss", "ZaspRecoveryCleanupFailure",
    "agentsec_ready", "zasp_worker_driver_ready", "zasp_worker_projection_backlog_age_seconds", "zasp_worker_lease_loss_total", "zasp_worker_exhaustion_total", "zasp_recovery_driver_ready", "zasp_recovery_queue_age_seconds", "zasp_recovery_hold_age_seconds", "zasp_recovery_exhaustion_total", "zasp_recovery_lease_loss_total", "zasp_recovery_cleanup_failure_total",
  ...
  const definitions = await Promise.all(["web", "api", "worker", "attack-lab-runner", "redteam-worker", "event-ingest", "gateway-control", "runtime-gateway", "sensor-agent"].map((name) => source(`deploy/production/${name}.Dockerfile`)));
...
async function goSourceSBOM() {
  ...
  const modulesToList = [
    { cwd: path.join(root, "services/platform"), packages: ["./agentsec-api", "./agentsec-migrate", "./agentsec-worker", "./red-team-adapter", "./cmd/zasp-healthcheck"] },
  ...
  for (const entry of modulesToList) {
    const { stdout } = await exec("go", ["list", "-deps", "-f", template, ...entry.packages], { cwd: entry.cwd, encoding: "utf8", maxBuffer: 16 * 1024 * 1024 });
