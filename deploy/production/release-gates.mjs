...
export async function verifyReleaseSources() {
  ...
  if (builds.length !== 8 || builds.some((build) => !build.readOnlyCompatible || build.containsSecret)) throw new Error("container gate rejected");
  ...
  for (const phrase of ["schema v25", "Do not run `agentsec-migrate down`", "correlation ID", "must not ask", "not supported production workflows"]) {
  ...
  for (const contract of [
    ...
    "ZaspProjectionBacklogAge", "ZaspWorkerLeaseLoss", "ZaspWorkerExhaustion", "ZaspTask4WorkerCapacityExhausted", "ZaspRedTeamOutboxUnavailable", "ZaspRedTeamWorkerUnavailable", "ZaspRedTeamAdapterUnavailable", "ZaspRedTeamDependencyNotReady",
  ...
  const definitions = await Promise.all(["web", "api", "worker", "redteam-worker", "event-ingest", "gateway-control", "runtime-gateway", "sensor-agent"].map((name) => source(`deploy/production/${name}.Dockerfile`)));
