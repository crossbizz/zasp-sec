...
export async function verifyReleaseSources() {
  ...
  for (const contract of ["ZASP_WORKER_MODE", "security-agent", "zasp_security_agent_worker", "projection-risk", "projection-graph", "projection-search", "ZASP_NEO4J_EXPECTED_PRINCIPAL", "ZASP_NEO4J_EXPECTED_ROLE"]) if (!workloads.includes(contract)) throw new Error("worker deployment gate rejected");
  ...
  for (const contract of ["agentsec-discovery-worker", "agentsec-security-agent", "security-agent-database", "agentsec-projection-risk", "agentsec-projection-graph", "agentsec-projection-search", "HorizontalPodAutoscaler", "PodDisruptionBudget", "default-deny"]) if (!resilience.includes(contract)) throw new Error("worker resilience gate rejected");
  ...
  for (const contract of [
    "ZaspDiscoverySchedulerUnavailable", "ZaspDiscoveryWorkerUnavailable", "ZaspSecurityAgentUnavailable", "ZaspOutboxPublisherUnavailable",
