...
test("rendered release rejects an unreviewed job identity", async () => {
  ...
  assert.deepEqual(names, ["agentsec-projection-graph-init-v1", "agentsec-projection-search-init-v1", "agentsec-schema-v38", "nango-migrate", "zasp-canary-secret-sync"]);
...
test("release renders one TLS origin, split ports, private internals, and migration/schema gates", async () => {
  ...
  assert.equal(one(resources, "Job", "agentsec-schema-v38").metadata.annotations["helm.sh/hook"], "pre-install,pre-upgrade");
  assert.match(one(resources, "Job", "agentsec-schema-v38").spec.template.spec.containers[0].args[0], /exec \/app\/agentsec-migrate up/);
  const migration = one(resources, "Job", "agentsec-schema-v38");
  ...
  assert.deepEqual(Object.fromEntries(migration.spec.template.spec.containers[0].env.map(({ name, value }) => [name, value])), {
    ...
    ZASP_RECOVERY_OUTBOX_DB_PRINCIPAL: "zasp_recovery_outbox_runtime",
    ZASP_RECOVERY_WORKER_DB_PRINCIPAL: "zasp_recovery_runtime",
  ...
  for (const [kind, name, weight] of [["ServiceAccount", "agentsec-migration", "-30"], ["SecretProviderClass", "zasp-production-migration-secrets", "-20"], ["Job", "agentsec-schema-v38", "-10"]]) {
  ...
  assert.equal(one(resources, "Deployment", "agentsec-api").spec.template.metadata.annotations["zasp.io/schema-version"], "38");
  assert.equal(one(resources, "Deployment", "agentsec-api").spec.template.spec.containers[0].env.find(({ name }) => name === "ZASP_EXPECTED_SCHEMA_VERSION").value, "38");
...
test("release runs the security agent with one exact database authority", async () => {
  const resources = await renderRelease(release);
  const deployment = one(resources, "Deployment", "agentsec-security-agent");
  const pod = deployment.spec.template.spec;
  const container = pod.containers[0];

  assert.equal(deployment.spec.replicas, 2);
  assert.equal(pod.serviceAccountName, "zasp-security-agent");
  assert.equal(one(resources, "ServiceAccount", "zasp-security-agent").metadata.annotations["eks.amazonaws.com/role-arn"], "arn:aws:iam::123456789012:role/zasp-production-security-agent-worker");
  assert.deepEqual(envOf(deployment), {
    ZASP_WORKER_MODE: "security-agent",
    ZASP_DATABASE_AUTHORITY: "zasp_security_agent_worker",
    ZASP_WORKER_ID: "fieldRef:metadata.name",
    ZASP_POLL_INTERVAL: "1s",
    ZASP_LEASE_DURATION: "60s",
    ZASP_BATCH_SIZE: "8",
    ZASP_SHUTDOWN_TIMEOUT: "20s",
    ZASP_SECURITY_AGENT_PLANNER_ENDPOINT: "https://openrouter.ai/api/v1/chat/completions",
    ZASP_SECURITY_AGENT_PLANNER_MODEL: "openai/gpt-5-mini",
    ZASP_SECURITY_AGENT_PLANNER_TOKEN_FILE: "/var/run/secrets/zasp-security-agent/openrouter-api-token",
    ZASP_SECURITY_AGENT_PLANNER_TIMEOUT: "10s",
    ZASP_SECURITY_AGENT_PLANNER_MAX_TOKENS: "512",
    ZASP_SECURITY_AGENT_PLANNER_POLICY_VERSION: "security-agent-planner-v1",
  });
  assert.match(container.args[0], /export ZASP_POSTGRES_DSN="\$\(cat \/var\/run\/secrets\/zasp-security-agent\/postgres-dsn\)"/);
  assert.deepEqual(container.volumeMounts, [{ name: "security-agent-secrets", mountPath: "/var/run/secrets/zasp-security-agent", readOnly: true }]);
  const providerClass = one(resources, "SecretProviderClass", "zasp-production-security-agent-secrets");
  assert.equal(providerClass.spec.secretObjects[0].data.length, 1);
  assert.doesNotMatch(JSON.stringify(providerClass.spec.secretObjects), /openrouter-api-token|ZASP_SECURITY_AGENT_PLANNER_TOKEN/);
  assert.match(JSON.stringify(providerClass.spec.parameters), /zasp\/production\/openrouter-security-agent-api-key/);
  assert.match(providerClass.spec.parameters.objects, /objectAlias: openrouter-api-token\n\s+filePermission: "0444"/);
  assert.equal(one(resources, "Service", "agentsec-security-agent").spec.ports[0].port, 8081);
  assert.equal(one(resources, "PodDisruptionBudget", "agentsec-security-agent").spec.minAvailable, 1);
  assert.equal(one(resources, "HorizontalPodAutoscaler", "agentsec-security-agent").spec.maxReplicas, 6);
  assert.equal(one(resources, "ServiceMonitor", "agentsec-security-agent").spec.endpoints[0].path, "/metrics");
  const databasePolicy = one(resources, "NetworkPolicy", "security-agent-database");
  assert.deepEqual(databasePolicy.spec.egress, [{
    to: [{ ipBlock: { cidr: "10.30.0.0/24" } }],
    ports: [{ protocol: "TCP", port: 5432 }],
  }]);
  assert.deepEqual(one(resources, "NetworkPolicy", "security-agent-planner").spec.egress, release.securityAgentPlanner.egressCIDRs.map((cidr) => ({ to: [{ ipBlock: { cidr } }], ports: [{ protocol: "TCP", port: 443 }] })));
  assert.match(JSON.stringify(one(resources, "PrometheusRule", "zasp-production-slos")), /ZaspSecurityAgentUnavailable.*agentsec-security-agent/);
  assert.doesNotMatch(JSON.stringify(deployment), /ZASP_(?:AWS|DISCOVERY|OUTBOX|OPENSEARCH|NEO4J|CONNECTOR|RUNTIME)_/);

  const terraform = await readFile(new URL("../staging/main.tf", import.meta.url), "utf8");
  const policyStart = terraform.indexOf('resource "aws_iam_role_policy" "security_agent_worker"');
  const policy = terraform.slice(policyStart, terraform.indexOf("\nresource ", policyStart + 1));
  assert.match(terraform, /system:serviceaccount:agentsec:zasp-security-agent/);
  assert.match(policy, /postgres-security-agent-worker-dsn/);
  assert.match(policy, /openrouter-security-agent-api-key/);
  assert.match(policy, /secretsmanager:DescribeSecret.*secretsmanager:GetSecretValue/s);
  assert.match(policy, /kms:EncryptionContext:SecretARN/);
  assert.doesNotMatch(policy, /Resource\s*=\s*"\*"|s3:|sqs:|es:|sts:/);
  assert.match(await readFile(new URL("../staging/outputs.tf", import.meta.url), "utf8"), /output "security_agent_worker_role_arn"/);
});

test("release runs temporary policy actions with a separate signing identity", async () => {
  const resources = await renderRelease(release);
  const deployment = one(resources, "Deployment", "agentsec-security-agent-action");
  const pod = deployment.spec.template.spec;
  const container = pod.containers[0];

  assert.equal(deployment.spec.replicas, 2);
  assert.equal(pod.serviceAccountName, "zasp-security-agent-action");
  assert.equal(one(resources, "ServiceAccount", "zasp-security-agent-action").metadata.annotations["eks.amazonaws.com/role-arn"], "arn:aws:iam::123456789012:role/zasp-production-security-agent-action-worker");
  assert.deepEqual(envOf(deployment), {
    ZASP_WORKER_MODE: "security-agent-action",
    ZASP_DATABASE_AUTHORITY: "zasp_security_agent_action_worker",
    ZASP_WORKER_ID: "fieldRef:metadata.name",
    ZASP_POLL_INTERVAL: "1s",
    ZASP_LEASE_DURATION: "60s",
    ZASP_BATCH_SIZE: "8",
    ZASP_SHUTDOWN_TIMEOUT: "20s",
    ZASP_GATEWAY_SIGNING_KEY_ID: "gateway-key-01",
    ZASP_GATEWAY_SIGNING_PRIVATE_KEY_FILE: "/var/run/secrets/zasp-security-agent-action/gateway-signing-private-key",
  });
  assert.match(container.args[0], /export ZASP_POSTGRES_DSN="\$\(cat \/var\/run\/secrets\/zasp-security-agent-action\/postgres-dsn\)"/);
  assert.equal(one(resources, "SecretProviderClass", "zasp-production-security-agent-action-secrets").spec.secretObjects[0].data.length, 2);
  assert.equal(one(resources, "Service", "agentsec-security-agent-action").spec.ports[0].port, 8081);
  assert.equal(one(resources, "PodDisruptionBudget", "agentsec-security-agent-action").spec.minAvailable, 1);
  assert.equal(one(resources, "HorizontalPodAutoscaler", "agentsec-security-agent-action").spec.maxReplicas, 6);
  assert.equal(one(resources, "ServiceMonitor", "agentsec-security-agent-action").spec.endpoints[0].path, "/metrics");
  assert.match(JSON.stringify(one(resources, "PrometheusRule", "zasp-production-slos")), /ZaspSecurityAgentActionUnavailable.*agentsec-security-agent-action/);

  const terraform = await readFile(new URL("../staging/main.tf", import.meta.url), "utf8");
  const policyStart = terraform.indexOf('resource "aws_iam_role_policy" "security_agent_action_worker"');
  const policy = terraform.slice(policyStart, terraform.indexOf("\nresource ", policyStart + 1));
  assert.match(policy, /postgres-security-agent-action-worker-dsn/);
  assert.match(policy, /gateway-policy-signing-private-key/);
  assert.doesNotMatch(policy, /Resource\s*=\s*"\*"|s3:|sqs:|es:|sts:/);
});

test("release deploys the central policy writer with one exact signing identity", async () => {
  const resources = await renderRelease(release);
  const deployment = one(resources, "Deployment", "agentsec-policy-deployment");
  const pod = deployment.spec.template.spec;
  const container = pod.containers[0];

  assert.equal(deployment.spec.replicas, 2);
  assert.equal(pod.serviceAccountName, "zasp-policy-deployment");
  assert.equal(one(resources, "ServiceAccount", "zasp-policy-deployment").metadata.annotations["eks.amazonaws.com/role-arn"], "arn:aws:iam::123456789012:role/zasp-production-policy-deployment-worker");
  assert.deepEqual(envOf(deployment), {
    ZASP_WORKER_MODE: "policy-deployment",
    ZASP_DATABASE_AUTHORITY: "zasp_policy_deployment_worker",
    ZASP_WORKER_ID: "fieldRef:metadata.name",
    ZASP_POLL_INTERVAL: "1s",
    ZASP_LEASE_DURATION: "60s",
    ZASP_BATCH_SIZE: "8",
    ZASP_SHUTDOWN_TIMEOUT: "20s",
    ZASP_GATEWAY_SIGNING_KEY_ID: "gateway-key-01",
    ZASP_GATEWAY_SIGNING_PRIVATE_KEY_FILE: "/var/run/secrets/zasp-policy-deployment/gateway-signing-private-key",
  });
  assert.match(container.args[0], /export ZASP_POSTGRES_DSN="\$\(cat \/var\/run\/secrets\/zasp-policy-deployment\/postgres-dsn\)"/);
  assert.equal(one(resources, "SecretProviderClass", "zasp-production-policy-deployment-secrets").spec.secretObjects[0].data.length, 2);
  assert.equal(one(resources, "Service", "agentsec-policy-deployment").spec.ports[0].port, 8081);
  assert.equal(one(resources, "PodDisruptionBudget", "agentsec-policy-deployment").spec.minAvailable, 1);
  assert.equal(one(resources, "HorizontalPodAutoscaler", "agentsec-policy-deployment").spec.maxReplicas, 6);
  assert.equal(one(resources, "ServiceMonitor", "agentsec-policy-deployment").spec.endpoints[0].path, "/metrics");
  assert.match(JSON.stringify(one(resources, "PrometheusRule", "zasp-production-slos")), /ZaspPolicyDeploymentUnavailable.*agentsec-policy-deployment/);

  const network = one(resources, "NetworkPolicy", "policy-deployment-database");
  assert.deepEqual(network.spec.podSelector.matchLabels, { "app.kubernetes.io/name": "agentsec-policy-deployment" });
  assert.deepEqual(network.spec.egress[0].ports, [{ protocol: "TCP", port: 5432 }]);

  const terraform = await readFile(new URL("../staging/main.tf", import.meta.url), "utf8");
  const policyStart = terraform.indexOf('resource "aws_iam_role_policy" "policy_deployment_worker"');
  const policy = terraform.slice(policyStart, terraform.indexOf("\nresource ", policyStart + 1));
  assert.match(policy, /postgres-policy-deployment-worker-dsn/);
  assert.match(policy, /gateway-policy-signing-private-key/);
  assert.doesNotMatch(policy, /Resource\s*=\s*"\*"|s3:|sqs:|es:|sts:/);
});
...
test("release applies non-root rollout, zone and host spread, drain, PDB, and default-deny policies", async () => {
  const resources = await renderRelease(release);
  const workloadNames = ["agentsec-api", "agentsec-attack-lab-controller", "agentsec-attack-lab-outbox", "agentsec-attack-lab-proxy", "agentsec-discovery-scheduler", "agentsec-discovery-worker", "agentsec-event-ingest", "agentsec-gateway-control", "agentsec-outbox-publisher", "agentsec-policy-deployment", "agentsec-projection-graph", "agentsec-projection-risk", "agentsec-projection-search", "agentsec-recovery-backup", "agentsec-recovery-backup-outbox", "agentsec-recovery-restore", "agentsec-recovery-restore-outbox", "agentsec-red-team-adapter", "agentsec-red-team-outbox", "agentsec-red-team-worker", "agentsec-runtime-archive", "agentsec-runtime-complete", "agentsec-runtime-coordinator", "agentsec-runtime-correlation", "agentsec-runtime-index", "agentsec-runtime-outbox", "agentsec-runtime-projection", "agentsec-security-agent", "agentsec-security-agent-action", "nango", "otel-collector", "web"];
  ...
  for (const name of workloadNames) {
    ...
    if (name !== "web" && name !== "nango" && name !== "otel-collector") {
      ...
      assert.equal(shutdown.value, name === "agentsec-security-agent" || name === "agentsec-security-agent-action" || name === "agentsec-policy-deployment" || name.startsWith("agentsec-recovery-") ? "20s" : "15s");
...
test("release ships the complete v15 runtime data plane behind exact workload authorities", async () => {
  ...
  assert.deepEqual(envOf(one(resources, "Deployment", "agentsec-event-ingest")), {
    ...
    ZASP_EVENT_INGEST_RECONCILER_ID: "fieldRef:metadata.name", ZASP_EVENT_INGEST_RECONCILIATION_INTERVAL: "1s",
  });
});
...
test("release renders read-only synthetic and exact SLO budgets without credential values", async () => {
  ...
  for (const [alert, workload] of [
    ...
    ["ZaspSecurityAgentUnavailable", "agentsec-security-agent"],
    ["ZaspSecurityAgentActionUnavailable", "agentsec-security-agent-action"],
    ["ZaspPolicyDeploymentUnavailable", "agentsec-policy-deployment"],
  ...
  for (const [alert, metric] of [
    ["ZaspTask4WorkerDependencyNotReady", 'agentsec_ready{namespace="agentsec",service=~"agentsec-(discovery-(scheduler|worker)|security-agent(-action)?|policy-deployment|outbox-publisher|projection-(risk|graph|search))"} == 0'],
...
test("terraform isolates red team queue evidence and target credentials behind three identities", async () => {
  ...
  assert.match(terraform, /"red-team-tests"\s*=\s*\{ visibility = 900, max_receive = 5, schema = "agentsec\.red-team-tests\.v1" \}/);
...
test("terraform isolates Attack Lab queue evidence proxy and runner pod networks", async () => {
  ...
  assert.match(terraform, /"attack-lab-jobs"\s*=\s*\{ visibility = 60, max_receive = 5, schema = "agentsec\.attack-lab-jobs\.v1" \}/);
...
test("terraform provisions the exact encrypted v15 runtime plane and isolated identities", async () => {
  ...
  assert.match(terraform, /runtime-events\s*=\s*\{ visibility = 120, max_receive = 5, schema = "agentsec\.runtime-events\.v1" \}/);
...
test("production recovery renders isolated inert workers and least-privilege cloud authority", async () => {
  const resources = await renderRelease(release);
  const expected = new Map([
    ["agentsec-recovery-backup-outbox", ["zasp-recovery-backup-outbox", "recovery-outbox"]],
    ["agentsec-recovery-restore-outbox", ["zasp-recovery-restore-outbox", "recovery-outbox"]],
    ["agentsec-recovery-backup", ["zasp-recovery-backup", "recovery"]],
    ["agentsec-recovery-restore", ["zasp-recovery-restore", "recovery"]],
  ]);
  for (const [name, [serviceAccount, mode]] of expected) {
    const deployment = one(resources, "Deployment", name);
    const env = envOf(deployment);
    assert.equal(deployment.spec.replicas, 2, `${name} is promoted only by the explicit recovery canary input`);
    assert.equal(deployment.spec.template.spec.serviceAccountName, serviceAccount);
    assert.equal(deployment.spec.template.spec.automountServiceAccountToken, false);
    assert.equal(env.ZASP_WORKER_MODE, mode);
    assert.equal(env.ZASP_AWS_REGION, release.recovery.awsRegion);
    if (mode === "recovery") {
      assert.equal(env.ZASP_EVIDENCE_BUCKET, release.recovery.evidenceBucket);
      assert.equal(env.ZASP_EVIDENCE_KMS_KEY_ARN, release.recovery.evidenceKMSKeyArn);
      assert.equal(env.ZASP_RECOVERY_QUEUE_URL, name.endsWith("backup") ? release.recovery.backupQueueURL : release.recovery.restoreQueueURL);
    } else {
      assert.equal(env.ZASP_EVIDENCE_BUCKET, undefined);
      assert.equal(env.ZASP_EVIDENCE_KMS_KEY_ARN, undefined);
    }
    assert.equal(resources.some(({ kind, metadata }) => kind === "Ingress" && metadata?.name === name), false);
    one(resources, "PodDisruptionBudget", name);
    one(resources, "HorizontalPodAutoscaler", name);
    one(resources, "Service", name);
    one(resources, "ServiceMonitor", name);
  }
  const restore = envOf(one(resources, "Deployment", "agentsec-recovery-restore"));
  assert.equal(restore.ZASP_RECOVERY_NEON_SECRET_REFERENCE, "ref:neon/project-api-key");
  assert.equal(restore.ZASP_RECOVERY_KUBERNETES_ENDPOINT, "https://kubernetes.default.svc");
  assert.equal(restore.ZASP_RECOVERY_RUNNER_SERVICE_ACCOUNT, "agentsec-recovery-runner");
  assert.equal(resources.some(({ kind, metadata }) => kind === "ServiceAccount" && metadata?.name === "agentsec-recovery-runner"), false);
  const controllerRole = one(resources, "ClusterRole", "agentsec-recovery-controller");
  assert.deepEqual(controllerRole.rules.map(({ resources: names, resourceNames, verbs }) => [names, resourceNames ?? [], verbs]), [
    [["namespaces"], [], ["create", "get", "delete"]],
    [["rolebindings"], [], ["create", "get"]],
    [["clusterroles"], ["agentsec-recovery-namespace-operator"], ["bind"]],
  ]);
  const namespaceRole = one(resources, "ClusterRole", "agentsec-recovery-namespace-operator");
  assert.deepEqual(namespaceRole.rules.map(({ resources: names, verbs }) => [names, verbs]), [
  ...
  assert.equal(one(resources, "ClusterRoleBinding", "agentsec-recovery-controller").subjects[0].name, "zasp-recovery-restore");
  const admission = one(resources, "ValidatingAdmissionPolicy", "agentsec-recovery-namespace-fence");
  assert.equal(admission.spec.failurePolicy, "Fail");
  assert.match(JSON.stringify(admission.spec), /system:serviceaccount:agentsec:zasp-recovery-restore/);
  assert.ok(JSON.stringify(admission.spec).includes("^zasp-recovery-[a-f0-9]{32}$"));
  assert.match(JSON.stringify(admission.spec), /agentsec-recovery-namespace-operator/);
  assert.deepEqual(one(resources, "ValidatingAdmissionPolicyBinding", "agentsec-recovery-namespace-fence").spec.validationActions, ["Deny"]);

  const [terraform, outputs] = await Promise.all([
    readFile(new URL("../staging/main.tf", import.meta.url), "utf8"),
    readFile(new URL("../staging/outputs.tf", import.meta.url), "utf8"),
  ]);
  assert.match(terraform, /"recovery-backup-jobs"\s*=\s*\{ visibility = 30, max_receive = 100, schema = "agentsec\.recovery-backup-jobs\.v1" \}/);
  assert.match(terraform, /"recovery-restore-jobs"\s*=\s*\{ visibility = 30, max_receive = 100, schema = "agentsec\.recovery-restore-jobs\.v1" \}/);
  assert.match(terraform, /maxReceiveCount\s*=\s*each\.value\.max_receive/);
  assert.match(terraform, /resource "aws_kms_key" "recovery_signing"[\s\S]*?key_usage\s*=\s*"SIGN_VERIFY"[\s\S]*?customer_master_key_spec\s*=\s*"ECC_NIST_P256"/);
  assert.match(terraform, /recovery_backup_outbox[\s\S]*?system:serviceaccount:agentsec:zasp-recovery-backup-outbox/);
  assert.match(terraform, /recovery_restore_outbox[\s\S]*?system:serviceaccount:agentsec:zasp-recovery-restore-outbox/);
  assert.match(terraform, /recovery_backup[\s\S]*?system:serviceaccount:agentsec:zasp-recovery-backup/);
  assert.match(terraform, /recovery_restore[\s\S]*?system:serviceaccount:agentsec:zasp-recovery-restore/);
  const policyStart = terraform.indexOf('resource "aws_iam_role_policy" "recovery"');
  const policy = terraform.slice(policyStart, terraform.indexOf("\nresource ", policyStart + 1));
  for (const action of ["sqs:SendMessage", "sqs:GetQueueAttributes", "sqs:ReceiveMessage", "sqs:DeleteMessage", "sqs:ChangeMessageVisibility", "s3:GetObject", "s3:PutObject", "kms:Sign", "kms:Verify", "secretsmanager:GetSecretValue"]) assert.match(policy, new RegExp(action.replace(":", "\\:")));
  assert.doesNotMatch(policy, /"(?:s3|sqs|kms|secretsmanager):\*"/);
  assert.match(outputs, /output "recovery_release_authority"/);
  assert.match(outputs, /backup_queue_url\s*=/);
  assert.match(outputs, /restore_queue_url\s*=/);
  assert.match(outputs, /signing_kms_key_arn\s*=/);

  const monitoring = await readFile(new URL("../staging/product/templates/monitoring.yaml", import.meta.url), "utf8");
  for (const alert of ["ZaspRecoveryUnavailable", "ZaspRecoveryDriverNotReady", "ZaspRecoveryQueueAge", "ZaspRecoveryHoldTooOld", "ZaspRecoveryExhaustion", "ZaspRecoveryLeaseLoss", "ZaspRecoveryCleanupFailure"]) assert.match(monitoring, new RegExp(alert));
  assert.match(monitoring, /count\(count by \(service\) \(agentsec_ready\{namespace="\{\{ \.Release\.Namespace \}\}",service=~"agentsec-recovery-\(backup\|restore\)\(-outbox\)\?"\}\)\) < 4/);
  assert.match(monitoring, /count\(count by \(service\) \(zasp_recovery_driver_ready\{namespace="\{\{ \.Release\.Namespace \}\}",service=~"agentsec-recovery-\(backup\|restore\)"\}\)\) < 2/);
});
