...
COPY services/gateway-control ./gateway-control
WORKDIR /src/gateway-control
RUN go mod download && test -n "$VERSION" && \
    CGO_ENABLED=0 GOOS=linux go build -trimpath -ldflags="-s -w -X main.buildVersion=$VERSION" -o /out/gateway-control . && \
