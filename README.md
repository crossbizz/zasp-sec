...
reports `planned=42 api_available=7 available=91 public=98 internal=0`.
...
The adapter uses namespace-scoped Pod/Lease authority, node-local read-only
telemetry, durable per-node cursors, one elected cluster heartbeat reporter,
and bounded application control-plane/API/node-metrics egress. Tetragon's
required host-network egress remains a node-firewall release gate. The edge
runtime has no AWS or database identity.
...
The current honest result is `UI/API coverage passed: planned=42
api_available=7 available=91 public=98 internal=0.` The gate distinguishes
...
The complete local M5 gate does not claim live Attack Lab/Fargate execution.

M6-01 through M6-31 are Complete. The reviewed policy system includes the
domain and generated APIs, valid deterministic Rego evaluated through the OPA
Go SDK, signed ArtifactStore bundles, bounded restart recovery, runtime-token
...
```text
UI/API coverage passed: planned=42 api_available=7 available=91 public=98 internal=0.
```
