...
export function decodeSecurityAgentApproval(value: unknown): SecurityAgentApproval {
  ...
  printableString(record.expected_effect, 1, 128); if (record.expected_effect !== "Move finding to under review" || record.reversible !== true || record.ttl_seconds !== 0) fail(); productIDArray(record.evidence_summary, 100);
...
export function decodeSecurityAgentRunDetail(value: unknown): SecurityAgentRunDetail {
  ...
  for (let index = 0; index < steps.length; index++) { const step = exactRecord(steps[index], ["id", "index", "action", "authorization", "state", "version"]); productID(step.id); if (step.index !== index || stepIDs.has(step.id as string)) fail(); printableString(step.action, 1, 128); enumValue(step.authorization, ["allow", "approval_required", "deny"]); enumValue(step.state, ["queued", "authorized", "waiting_approval", "executing", "verifying", "succeeded", "failed", "inconclusive", "cancelled"]); positiveInteger(step.version); stepIDs.add(step.id as string); stepValues.set(step.id as string, { action: step.action as string, authorization: step.authorization as string }); }
  for (const item of approvals) { const approval = item as SecurityAgentApproval; if (!stepIDs.has(approval.step_id) || stepValues.get(approval.step_id)?.authorization !== "approval_required") fail(); }
