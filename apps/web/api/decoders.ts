...
export function decodeSecurityAgentApproval(value: unknown): SecurityAgentApproval {
  ...
  printableString(record.expected_effect, 1, 128); const validEffect = record.expected_effect === "Move finding to under review" && record.ttl_seconds === 0 || record.expected_effect === "Apply temporary containment policy" && Number.isInteger(record.ttl_seconds) && (record.ttl_seconds as number) >= 60 && (record.ttl_seconds as number) <= 3600; if (!validEffect || record.reversible !== true) fail(); productIDArray(record.evidence_summary, 100);
...
export function decodeSecurityAgentRunDetail(value: unknown): SecurityAgentRunDetail {
  ...
  for (const item of approvals) { const approval = item as SecurityAgentApproval; const step = stepValues.get(approval.step_id); const validEffect = step?.action === "update_finding_response" && approval.expected_effect === "Move finding to under review" && approval.ttl_seconds === 0 || step?.action === "create_temporary_policy" && approval.expected_effect === "Apply temporary containment policy" && approval.ttl_seconds >= 60 && approval.ttl_seconds <= 3600; if (!step || step.authorization !== "approval_required" || !validEffect) fail(); }
