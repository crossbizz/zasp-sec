...
Stop promotion when error, latency, auth, dependency or canary budgets fail. Preserve correlation and trace IDs. If schema v9 remains compatible, use Helm rollback to the previously attested image digests and verify readiness plus the read-only synthetic before reopening traffic.
...
For stale projections, keep PostgreSQL authoritative, disable the affected read surface, rebuild the projection from durable source data, compare scope-keyed counts, then re-enable. Do not surface stale OpenSearch/Neo4j data as authoritative truth.
