# M1-28d Runtime-Gateway Health Wiring Implementation Plan
...
> tests-only RED before production changes. Do not start M1-28.

**Goal:** Make `runtime-gateway` expose the exact shared health contract and
prove that liveness and readiness are distinct states.

**Architecture:** Keep runtime-gateway standalone. Add one local bounded server
...
- Work only on M1-28d. M1-28 remains Pending.
- Preserve M0-09, M0-18, and M0-19 as the only Blocked source tasks.
- Use the exact shared handler; add no gateway, proxy, MCP server, tool/API
  forwarding, OPA evaluation, authentication, configuration, provider call,
...
## Task 1: Design, plan, and start M1-28d

1. Commit this design and plan separately.
2. Add one exact repository contract for source/dependency/scope/status/docs.
3. Capture stale-status RED, then move only M1-28d In progress at
   `664/1/60/3` overall and `68/31/1/36/0` for M1.
4. Keep M1-28c Complete, M1-28 Pending, and blockers unchanged.
...
2. Require liveness 200/readiness 503 before serving and readiness 200 only
   during serving.
3. Add dependency tests for the exact runtime-gateway shared-health edge and
   all hostile consumer/version/replacement/indirect variants.
4. Run focused tests and record missing symbols/policy as genuine RED.

## Task 3: Implement the bounded local lifecycle

1. Add the exact shared-health requirement/replacement to runtime-gateway.
2. Expand the validator to the exact three-consumer allowlist.
...
1. Add `runtime-gateway:health:test` to root scripts and document the exact
   internal boundary, distinction, command, exclusions, and M1-28 Pending state.
...
1. Capture stale completion RED for `664/0/61/3` overall and
   `68/31/0/37/0` for M1.
2. Move only M1-28d Complete; keep M1-28 Pending and blockers unchanged.
