...
session configuration. The 728 source-plan counts are `697/1/27/3` because
...
| --- | --- | --- |
| M1-01a | August 15, 2026 | Adding the standalone service-local `runtime-gateway` Go command with exact bounded build-version output and no proxy, listener, MCP, tool/API, OPA, configuration, provider, or network behavior. |
