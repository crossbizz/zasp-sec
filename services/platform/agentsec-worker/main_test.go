...
func TestRunPrintsExactBuildVersion(t *testing.T) {
	...
	for _, version := range []string{
		...
		t.Run(version, func(t *testing.T) {
			...
			if got, want := output.String(), "agentsec-worker build "+version+"\n"; got != want {
...
func TestServeProcessExposesExactHealthEndpoints(t *testing.T) {
	...
	assertEndpoint(t, baseURL+"/version", http.StatusOK, "{\"service\":\"agentsec-worker\",\"version\":\"1.2.3-test\"}\n")
	assertContains(t, baseURL+"/metrics", "agentsec_build_info{service=\"agentsec-worker\",version=\"1.2.3-test\"} 1\n")
	cancel()
	select {
	case err := <-result:
		if err != nil {
	...
	if got, want := output.String(), "agentsec-worker build 1.2.3-test\n"; got != want {
		t.Fatalf("output = %q, want %q", got, want)
	}
}
