...
func combinedE2EPageValues(provider collection.Provider, scenario string) ([]json.RawMessage, []json.RawMessage, bool, error) {
	...
	switch provider {
	case collection.ProviderAWS:
		entities = []combinedE2EEntity{{ID: "pid_24000001-0000-4000-8000-000000000001", Kind: "aws_account", SourceNativeID: "123456789012", DisplayName: "Production AWS account", StableFields: json.RawMessage(`{"account_id":"123456789012","name":"production"}`), Attributes: json.RawMessage(`{"state":"active"}`)}}
	case collection.ProviderKubernetes:
		entities = []combinedE2EEntity{{ID: "pid_21000001-0000-4000-8000-000000000001", Kind: "kubernetes_agent", SourceNativeID: "support-agent", DisplayName: "Support agent", StableFields: json.RawMessage(`{"cluster":"prod.example/cluster-a","name":"support-agent","namespace":"zasp"}`), Attributes: json.RawMessage(`{"namespaced":true,"state":"active"}`)}}
		if scenario != "shared" {
			entities = append(entities, combinedE2EEntity{ID: "pid_21000002-0000-4000-8000-000000000002", Kind: "kubernetes_workload", SourceNativeID: "support-runtime", DisplayName: "Production runtime", StableFields: json.RawMessage(`{"cluster":"prod.example/cluster-a","name":"support-runtime","namespace":"zasp","resource_kind":"Deployment"}`), Attributes: json.RawMessage(`{"namespaced":true,"state":"active"}`)})
			relationships = []combinedE2ERelationship{{ID: "pid_21000003-0000-4000-8000-000000000003", Kind: "contains", SourceNativeID: "support-agent-runtime", FromEntityID: "pid_21000001-0000-4000-8000-000000000001", ToEntityID: "pid_21000002-0000-4000-8000-000000000002", Attributes: json.RawMessage(`{"state":"active"}`)}}
		}
	case collection.ProviderGitHub:
		entities = []combinedE2EEntity{{ID: "pid_22000001-0000-4000-8000-000000000001", Kind: "github_repository", SourceNativeID: "zasp/security-automation", DisplayName: "Automation repository", StableFields: json.RawMessage(`{"installation_id":"424242","owner":"zasp","repository":"security-automation","visibility":"private"}`), Attributes: json.RawMessage(`{"archived":false,"default_branch":"main","state":"active"}`)}}
	case collection.ProviderOkta:
