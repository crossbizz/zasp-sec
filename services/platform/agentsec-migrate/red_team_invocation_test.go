...
func TestAgentsecMigrateRedTeamInvocationRelease(t *testing.T) {
	...
	if err := runReleaseMigration(context.Background(), up, []string{"up"}); err != nil || up.version != 42 || !equalMigrationEvents(up.events, []string{"version", "up-production-red-team-invocation", "up-production-red-team-artifacts", "up-production-runtime-sessions", "up-production-runtime-session-reads", "up-production-runtime-session-search", "version"}) {
