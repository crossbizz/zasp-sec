...
func TestAgentsecMigrateRedTeamInvocationRelease(t *testing.T) {
	...
	if err := runReleaseMigration(context.Background(), up, []string{"up"}); err != nil || up.version != 39 || !equalMigrationEvents(up.events, []string{"version", "up-production-red-team-invocation", "up-production-red-team-artifacts", "version"}) {
