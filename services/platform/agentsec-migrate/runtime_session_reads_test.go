...
func TestAgentsecMigrateRuntimeSessionReadsRelease(t *testing.T) {
	...
	if err := runReleaseMigration(context.Background(), up, []string{"up"}); err != nil || up.version != 44 || !equalMigrationEvents(up.events, []string{"version", "up-production-runtime-session-reads", "up-production-runtime-session-search", "up-production-runtime-session-query", "up-production-runtime-session-evidence", "version"}) {
