...
func TestAgentsecMigrateReachesV36AndDowngradesRuntimeReplayFirst(t *testing.T) {
	...
	if err := runReleaseMigration(context.Background(), up, []string{"up"}); err != nil || !equalMigrationEvents(up.events, []string{"version", "up-production-runtime-queue-replay", "up-production-red-team-safety", "up-production-red-team-invocation", "version"}) {
...
func TestAgentsecMigrateReachesV35AndDowngradesWebhookFirst(t *testing.T) {
	...
	if err := runReleaseMigration(context.Background(), up, []string{"up"}); err != nil || !equalMigrationEvents(up.events, []string{"version", "up-production-integration-webhook", "up-production-runtime-queue-replay", "up-production-red-team-safety", "up-production-red-team-invocation", "version"}) {
		t.Fatalf("up=%v err=%v", up.events, err)
	}
...
func TestAgentsecMigrateV14InstallsRollsBackReappliesAndBlocksPostCutoverRollback(t *testing.T) {
	...
	if version, versionErr := runner.Version(ctx); versionErr != nil || version != 38 {
...
func TestRunReleaseMigrationRejectsDriftAndHonorsDeadline(t *testing.T) {
	if err := runReleaseMigration(context.Background(), &scriptedMigrationRunner{version: 39}, []string{"up"}); !errors.Is(err, migrations.ErrInvalidState) {
...
func TestReleaseMigrationReachesExactPostgresTargetFromEmptyV1AndV2AndRejectsDrift(t *testing.T) {
	...
	if version, err := runner.Version(ctx); err != nil || version != 38 {
