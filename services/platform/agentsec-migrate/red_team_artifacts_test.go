...
func TestAgentsecMigrateRedTeamArtifactsRelease(t *testing.T) {
	...
	if err := runReleaseMigration(context.Background(), up, []string{"up"}); err != nil || up.version != 41 || !equalMigrationEvents(up.events, []string{"version", "up-production-red-team-artifacts", "up-production-runtime-sessions", "up-production-runtime-session-reads", "version"}) {
