...
func NewCollectionClient(api CollectionAPI, artifacts artifactstore.ArtifactStore, config CollectionClientConfig) (collection.ProviderClient, error) {
	return providercollection.New(providercollection.Config{Provider: collection.ProviderAWS, API: api, Artifacts: artifacts, Bucket: config.Bucket, CollectorVersion: config.CollectorVersion, Clock: config.Clock})
}
