...
BEGIN
 ...
 IF FOUND THEN IF (prior_value.request_digest,prior_value.sensor_id,prior_value.expected_version,prior_value.token_id,prior_value.token_generation) IS DISTINCT FROM (request_digest_value,sensor_value,0,token_value,generation_value) THEN RAISE EXCEPTION USING ERRCODE='23505',MESSAGE='sensor mutation conflict';END IF;RETURN jsonb_set(prior_value.result,'{replayed}','true'::jsonb);END IF;
...
BEGIN
 ...
 IF FOUND THEN IF (prior_value.request_digest,prior_value.sensor_id,prior_value.expected_version,prior_value.token_id,prior_value.token_generation) IS DISTINCT FROM (request_digest_value,sensor_value,expected_version_value,token_value,generation_value) THEN RAISE EXCEPTION USING ERRCODE='23505',MESSAGE='sensor mutation conflict';END IF;RETURN jsonb_set(prior_value.result,'{replayed}','true'::jsonb);END IF;
...
CREATE FUNCTION public.zasp_runtime_data_plane_live_fingerprint() RETURNS text LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO pg_catalog, public AS $$
 WITH semantic_object(kind,identity,definition) AS (
  ...
  UNION ALL SELECT 'constraint',constraint_value.conrelid::regclass::text||'.'||constraint_value.conname,pg_get_constraintdef(constraint_value.oid,true) FROM pg_constraint constraint_value WHERE constraint_value.conrelid IN('public.zasp_sensors'::regclass,'public.zasp_sensor_tokens'::regclass,'public.zasp_gateway_enrollment_tokens'::regclass,'public.zasp_gateway_credentials'::regclass,'public.zasp_runtime_data_plane_state'::regclass,'public.zasp_runtime_sensor_mutations'::regclass,'public.zasp_runtime_legacy_token_transitions'::regclass,'public.zasp_runtime_principal_bindings'::regclass,'public.zasp_runtime_batch_authorities'::regclass,'public.zasp_runtime_stage_work'::regclass,'public.zasp_runtime_deliveries'::regclass,'public.zasp_runtime_stage_fairness'::regclass,'public.zasp_runtime_gateway_replay_receipts'::regclass,'public.zasp_runtime_gateway_policy_bundles'::regclass,'public.zasp_runtime_gateway_events'::regclass)
...
INSERT INTO public.zasp_schema_metadata(key,value) VALUES('runtime_data_plane_fingerprint', '27231caf874018313cc8c25de846549ecb8d65c1e41cd49f43233af1d6277ab5');
