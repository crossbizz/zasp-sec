...
CREATE TABLE public.zasp_red_team_runs(
  ...
  CHECK(worker_id IS NULL OR zasp_valid_product_id(worker_id)),CHECK(lease_token IS NULL OR octet_length(lease_token)=32),
...
CREATE TABLE public.zasp_red_team_outbox(
  ...
  CHECK((state='leased')=(worker_id IS NOT NULL AND lease_token IS NOT NULL AND lease_expires_at IS NOT NULL)),CHECK(worker_id IS NULL OR zasp_valid_product_id(worker_id)),CHECK(lease_token IS NULL OR octet_length(lease_token)=32),
...
BEGIN IF NOT zasp_red_team_principal_ready('zasp_red_team_worker') OR NOT zasp_valid_product_id(organization_value) OR NOT zasp_valid_product_id(workspace_value) OR NOT zasp_valid_product_id(environment_value) OR NOT zasp_valid_product_id(run_value) OR NOT zasp_valid_product_id(worker_value) OR octet_length(token_value)<>32 OR lease_seconds NOT BETWEEN 30 AND 900 THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='red team run claim rejected';END IF;
...
INSERT INTO public.zasp_schema_metadata(key,value) VALUES('red_team_execution_fingerprint', 'b1d846e439d0fb8bfb31b3cde65a5277bf793283653b767f4dd26ca210e3a17c') ON CONFLICT(key) DO UPDATE SET value=excluded.value;
