...
CREATE FUNCTION public.zasp_red_team_definition_valid(name_value text,target_value text,target_kind_value text,categories_value jsonb,safety_value jsonb) RETURNS boolean LANGUAGE sql IMMUTABLE SET search_path TO pg_catalog, public AS $valid$
 ...
 AND (SELECT count(*)=count(DISTINCT item#>>'{}') FROM jsonb_array_elements(safety_value->'expected_side_effects') item)
$valid$;
...
CREATE FUNCTION public.zasp_red_team_run_json(row_value public.zasp_red_team_runs) RETURNS jsonb LANGUAGE sql STABLE SET search_path TO pg_catalog, public AS $json$
 SELECT jsonb_strip_nulls(jsonb_build_object('id',row_value.run_id,'version',row_value.version,'definition_id',row_value.definition_id,'definition_version',row_value.definition_version,'status',row_value.state,'attempt',row_value.attempt,'cancel_requested',row_value.cancel_requested,'queued_at',to_char(row_value.queued_at AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),'started_at',to_char(row_value.started_at AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),'completed_at',to_char(row_value.completed_at AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),'verdict',row_value.verdict,'error_code',row_value.error_code,'evidence_reference',row_value.evidence_reference))
$json$;
...
INSERT INTO public.zasp_schema_metadata(key,value) VALUES('red_team_execution_fingerprint', 'a47f563170e40b2553b80063014483bd043862b4454a0c75f029d6fc02064119') ON CONFLICT(key) DO UPDATE SET value=excluded.value;
