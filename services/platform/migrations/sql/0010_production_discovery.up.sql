...
CREATE FUNCTION "public"."zasp_discovery_s3_object_reference"(value text) RETURNS boolean
LANGUAGE sql IMMUTABLE STRICT AS $$
    SELECT length(value) BETWEEN 8 AND 1024
       AND value ~ '^s3://[a-z0-9][a-z0-9.-]{1,61}[a-z0-9]/[A-Za-z0-9][A-Za-z0-9._/-]*$'
       AND split_part(substring(value FROM 6),'/',1) !~ '(\.\.|\.-|-\.)'
       AND NOT EXISTS (
           SELECT 1 FROM unnest(string_to_array(regexp_replace(value,'^s3://[^/]+/',''),'/')) segment
           WHERE segment IN ('','.','..')
       )
$$;

CREATE TABLE "public"."zasp_discovery_principal_bindings" (
    principal_name text NOT NULL CHECK(principal_name ~ '^[a-z][a-z0-9_]{2,62}$'),
    authority_role text NOT NULL CHECK(authority_role IN ('zasp_discovery_authority','zasp_discovery_api','zasp_discovery_worker','zasp_runtime_ingest','zasp_runtime_worker','zasp_outbox_worker','zasp_runtime_gateway')),
...
CREATE TABLE "public"."zasp_discovery_snapshots" (
    ...
    source text NOT NULL CHECK(length(source) BETWEEN 1 AND 64), manifest_reference text NOT NULL CHECK(zasp_discovery_s3_object_reference(manifest_reference)),
...
CREATE TABLE "public"."zasp_inventory_evidence" (
    ...
    finding_id text, object_reference text NOT NULL CHECK(zasp_discovery_s3_object_reference(object_reference)), checksum bytea NOT NULL CHECK(octet_length(checksum)=32),
...
CREATE TABLE "public"."zasp_runtime_batches" (
    ...
    payload_reference text NOT NULL CHECK(zasp_discovery_s3_object_reference(payload_reference)),
    ...
    CHECK(archive_reference IS NULL OR zasp_discovery_s3_object_reference(archive_reference)), CHECK((state IN ('succeeded','failed'))=(completed_at IS NOT NULL))
);
...
RETURNS jsonb LANGUAGE plpgsql AS $$ DECLARE response jsonb;required_role text; BEGIN
 ...
 required_role:=CASE WHEN requested_kind='runtime' THEN 'zasp_runtime_worker' ELSE 'zasp_discovery_worker' END;
 IF NOT pg_has_role(session_user,required_role,'MEMBER') AND NOT pg_has_role(session_user,'zasp_discovery_authority','MEMBER') THEN RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='job authority denied';END IF;
...
RETURNS jsonb LANGUAGE plpgsql AS $$
#variable_conflict use_column
DECLARE requested bytea;effective text;result jsonb;current_attempt integer;job_kind text;job_authority_id text;required_role text;
BEGIN
 ...
 SELECT kind INTO job_kind FROM zasp_discovery_jobs WHERE organization_id=$1 AND workspace_id=$2 AND environment_id=$3 AND id=$4;
 IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE='P0002',MESSAGE='job missing';END IF;
 required_role:=CASE WHEN job_kind='runtime' THEN 'zasp_runtime_worker' ELSE 'zasp_discovery_worker' END;
 IF NOT pg_has_role(session_user,required_role,'MEMBER') AND NOT pg_has_role(session_user,'zasp_discovery_authority','MEMBER') THEN RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='job authority denied';END IF;
 ...
 SELECT attempt,authority_id INTO current_attempt,job_authority_id FROM zasp_discovery_jobs WHERE organization_id=$1 AND workspace_id=$2 AND environment_id=$3 AND id=$4 AND state='leased' AND lease_owner=$5 AND lease_token=$6 AND lease_expires_at>transaction_timestamp() FOR UPDATE;
...
CREATE FUNCTION "public"."zasp_discovery_register_principals"(migration_principal text,api_principal text,discovery_principal text,ingest_principal text,runtime_principal text,outbox_principal text,gateway_principal text) RETURNS boolean LANGUAGE plpgsql AS $$
DECLARE principals text[]:=ARRAY[$1,$2,$3,$4,$5,$6,$7];authorities text[]:=ARRAY['zasp_discovery_authority','zasp_discovery_api','zasp_discovery_worker','zasp_runtime_ingest','zasp_runtime_worker','zasp_outbox_worker','zasp_runtime_gateway'];principal text;authority text;index integer;role_value record;
BEGIN
 IF $1<>session_user OR cardinality(ARRAY(SELECT DISTINCT unnest(principals)))<>7 OR EXISTS(SELECT 1 FROM unnest(principals) value WHERE value !~ '^[a-z][a-z0-9_]{2,62}$') THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='invalid discovery principals';END IF;
 PERFORM pg_advisory_xact_lock(hashtextextended('zasp-discovery-principal-registration',0));
 FOR index IN 1..7 LOOP
   principal:=principals[index];authority:=authorities[index];
   SELECT r.oid,r.rolcanlogin,r.rolsuper,r.rolcreatedb,r.rolcreaterole,r.rolreplication,r.rolinherit,r.rolbypassrls INTO role_value FROM pg_roles r WHERE r.rolname=principal;
   IF NOT FOUND OR NOT role_value.rolcanlogin OR authority<>'zasp_discovery_authority' AND (role_value.rolsuper OR role_value.rolcreatedb OR role_value.rolcreaterole OR role_value.rolreplication OR NOT role_value.rolinherit OR role_value.rolbypassrls) THEN RAISE EXCEPTION USING ERRCODE='55000',MESSAGE='unsafe discovery principal';END IF;
   IF EXISTS(SELECT 1 FROM pg_auth_members membership JOIN pg_roles granted ON granted.oid=membership.roleid WHERE membership.member=role_value.oid AND granted.rolname LIKE 'zasp_%' AND (granted.rolname<>authority OR membership.admin_option<>(authority='zasp_discovery_authority'))) THEN RAISE EXCEPTION USING ERRCODE='55000',MESSAGE='unsafe discovery principal membership';END IF;
   IF authority<>'zasp_discovery_authority' THEN EXECUTE format('GRANT %I TO %I',authority,principal);END IF;
   INSERT INTO zasp_discovery_principal_bindings(principal_name,authority_role) VALUES(principal,authority) ON CONFLICT(principal_name) DO UPDATE SET authority_role=excluded.authority_role WHERE zasp_discovery_principal_bindings.authority_role=excluded.authority_role;
   IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE='23505',MESSAGE='discovery principal conflict';END IF;
 END LOOP;
 RETURN true;
END $$;

CREATE FUNCTION "public"."zasp_discovery_principal_ready"(expected_authority text) RETURNS boolean LANGUAGE sql STABLE AS $$
 SELECT expected_authority IN ('zasp_discovery_api','zasp_discovery_worker','zasp_runtime_ingest','zasp_runtime_worker','zasp_outbox_worker','zasp_runtime_gateway')
 ...
 AND NOT EXISTS(SELECT 1 FROM pg_auth_members membership JOIN pg_roles principal ON principal.oid=membership.member JOIN pg_roles granted ON granted.oid=membership.roleid WHERE principal.rolname=session_user AND granted.rolname LIKE 'zasp_%' AND (granted.rolname<>expected_authority OR membership.admin_option))
 AND NOT EXISTS(SELECT 1 FROM pg_class class JOIN pg_namespace namespace ON namespace.oid=class.relnamespace CROSS JOIN aclexplode(COALESCE(class.relacl,acldefault('r',class.relowner))) acl WHERE namespace.nspname='public' AND class.relname=ANY(ARRAY['zasp_discovery_principal_bindings','zasp_integrations','zasp_integration_connections','zasp_discovery_schedules','zasp_discovery_syncs','zasp_discovery_jobs','zasp_discovery_snapshots','zasp_discovery_cursors','zasp_inventory_entities','zasp_inventory_source_observations','zasp_inventory_relationships','zasp_inventory_evidence','zasp_sensors','zasp_sensor_tokens','zasp_sensor_heartbeats','zasp_runtime_batches','zasp_runtime_stages','zasp_discovery_outbox','zasp_projection_work','zasp_gateway_devices','zasp_gateway_enrollment_tokens','zasp_gateway_credentials','zasp_gateway_policy_subscriptions']) AND acl.grantee=(SELECT oid FROM pg_roles WHERE rolname=session_user))
 AND NOT EXISTS(SELECT 1 FROM pg_proc procedure JOIN pg_namespace namespace ON namespace.oid=procedure.pronamespace CROSS JOIN aclexplode(COALESCE(procedure.proacl,acldefault('f',procedure.proowner))) acl WHERE namespace.nspname='public' AND procedure.proname LIKE 'zasp_discovery_%' AND acl.grantee=(SELECT oid FROM pg_roles WHERE rolname=session_user));
$$;

CREATE FUNCTION "public"."zasp_discovery_security_ready"() RETURNS boolean LANGUAGE sql STABLE AS $$
 SELECT NOT EXISTS(SELECT 1 FROM pg_roles r WHERE r.rolname IN('zasp_discovery_authority','zasp_discovery_api','zasp_discovery_worker','zasp_runtime_ingest','zasp_runtime_worker','zasp_outbox_worker','zasp_runtime_gateway') AND (r.rolcanlogin OR r.rolsuper OR r.rolcreatedb OR r.rolcreaterole OR r.rolinherit OR r.rolbypassrls))
 AND NOT EXISTS(SELECT 1 FROM unnest(ARRAY['zasp_discovery_api','zasp_discovery_worker','zasp_runtime_ingest','zasp_runtime_worker','zasp_outbox_worker','zasp_runtime_gateway']) role_name WHERE pg_has_role(role_name,'zasp_discovery_authority','MEMBER'))
 AND NOT EXISTS(SELECT 1 FROM pg_auth_members membership JOIN pg_roles granted ON granted.oid=membership.roleid JOIN pg_roles member ON member.oid=membership.member LEFT JOIN zasp_discovery_principal_bindings binding ON binding.principal_name=member.rolname AND binding.authority_role=granted.rolname WHERE granted.rolname IN('zasp_discovery_authority','zasp_discovery_api','zasp_discovery_worker','zasp_runtime_ingest','zasp_runtime_worker','zasp_outbox_worker','zasp_runtime_gateway') AND NOT (granted.rolname<>'zasp_discovery_authority' AND member.rolname='zasp_discovery_authority' AND membership.admin_option OR granted.rolname='zasp_discovery_authority' AND member.rolname=session_user AND membership.admin_option OR binding.principal_name IS NOT NULL AND membership.admin_option=(granted.rolname='zasp_discovery_authority')))
 AND NOT EXISTS(SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname=ANY(ARRAY['zasp_discovery_principal_bindings','zasp_integrations','zasp_integration_connections','zasp_discovery_schedules','zasp_discovery_syncs','zasp_discovery_jobs','zasp_discovery_snapshots','zasp_discovery_cursors','zasp_inventory_entities','zasp_inventory_source_observations','zasp_inventory_relationships','zasp_inventory_evidence','zasp_sensors','zasp_sensor_tokens','zasp_sensor_heartbeats','zasp_runtime_batches','zasp_runtime_stages','zasp_discovery_outbox','zasp_projection_work','zasp_gateway_devices','zasp_gateway_enrollment_tokens','zasp_gateway_credentials','zasp_gateway_policy_subscriptions']) AND (c.relowner<>(SELECT oid FROM pg_roles WHERE rolname='zasp_discovery_authority') OR NOT c.relrowsecurity OR NOT c.relforcerowsecurity OR (SELECT count(*) FROM pg_policy p WHERE p.polrelid=c.oid)<>1 OR NOT EXISTS(SELECT 1 FROM pg_policy p WHERE p.polrelid=c.oid AND p.polname=c.relname||'_authority' AND p.polpermissive AND p.polcmd='*' AND p.polroles=ARRAY[(SELECT oid FROM pg_roles WHERE rolname='zasp_discovery_authority')] AND pg_get_expr(p.polqual,p.polrelid)='true' AND pg_get_expr(p.polwithcheck,p.polrelid)='true') OR EXISTS(SELECT 1 FROM aclexplode(COALESCE(c.relacl,acldefault('r',c.relowner))) a WHERE a.grantee<>c.relowner)))
 AND NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname LIKE 'zasp_discovery_%' AND (p.proowner<>(SELECT oid FROM pg_roles WHERE rolname='zasp_discovery_authority') OR NOT p.prosecdef OR NOT COALESCE(p.proconfig,'{}') @> ARRAY['search_path=pg_catalog, public'] OR EXISTS(SELECT 1 FROM aclexplode(COALESCE(p.proacl,acldefault('f',p.proowner))) a WHERE a.privilege_type='EXECUTE' AND a.grantee NOT IN(p.proowner,(SELECT oid FROM pg_roles WHERE rolname='zasp_discovery_api'),(SELECT oid FROM pg_roles WHERE rolname='zasp_discovery_worker'),(SELECT oid FROM pg_roles WHERE rolname='zasp_runtime_ingest'),(SELECT oid FROM pg_roles WHERE rolname='zasp_runtime_worker'),(SELECT oid FROM pg_roles WHERE rolname='zasp_outbox_worker'),(SELECT oid FROM pg_roles WHERE rolname='zasp_runtime_gateway'))) OR has_function_privilege('zasp_discovery_api',p.oid,'EXECUTE')<>(p.proname=ANY(ARRAY['zasp_discovery_security_ready','zasp_discovery_readiness','zasp_discovery_principal_ready','zasp_discovery_create_integration','zasp_discovery_transition_integration','zasp_discovery_put_connection','zasp_discovery_transition_connection','zasp_discovery_put_schedule','zasp_discovery_transition_schedule','zasp_discovery_create_sensor','zasp_discovery_transition_sensor','zasp_discovery_create_gateway_device','zasp_discovery_transition_gateway_device','zasp_discovery_issue_gateway_enrollment','zasp_discovery_revoke_gateway_enrollment','zasp_discovery_issue_sensor_token','zasp_discovery_sensor_rotate','zasp_discovery_sensor_revoke','zasp_discovery_request_sync','zasp_discovery_entity_page','zasp_discovery_evidence_get'])) OR has_function_privilege('zasp_discovery_worker',p.oid,'EXECUTE')<>(p.proname=ANY(ARRAY['zasp_discovery_security_ready','zasp_discovery_readiness','zasp_discovery_principal_ready','zasp_discovery_apply_snapshot','zasp_discovery_claim_jobs','zasp_discovery_claim_schedules','zasp_discovery_complete_schedule','zasp_discovery_claim_projection_work','zasp_discovery_finish_job','zasp_discovery_finish_projection','zasp_discovery_complete_job','zasp_discovery_complete_projection'])) OR has_function_privilege('zasp_runtime_ingest',p.oid,'EXECUTE')<>(p.proname=ANY(ARRAY['zasp_discovery_security_ready','zasp_discovery_readiness','zasp_discovery_principal_ready','zasp_discovery_sensor_heartbeat','zasp_discovery_create_runtime_batch'])) OR has_function_privilege('zasp_runtime_worker',p.oid,'EXECUTE')<>(p.proname=ANY(ARRAY['zasp_discovery_security_ready','zasp_discovery_readiness','zasp_discovery_principal_ready','zasp_discovery_claim_jobs','zasp_discovery_finish_job','zasp_discovery_complete_job','zasp_discovery_complete_runtime_stage'])) OR has_function_privilege('zasp_outbox_worker',p.oid,'EXECUTE')<>(p.proname=ANY(ARRAY['zasp_discovery_security_ready','zasp_discovery_readiness','zasp_discovery_principal_ready','zasp_discovery_claim_outbox','zasp_discovery_ack_outbox','zasp_discovery_retry_outbox'])) OR has_function_privilege('zasp_runtime_gateway',p.oid,'EXECUTE')<>(p.proname=ANY(ARRAY['zasp_discovery_security_ready','zasp_discovery_readiness','zasp_discovery_principal_ready','zasp_discovery_gateway_enroll','zasp_discovery_gateway_advance_replay','zasp_discovery_gateway_rotate','zasp_discovery_gateway_revoke','zasp_discovery_put_gateway_policy']))))
 AND NOT EXISTS(SELECT 1 FROM zasp_discovery_principal_bindings binding JOIN pg_roles principal ON principal.rolname=binding.principal_name WHERE NOT principal.rolcanlogin OR binding.authority_role<>'zasp_discovery_authority' AND (NOT principal.rolinherit OR principal.rolsuper OR principal.rolcreatedb OR principal.rolcreaterole OR principal.rolreplication OR principal.rolbypassrls OR pg_has_role(binding.principal_name,'zasp_discovery_authority','MEMBER')) OR NOT pg_has_role(binding.principal_name,binding.authority_role,'MEMBER') OR EXISTS(SELECT 1 FROM pg_auth_members membership JOIN pg_roles granted ON granted.oid=membership.roleid WHERE membership.member=principal.oid AND granted.rolname LIKE 'zasp_%' AND (granted.rolname<>binding.authority_role OR membership.admin_option<>(binding.authority_role='zasp_discovery_authority'))))
$$;
...
BEGIN
  FOREACH role_name IN ARRAY ARRAY['zasp_discovery_authority','zasp_discovery_api','zasp_discovery_worker','zasp_runtime_ingest','zasp_runtime_worker','zasp_outbox_worker','zasp_runtime_gateway'] LOOP
...
GRANT zasp_discovery_api,zasp_discovery_worker,zasp_runtime_ingest,zasp_runtime_worker,zasp_outbox_worker,zasp_runtime_gateway TO zasp_discovery_authority WITH ADMIN OPTION;
GRANT USAGE ON SCHEMA public TO zasp_discovery_api,zasp_discovery_worker,zasp_runtime_ingest,zasp_runtime_worker,zasp_outbox_worker,zasp_runtime_gateway;
GRANT SELECT ON zasp_schema_versions,zasp_schema_metadata TO zasp_discovery_authority,zasp_discovery_api,zasp_discovery_worker,zasp_runtime_ingest,zasp_runtime_worker,zasp_outbox_worker,zasp_runtime_gateway;

DO $legacy_api$
DECLARE object_value record;
BEGIN
  FOR object_value IN SELECT class.oid,class.relkind FROM pg_class class JOIN pg_namespace namespace ON namespace.oid=class.relnamespace WHERE namespace.nspname='public' AND class.relname LIKE 'zasp_%' AND class.relkind IN('r','p','S') AND class.relname<>ALL(ARRAY['zasp_discovery_principal_bindings','zasp_integrations','zasp_integration_connections','zasp_discovery_schedules','zasp_discovery_syncs','zasp_discovery_jobs','zasp_discovery_snapshots','zasp_discovery_cursors','zasp_inventory_entities','zasp_inventory_source_observations','zasp_inventory_relationships','zasp_inventory_evidence','zasp_sensors','zasp_sensor_tokens','zasp_sensor_heartbeats','zasp_runtime_batches','zasp_runtime_stages','zasp_discovery_outbox','zasp_projection_work','zasp_gateway_devices','zasp_gateway_enrollment_tokens','zasp_gateway_credentials','zasp_gateway_policy_subscriptions']) LOOP
    IF object_value.relkind='S' THEN EXECUTE format('GRANT USAGE,SELECT,UPDATE ON SEQUENCE %s TO zasp_discovery_api',object_value.oid::regclass);
    ELSE EXECUTE format('GRANT SELECT,INSERT,UPDATE,DELETE ON TABLE %s TO zasp_discovery_api',object_value.oid::regclass);END IF;
  END LOOP;
  FOR object_value IN SELECT procedure.oid FROM pg_proc procedure JOIN pg_namespace namespace ON namespace.oid=procedure.pronamespace WHERE namespace.nspname='public' AND procedure.proname LIKE 'zasp_%' AND procedure.proname NOT LIKE 'zasp_discovery_%' AND procedure.proname NOT IN('zasp_valid_product_id','zasp_reference_only') LOOP
    EXECUTE format('GRANT EXECUTE ON FUNCTION %s TO zasp_discovery_api',object_value.oid::regprocedure);
  END LOOP;
END $legacy_api$;

DO $rls$ DECLARE table_name text; BEGIN
  FOREACH table_name IN ARRAY ARRAY['zasp_discovery_principal_bindings','zasp_integrations','zasp_integration_connections','zasp_discovery_schedules','zasp_discovery_syncs','zasp_discovery_jobs','zasp_discovery_snapshots','zasp_discovery_cursors','zasp_inventory_entities','zasp_inventory_source_observations','zasp_inventory_relationships','zasp_inventory_evidence','zasp_sensors','zasp_sensor_tokens','zasp_sensor_heartbeats','zasp_runtime_batches','zasp_runtime_stages','zasp_discovery_outbox','zasp_projection_work','zasp_gateway_devices','zasp_gateway_enrollment_tokens','zasp_gateway_credentials','zasp_gateway_policy_subscriptions'] LOOP
    EXECUTE format('ALTER TABLE public.%I OWNER TO zasp_discovery_authority',table_name);
    EXECUTE format('ALTER TABLE public.%I ENABLE ROW LEVEL SECURITY',table_name);
    EXECUTE format('ALTER TABLE public.%I FORCE ROW LEVEL SECURITY',table_name);
    EXECUTE format('REVOKE ALL ON public.%I FROM PUBLIC,zasp_discovery_api,zasp_discovery_worker,zasp_runtime_ingest,zasp_runtime_worker,zasp_outbox_worker,zasp_runtime_gateway',table_name);
...
GRANT EXECUTE ON FUNCTION zasp_discovery_sensor_heartbeat(text,text,text,text,bigint,text,bigint,jsonb),zasp_discovery_create_runtime_batch(text,text,text,text,text,text,text,text,bytea,integer,text,bigint,text,text) TO zasp_runtime_ingest;
GRANT EXECUTE ON FUNCTION zasp_discovery_claim_jobs(text,text,integer,integer,text),zasp_discovery_finish_job(text,text,text,text,text,text,text,bytea,text,integer),zasp_discovery_complete_job(text,text,text,text,text,text,bytea,boolean,text),zasp_discovery_complete_runtime_stage(text,text,text,text,text,bytea,boolean,text) TO zasp_runtime_worker;
...
GRANT EXECUTE ON FUNCTION zasp_discovery_gateway_enroll(text,text,text,text,text,text,bytea,text,text,bytea,timestamptz),zasp_discovery_gateway_advance_replay(text,text,text,text,bigint,bigint),zasp_discovery_gateway_rotate(text,text,text,text,text,text,text,bytea,timestamptz),zasp_discovery_gateway_revoke(text,text,text,text,text),zasp_discovery_put_gateway_policy(text,text,text,text,text,bigint,bytea,bytea,timestamptz,timestamptz,bigint) TO zasp_runtime_gateway;
...
INSERT INTO zasp_schema_metadata(key,value) VALUES ('production_discovery_fingerprint', 'a3ee9cb3bfd3e6ed0d37399817432ec9ebdc4e4a66b778d2e1b79c62f99a65f9');
