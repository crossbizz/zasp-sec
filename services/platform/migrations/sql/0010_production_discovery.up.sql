...
DECLARE result jsonb;BEGIN UPDATE zasp_integration_connections SET state=$7,version=version+1,updated_at=transaction_timestamp(),verified_at=CASE WHEN $7='verified' THEN transaction_timestamp() ELSE verified_at END,revoked_at=CASE WHEN $7='revoked' THEN transaction_timestamp() ELSE NULL END WHERE zasp_integration_connections.organization_id=$1 AND zasp_integration_connections.workspace_id=$2 AND zasp_integration_connections.environment_id=$3 AND id=$4 AND integration_id=$5 AND version=$6 AND (($7='verified' AND state='pending') OR ($7='invalid' AND state IN('pending','verified')) OR ($7='revoked' AND state IN('pending','verified','invalid'))) RETURNING jsonb_build_object('id',id,'state',state,'version',version,'updated_at',updated_at) INTO result;IF NOT FOUND THEN SELECT jsonb_build_object('id',id,'state',state,'version',version,'updated_at',updated_at) INTO result FROM zasp_integration_connections WHERE organization_id=$1 AND workspace_id=$2 AND environment_id=$3 AND id=$4 AND integration_id=$5 AND version=$6+1 AND state=$7;END IF;IF result IS NULL THEN RAISE EXCEPTION USING ERRCODE='40001',MESSAGE='connection transition conflict';END IF;RETURN result;END $$;
...
RETURNS jsonb LANGUAGE plpgsql AS $$ DECLARE requested bytea;effective text;result jsonb;current_attempt integer;BEGIN IF outcome NOT IN('succeeded','retryable','failed','cancelled') OR retry_after_seconds NOT BETWEEN 0 AND 3600 THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='invalid job outcome';END IF;requested:=digest(convert_to(concat_ws(chr(31),$4,$5,$6,$7,COALESCE(encode($8,'hex'),''),COALESCE($9,''),$10::text),'UTF8'),'sha256');SELECT completion_result INTO result FROM zasp_discovery_jobs WHERE organization_id=$1 AND workspace_id=$2 AND environment_id=$3 AND id=$4 AND completion_digest=requested;IF FOUND THEN RETURN result;END IF;SELECT attempt INTO current_attempt FROM zasp_discovery_jobs WHERE organization_id=$1 AND workspace_id=$2 AND environment_id=$3 AND id=$4 AND state='leased' AND lease_owner=$5 AND lease_token=$6 AND lease_expires_at>transaction_timestamp() FOR UPDATE;IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE='P0002',MESSAGE='job lease missing';END IF;effective:=CASE WHEN outcome='retryable' AND current_attempt>=5 THEN 'failed' ELSE outcome END;result:=jsonb_build_object('id',$4,'state',effective,'attempt',current_attempt,'completed_at',CASE WHEN effective IN('succeeded','failed','cancelled') THEN transaction_timestamp() ELSE NULL END);UPDATE zasp_discovery_jobs SET state=effective,result_digest=$8,last_error=NULLIF($9,''),available_at=CASE WHEN effective='retryable' THEN transaction_timestamp()+make_interval(secs=>$10) ELSE available_at END,completed_at=CASE WHEN effective IN('succeeded','failed','cancelled') THEN transaction_timestamp() ELSE NULL END,lease_owner=NULL,lease_token=NULL,lease_expires_at=NULL,completion_digest=requested,completion_result=result,updated_at=transaction_timestamp() WHERE organization_id=$1 AND workspace_id=$2 AND environment_id=$3 AND id=$4;RETURN result;END $$;
...
INSERT INTO zasp_schema_metadata(key,value) VALUES ('production_discovery_fingerprint', 'c73e085b02ae87e7fcba211a4857176174abd71f767924ac906a7b444fad4269');
