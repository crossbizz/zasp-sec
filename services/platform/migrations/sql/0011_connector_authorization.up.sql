...
CREATE FUNCTION "public"."zasp_connector_effect_lanes_insert"() RETURNS trigger
LANGUAGE plpgsql AS $$
BEGIN
  INSERT INTO zasp_connector_effect_lane_scopes(provider,operation,organization_id,workspace_id,environment_id,eligible_count)
  SELECT provider,operation,organization_id,workspace_id,environment_id,count(*) FROM new_connector_effects WHERE status='unknown' AND attempt<100 GROUP BY provider,operation,organization_id,workspace_id,environment_id
  ON CONFLICT(provider,operation,organization_id,workspace_id,environment_id) DO UPDATE SET eligible_count=zasp_connector_effect_lane_scopes.eligible_count+EXCLUDED.eligible_count;
  RETURN NULL;
END $$;

CREATE FUNCTION "public"."zasp_connector_effect_lanes_update"() RETURNS trigger
LANGUAGE plpgsql AS $$
BEGIN
  INSERT INTO zasp_connector_effect_lane_scopes(provider,operation,organization_id,workspace_id,environment_id,eligible_count)
  SELECT provider,operation,organization_id,workspace_id,environment_id,count(*) FROM new_connector_effects WHERE status='unknown' AND attempt<100 GROUP BY provider,operation,organization_id,workspace_id,environment_id
  ON CONFLICT(provider,operation,organization_id,workspace_id,environment_id) DO UPDATE SET eligible_count=zasp_connector_effect_lane_scopes.eligible_count+EXCLUDED.eligible_count;
  WITH retired AS (SELECT provider,operation,organization_id,workspace_id,environment_id,count(*) eligible_count FROM old_connector_effects WHERE status='unknown' AND attempt<100 GROUP BY provider,operation,organization_id,workspace_id,environment_id)
  DELETE FROM zasp_connector_effect_lane_scopes lane USING retired WHERE (lane.provider,lane.operation,lane.organization_id,lane.workspace_id,lane.environment_id)=(retired.provider,retired.operation,retired.organization_id,retired.workspace_id,retired.environment_id) AND lane.eligible_count<=retired.eligible_count;
  WITH retired AS (SELECT provider,operation,organization_id,workspace_id,environment_id,count(*) eligible_count FROM old_connector_effects WHERE status='unknown' AND attempt<100 GROUP BY provider,operation,organization_id,workspace_id,environment_id)
  UPDATE zasp_connector_effect_lane_scopes lane SET eligible_count=lane.eligible_count-retired.eligible_count FROM retired WHERE (lane.provider,lane.operation,lane.organization_id,lane.workspace_id,lane.environment_id)=(retired.provider,retired.operation,retired.organization_id,retired.workspace_id,retired.environment_id) AND lane.eligible_count>retired.eligible_count;
  RETURN NULL;
END $$;

CREATE FUNCTION "public"."zasp_connector_effect_lanes_delete"() RETURNS trigger
LANGUAGE plpgsql AS $$
BEGIN
  WITH retired AS (SELECT provider,operation,organization_id,workspace_id,environment_id,count(*) eligible_count FROM old_connector_effects WHERE status='unknown' AND attempt<100 GROUP BY provider,operation,organization_id,workspace_id,environment_id)
  DELETE FROM zasp_connector_effect_lane_scopes lane USING retired WHERE (lane.provider,lane.operation,lane.organization_id,lane.workspace_id,lane.environment_id)=(retired.provider,retired.operation,retired.organization_id,retired.workspace_id,retired.environment_id) AND lane.eligible_count<=retired.eligible_count;
...
CREATE TRIGGER zasp_connector_effect_lanes_insert AFTER INSERT ON zasp_connector_effects REFERENCING NEW TABLE AS new_connector_effects FOR EACH STATEMENT EXECUTE FUNCTION zasp_connector_effect_lanes_insert();
CREATE TRIGGER zasp_connector_effect_lanes_update AFTER UPDATE ON zasp_connector_effects REFERENCING OLD TABLE AS old_connector_effects NEW TABLE AS new_connector_effects FOR EACH STATEMENT EXECUTE FUNCTION zasp_connector_effect_lanes_update();
CREATE TRIGGER zasp_connector_effect_lanes_delete AFTER DELETE ON zasp_connector_effects REFERENCING OLD TABLE AS old_connector_effects FOR EACH STATEMENT EXECUTE FUNCTION zasp_connector_effect_lanes_delete();
...
BEGIN
	IF provider_value NOT IN ('github','okta') AND provider_value !~ '^nango:[a-z0-9][a-z0-9_-]{1,62}$' OR expires_value<=transaction_timestamp() OR expires_value>transaction_timestamp()+interval '10 minutes' OR NOT zasp_valid_product_id(cleanup_value) OR NOT zasp_valid_product_id(authorize_value) THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='invalid oauth attempt'; END IF;
  ...
  IF EXISTS(SELECT 1 FROM zasp_connector_effects WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND integration_id=integration_value AND provider=provider_value AND operation='authorize' AND status IN('pending','unknown') AND id<>authorize_value) THEN RAISE EXCEPTION USING ERRCODE='23505',MESSAGE='connector authorization unresolved'; END IF;
  ...
  UPDATE zasp_connector_effects SET oauth_attempt_id=attempt_value,updated_at=updated_at WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND id=cleanup_value AND integration_id=integration_value AND oauth_attempt_id IS NULL AND provider=provider_value AND operation='pkce_cleanup' AND request_digest=request_value AND connection_reference=pkce_reference_value AND available_at=row_value.expires_at AND last_error_code='oauth_attempt_expiry';
  IF NOT FOUND THEN PERFORM zasp_connector_stage_pkce_cleanup(organization_value,workspace_value,environment_value,cleanup_value,integration_value,attempt_value,provider_value,pkce_reference_value,request_value,row_value.expires_at,'oauth_attempt_expiry'); END IF;
  PERFORM zasp_connector_begin_effect(organization_value,workspace_value,environment_value,authorize_value,integration_value,attempt_value,provider_value,'authorize','oauth-authorize:'||attempt_value,request_value);
...
LANGUAGE plpgsql AS $$
DECLARE row_value zasp_connector_oauth_attempts%ROWTYPE; effect_row zasp_connector_effects%ROWTYPE; cleanup_row zasp_connector_effects%ROWTYPE;
BEGIN
  SELECT * INTO row_value FROM zasp_connector_oauth_attempts WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND state_hash=state_value;
  IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE='P0002',MESSAGE='oauth attempt unavailable'; END IF;
  SELECT * INTO cleanup_row FROM zasp_connector_effects WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND oauth_attempt_id=row_value.id AND operation='pkce_cleanup' FOR UPDATE;
  IF NOT FOUND OR cleanup_row.status<>'unknown' OR cleanup_row.lease_expires_at>transaction_timestamp() THEN RAISE EXCEPTION USING ERRCODE='23505',MESSAGE='oauth cleanup unavailable'; END IF;
  SELECT * INTO row_value FROM zasp_connector_oauth_attempts WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND id=row_value.id AND state_hash=state_value FOR UPDATE;
  ...
  SELECT * INTO effect_row FROM zasp_connector_effects WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND oauth_attempt_id=row_value.id AND operation='authorize' FOR UPDATE;
  IF NOT FOUND OR effect_row.status NOT IN('pending','unknown') THEN RAISE EXCEPTION USING ERRCODE='23505',MESSAGE='oauth effect unavailable'; END IF;
  UPDATE zasp_connector_effects SET status='unknown',last_error_code='provider_effect_started',updated_at=transaction_timestamp() WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND id=effect_row.id RETURNING * INTO effect_row;
  ...
  PERFORM zasp_connector_audit_event(organization_value,workspace_value,environment_value,row_value.integration_id,row_value.id,effect_row.id,'effect_unknown','', 'provider_effect_started','{}'::jsonb);
  RETURN jsonb_build_object('id',row_value.id,'integration_id',row_value.integration_id,'effect_id',effect_row.id,'provider',row_value.provider,'principal_id',row_value.principal_id,'pkce_verifier_reference',row_value.pkce_verifier_reference,'request_digest',encode(row_value.request_digest,'hex'),'requested_scopes',row_value.requested_scopes,'return_path',row_value.return_path,'expires_at',row_value.expires_at,'consumed_at',row_value.consumed_at);
END $$;
...
LANGUAGE plpgsql AS $$
DECLARE row_value zasp_connector_effects%ROWTYPE; attempt_row zasp_connector_oauth_attempts%ROWTYPE; authorize_row zasp_connector_effects%ROWTYPE;
BEGIN
  ...
  IF row_value.status<>'unknown' OR owner_value<>'' AND (row_value.lease_owner IS DISTINCT FROM owner_value OR row_value.lease_token IS DISTINCT FROM token_value OR row_value.lease_expires_at IS NULL OR row_value.lease_expires_at<=transaction_timestamp()) THEN RAISE EXCEPTION USING ERRCODE='40001',MESSAGE='pkce cleanup lease unavailable'; END IF;
  IF owner_value<>'' AND row_value.oauth_attempt_id IS NOT NULL THEN
    SELECT * INTO attempt_row FROM zasp_connector_oauth_attempts WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND id=row_value.oauth_attempt_id FOR UPDATE;
    SELECT * INTO authorize_row FROM zasp_connector_effects WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND oauth_attempt_id=row_value.oauth_attempt_id AND operation='authorize' FOR UPDATE;
    IF NOT FOUND OR attempt_row.id IS NULL OR attempt_row.status='consuming' OR attempt_row.status='pending' AND authorize_row.status<>'pending' THEN RAISE EXCEPTION USING ERRCODE='40001',MESSAGE='pkce cleanup attempt unavailable'; END IF;
    UPDATE zasp_connector_oauth_attempts SET status='expired',completed_at=COALESCE(completed_at,transaction_timestamp()),updated_at=transaction_timestamp()
    WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND id=row_value.oauth_attempt_id AND status='pending';
    IF NOT FOUND AND NOT EXISTS(SELECT 1 FROM zasp_connector_oauth_attempts WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND id=row_value.oauth_attempt_id AND status IN('expired','rejected','succeeded')) THEN RAISE EXCEPTION USING ERRCODE='23505',MESSAGE='pkce cleanup attempt conflict'; END IF;
    UPDATE zasp_connector_effects SET status='failed',last_error_code='oauth_attempt_expired',resolved_at=transaction_timestamp(),updated_at=transaction_timestamp()
    WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND oauth_attempt_id=row_value.oauth_attempt_id AND operation='authorize' AND status='pending';
  END IF;
  ...
  IF owner_value<>'' AND row_value.oauth_attempt_id IS NOT NULL THEN PERFORM zasp_connector_audit_event(organization_value,workspace_value,environment_value,row_value.integration_id,row_value.oauth_attempt_id,row_value.id,'authorization_rejected','','oauth_attempt_expired','{}'::jsonb); END IF;
  RETURN jsonb_build_object('id',row_value.id,'status',row_value.status,'attempt',row_value.attempt,'updated_at',row_value.updated_at);
END $$;
...
BEGIN
  ...
  IF row_value.status=status_value AND row_value.connection_reference IS NOT DISTINCT FROM NULLIF(reference_value,'') AND row_value.metadata=metadata_value AND row_value.last_error_code IS NOT DISTINCT FROM NULLIF(error_value,'') THEN
    ...
    RETURN jsonb_build_object('id',row_value.id,'status',row_value.status,'attempt',row_value.attempt,'updated_at',row_value.updated_at);
  END IF;
  IF row_value.status NOT IN('pending','unknown') OR row_value.status='unknown' AND (row_value.last_error_code<>'provider_effect_started' OR row_value.lease_owner IS NOT NULL OR row_value.connection_reference IS NOT NULL) THEN RAISE EXCEPTION USING ERRCODE='23505',MESSAGE='connector effect already resolved'; END IF;
...
LANGUAGE plpgsql AS $$
DECLARE result_value jsonb; available_slots integer;
BEGIN
...
BEGIN
	SELECT * INTO effect_row FROM zasp_connector_effects WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND id=effect_value AND operation='authorize' FOR UPDATE;
	IF NOT FOUND OR effect_row.status<>'unknown' OR effect_row.last_error_code<>'cleanup_pending' OR effect_row.lease_owner IS DISTINCT FROM owner_value OR effect_row.lease_token IS DISTINCT FROM token_value OR effect_row.lease_expires_at IS NULL OR effect_row.lease_expires_at<=transaction_timestamp() THEN
...
BEGIN
	...
	IF NOT FOUND OR effect_row.status<>'unknown' OR effect_row.attempt<>100 OR effect_row.lease_owner IS DISTINCT FROM owner_value OR effect_row.lease_token IS DISTINCT FROM token_value OR effect_row.lease_expires_at IS NULL THEN RAISE EXCEPTION USING ERRCODE='40001',MESSAGE='connector quarantine lease unavailable'; END IF;
...
CREATE FUNCTION "public"."zasp_connector_recover_final_attempts"(owner_value text,lease_seconds integer,limit_value integer) RETURNS jsonb
LANGUAGE plpgsql AS $$
DECLARE effect_row record; token_value text; error_value text; recovered_value integer:=0;
BEGIN
	IF length(owner_value) NOT BETWEEN 3 AND 128 OR lease_seconds NOT BETWEEN 5 AND 300 OR limit_value NOT BETWEEN 1 AND 100 THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='invalid final attempt recovery'; END IF;
	PERFORM pg_advisory_xact_lock(742516322);
	FOR effect_row IN SELECT organization_id,workspace_id,environment_id,id,operation,last_error_code FROM zasp_connector_effects
	 WHERE status='unknown' AND attempt=100 AND lease_expires_at IS NOT NULL AND lease_expires_at<=transaction_timestamp()
	   AND COALESCE(last_error_code,'') NOT IN('provider_outcome_ambiguous','provider_cleanup_ambiguous','provider_revocation_ambiguous','pkce_cleanup_ambiguous')
	 ORDER BY lease_expires_at,id FOR UPDATE SKIP LOCKED LIMIT limit_value
	LOOP
		token_value:=encode(gen_random_bytes(32),'hex');
		UPDATE zasp_connector_effects SET lease_owner=owner_value,lease_token=token_value,lease_expires_at=transaction_timestamp()+make_interval(secs=>lease_seconds),updated_at=transaction_timestamp()
		 WHERE organization_id=effect_row.organization_id AND workspace_id=effect_row.workspace_id AND environment_id=effect_row.environment_id AND id=effect_row.id;
		error_value:=CASE WHEN effect_row.operation='pkce_cleanup' THEN 'pkce_cleanup_ambiguous' WHEN effect_row.operation='revoke' THEN 'provider_revocation_ambiguous' WHEN effect_row.last_error_code='cleanup_pending' THEN 'provider_cleanup_ambiguous' ELSE 'provider_outcome_ambiguous' END;
		PERFORM zasp_connector_quarantine_reconciliation(effect_row.organization_id,effect_row.workspace_id,effect_row.environment_id,effect_row.id,owner_value,token_value,error_value);
		recovered_value:=recovered_value+1;
	END LOOP;
...
DECLARE effect_row zasp_connector_effects%ROWTYPE;
BEGIN
	SELECT * INTO STRICT effect_row FROM (SELECT * FROM zasp_connector_effects WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND integration_id=integration_value AND status='unknown' AND last_error_code IN('provider_outcome_ambiguous','provider_cleanup_ambiguous','provider_revocation_ambiguous','pkce_cleanup_ambiguous') ORDER BY updated_at DESC,id LIMIT 2) active_effect;
...
BEGIN
	...
	END IF;
	IF effect_row.last_error_code='pkce_cleanup_ambiguous' AND effect_row.oauth_attempt_id IS NOT NULL THEN
		UPDATE zasp_connector_oauth_attempts SET status='expired',completed_at=COALESCE(completed_at,transaction_timestamp()),updated_at=transaction_timestamp() WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND id=effect_row.oauth_attempt_id AND status IN('pending','consuming');
		IF NOT FOUND AND NOT EXISTS(SELECT 1 FROM zasp_connector_oauth_attempts WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND id=effect_row.oauth_attempt_id AND status IN('expired','rejected','succeeded')) THEN RAISE EXCEPTION USING ERRCODE='23505',MESSAGE='pkce remediation attempt conflict'; END IF;
	END IF;
...
BEGIN
  ...
  END IF;
  IF effect_row.status<>'unknown' OR effect_row.lease_owner IS DISTINCT FROM owner_value OR effect_row.lease_token IS DISTINCT FROM token_value OR effect_row.lease_expires_at IS NULL OR effect_row.lease_expires_at<=transaction_timestamp() THEN RAISE EXCEPTION USING ERRCODE='40001',MESSAGE='connector reconciliation lease unavailable'; END IF;
  ...
  UPDATE zasp_connector_effects SET lease_owner=owner_value,lease_token=token_value,lease_expires_at=effect_row.lease_expires_at WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND id=effect_value AND status='unknown' AND last_error_code='cleanup_pending';
  IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE='40001',MESSAGE='connector reconciliation cleanup lease unavailable'; END IF;
  RETURN result_value;
END $$;
...
BEGIN
  ...
  SELECT * INTO effect_row FROM zasp_connector_effects WHERE organization_id=organization_value AND workspace_id=workspace_value AND environment_id=environment_value AND id=effect_value FOR UPDATE;
  IF NOT FOUND OR effect_row.status<>'unknown' OR effect_row.lease_owner IS DISTINCT FROM owner_value OR effect_row.lease_token IS DISTINCT FROM token_value OR effect_row.lease_expires_at IS NULL OR effect_row.lease_expires_at<=transaction_timestamp() THEN RAISE EXCEPTION USING ERRCODE='40001',MESSAGE='connector reconciliation lease unavailable'; END IF;
...
BEGIN
  ...
  END IF;
  IF effect_row.status<>'unknown' OR effect_row.lease_owner IS DISTINCT FROM owner_value OR effect_row.lease_token IS DISTINCT FROM token_value OR effect_row.lease_expires_at IS NULL OR effect_row.lease_expires_at<=transaction_timestamp() THEN RAISE EXCEPTION USING ERRCODE='40001',MESSAGE='connector revocation lease unavailable'; END IF;
...
CREATE FUNCTION "public"."zasp_connector_security_ready"() RETURNS boolean LANGUAGE sql STABLE AS $$
 WITH expected(api_functions,worker_functions) AS (SELECT
   ARRAY(SELECT value FROM unnest(ARRAY[
     'zasp_connector_activate_pkce_cleanup(text,text,text,text)',
     'zasp_connector_claim_reconciliation(text,integer,integer)',
     'zasp_connector_complete_cleanup(text,text,text,text)',
     'zasp_connector_complete_oauth(text,text,text,text,text,text,text,text,text,text,jsonb,bytea)',
     'zasp_connector_complete_pkce_cleanup(text,text,text,text,text,text)',
     'zasp_connector_complete_reconciliation(text,text,text,text,text,text,text,text,text,text,text,text,jsonb,bytea)',
     'zasp_connector_complete_revocation(text,text,text,text,text,text)',
     'zasp_connector_consume_oauth(text,text,text,bytea,text,bytea)',
     'zasp_connector_fail_reconciliation(text,text,text,text,text,text,text)',
     'zasp_connector_get_quarantine(text,text,text,text)',
     'zasp_connector_quarantine_reconciliation(text,text,text,text,text,text,text)',
     'zasp_connector_recover_final_attempts(text,integer,integer)',
     'zasp_connector_readiness(text,text)',
     'zasp_connector_remediate_quarantine(text,text,text,text,text,text,text,text,bigint,jsonb,jsonb,text,text,text)',
     'zasp_connector_resolve_effect(text,text,text,text,text,text,jsonb,text)',
     'zasp_connector_security_ready()',
     'zasp_connector_stage_pkce_cleanup(text,text,text,text,text,text,text,text,bytea,timestamp with time zone,text)',
     'zasp_connector_start_oauth(text,text,text,text,text,text,text,bytea,bytea,text,bytea,jsonb,timestamp with time zone,bigint,jsonb,text,text)',
     'zasp_connector_workflow_mutate(text,text,text,text,text,text,text,text,text,bigint,jsonb,jsonb,text,text,text)'
   ]::text[]) value ORDER BY value),
   ARRAY(SELECT value FROM unnest(ARRAY[
     'zasp_connector_claim_reconciliation(text,integer,integer)',
     'zasp_connector_complete_cleanup(text,text,text,text,text,text)',
     'zasp_connector_complete_pkce_cleanup(text,text,text,text,text,text)',
     'zasp_connector_complete_reconciliation(text,text,text,text,text,text,text,text,text,text,text,text,jsonb,bytea)',
     'zasp_connector_complete_revocation(text,text,text,text,text,text)',
     'zasp_connector_fail_reconciliation(text,text,text,text,text,text,text)',
 ...
 AND (SELECT COALESCE(array_agg(t.tgname||':'||t.tgenabled::text||':'||t.tgfoid::regprocedure::text ORDER BY t.tgname),'{}'::text[]) FROM pg_trigger t JOIN pg_class c ON c.oid=t.tgrelid JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='zasp_connector_effects' AND NOT t.tgisinternal)=ARRAY['zasp_connector_effect_lanes_delete:O:zasp_connector_effect_lanes_delete()','zasp_connector_effect_lanes_insert:O:zasp_connector_effect_lanes_insert()','zasp_connector_effect_lanes_update:O:zasp_connector_effect_lanes_update()']::text[]
...
DO $authority$ DECLARE procedure_oid oid; BEGIN
  ...
  END LOOP;
END $authority$;
GRANT EXECUTE ON FUNCTION zasp_connector_security_ready(),zasp_connector_readiness(text,text),zasp_connector_workflow_mutate(text,text,text,text,text,text,text,text,text,bigint,jsonb,jsonb,text,text,text),zasp_connector_start_oauth(text,text,text,text,text,text,text,bytea,bytea,text,bytea,jsonb,timestamptz,bigint,jsonb,text,text),zasp_connector_consume_oauth(text,text,text,bytea,text,bytea),zasp_connector_stage_pkce_cleanup(text,text,text,text,text,text,text,text,bytea,timestamptz,text),zasp_connector_activate_pkce_cleanup(text,text,text,text),zasp_connector_complete_pkce_cleanup(text,text,text,text,text,text),zasp_connector_resolve_effect(text,text,text,text,text,text,jsonb,text),zasp_connector_complete_oauth(text,text,text,text,text,text,text,text,text,text,jsonb,bytea),zasp_connector_complete_cleanup(text,text,text,text),zasp_connector_claim_reconciliation(text,integer,integer),zasp_connector_recover_final_attempts(text,integer,integer),zasp_connector_complete_reconciliation(text,text,text,text,text,text,text,text,text,text,text,text,jsonb,bytea),zasp_connector_quarantine_reconciliation(text,text,text,text,text,text,text),zasp_connector_get_quarantine(text,text,text,text),zasp_connector_remediate_quarantine(text,text,text,text,text,text,text,text,bigint,jsonb,jsonb,text,text,text),zasp_connector_fail_reconciliation(text,text,text,text,text,text,text),zasp_connector_complete_revocation(text,text,text,text,text,text) TO zasp_discovery_api;
GRANT EXECUTE ON FUNCTION zasp_connector_security_ready(),zasp_connector_readiness(text,text),zasp_connector_claim_reconciliation(text,integer,integer),zasp_connector_recover_final_attempts(text,integer,integer),zasp_connector_complete_pkce_cleanup(text,text,text,text,text,text),zasp_connector_complete_cleanup(text,text,text,text,text,text),zasp_connector_complete_reconciliation(text,text,text,text,text,text,text,text,text,text,text,text,jsonb,bytea),zasp_connector_quarantine_reconciliation(text,text,text,text,text,text,text),zasp_connector_fail_reconciliation(text,text,text,text,text,text,text),zasp_connector_complete_revocation(text,text,text,text,text,text) TO zasp_discovery_worker;
...
INSERT INTO zasp_schema_metadata(key,value) VALUES ('connector_authorization_fingerprint', '9bc53b82427248d1679223e9625eda9e77f5b3da2d50a7b0806618780e435be8');
