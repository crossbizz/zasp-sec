...
BEGIN
  IF octet_length(input_digest_value)<>32 OR octet_length(output_digest_value)<>32 OR length(model_value) NOT BETWEEN 1 AND 128 OR length(policy_version_value) NOT BETWEEN 1 AND 64 OR jsonb_typeof(candidate_value)<>'object' OR (SELECT array_agg(key ORDER BY key) FROM jsonb_object_keys(candidate_value) key)<>ARRAY['steps','summary','version']::text[] OR candidate_value->>'version'<>'1' OR jsonb_typeof(candidate_value->'steps')<>'array' OR jsonb_array_length(candidate_value->'steps')<>1 OR jsonb_typeof(candidate_value->'steps'->0)<>'object' OR (SELECT array_agg(key ORDER BY key) FROM jsonb_object_keys(candidate_value->'steps'->0) key)<>ARRAY['action','index','target_id']::text[] OR candidate_value->'steps'->0->>'index'<>'0' THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='security agent planner candidate rejected';END IF;
  ...
  IF expected_input<>input_digest_value OR candidate_value->'steps'->0->>'action'<>context_value->'allowed_actions'->>0 OR candidate_value->'steps'->0->>'target_id'<>context_value->'allowed_targets'->>0 THEN RAISE EXCEPTION USING ERRCODE='40001',MESSAGE='security agent planner authority changed';END IF;
...
BEGIN
  IF octet_length(input_digest_value)<>32 OR output_digest_value IS NOT NULL AND octet_length(output_digest_value)<>32 OR error_code_value NOT IN('planner_unavailable','planner_rejected') OR error_code_value='planner_rejected' AND output_digest_value IS NULL OR length(model_value) NOT BETWEEN 1 AND 128 OR length(policy_version_value) NOT BETWEEN 1 AND 64 OR NOT zasp_valid_product_id(audit_value) OR NOT zasp_valid_product_id(correlation_value) THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='security agent planner failure rejected';END IF;
...
INSERT INTO public.zasp_schema_metadata(key,value) VALUES('production_security_agent_planner_fingerprint', '08cc739df90088092ed3f1a0535843c54c3d5115570bf3f95314ae39dfaeed58') ON CONFLICT(key) DO UPDATE SET value=excluded.value;
