...
CREATE TABLE public.zasp_recovery_restores(
 ...
 actor_id text NOT NULL,idempotency_key text NOT NULL,request_digest bytea NOT NULL,target_environment text NOT NULL CHECK(target_environment='disposable'),manifest jsonb NOT NULL,manifest_digest bytea NOT NULL,
...
BEGIN
 ...
 result_value:=jsonb_build_object('audit_id',audit_value,'body',jsonb_build_object('id',backup_value,'version',1,'state','queued','retention_days',retention_value,'created_at',transaction_timestamp()),'correlation_id',correlation_value,'receipt_id',receipt_value,'replayed',false);
...
CREATE FUNCTION public.zasp_recovery_create_restore(organization_value text,workspace_value text,environment_value text,actor_value text,idempotency_value text,restore_value text,correlation_value text,audit_value text,receipt_value text,request_digest_value bytea,manifest_value jsonb,manifest_digest_value bytea) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO pg_catalog, public AS $create$
DECLARE receipt_row zasp_recovery_request_receipts%ROWTYPE;result_value jsonb;outbox_value text;payload_value jsonb;
BEGIN
 IF NOT zasp_discovery_principal_ready('zasp_discovery_api') OR NOT zasp_valid_product_id(organization_value) OR NOT zasp_valid_product_id(workspace_value) OR NOT zasp_valid_product_id(environment_value) OR NOT zasp_valid_product_id(actor_value) OR NOT zasp_valid_product_id(restore_value) OR NOT zasp_valid_product_id(correlation_value) OR NOT zasp_valid_product_id(audit_value) OR NOT zasp_valid_product_id(receipt_value) OR length(idempotency_value) NOT BETWEEN 16 AND 128 OR octet_length(request_digest_value)<>32 OR octet_length(manifest_digest_value)<>32 OR jsonb_typeof(manifest_value)<>'object' OR octet_length(manifest_value::text)>65536
  OR (SELECT count(*) FROM jsonb_object_keys(manifest_value))<>4 OR manifest_value->>'schema_version'<>'recovery_signed_manifest_v1'
  OR jsonb_typeof(manifest_value->'signing_key_arn')<>'string' OR manifest_value->>'signing_key_arn'!~'^arn:(aws|aws-us-gov|aws-cn):kms:[a-z0-9-]{3,32}:[0-9]{12}:key/[0-9a-f-]{36}$'
  OR jsonb_typeof(manifest_value->'payload')<>'string' OR length(manifest_value->>'payload') NOT BETWEEN 32 AND 76460 OR jsonb_typeof(manifest_value->'signature')<>'string' OR length(manifest_value->>'signature') NOT BETWEEN 43 AND 684
 ...
 INSERT INTO zasp_recovery_restores(organization_id,workspace_id,environment_id,restore_id,actor_id,idempotency_key,request_digest,target_environment,manifest,manifest_digest) VALUES(organization_value,workspace_value,environment_value,restore_value,actor_value,idempotency_value,request_digest_value,'disposable',manifest_value,manifest_digest_value);
 outbox_value:=zasp_discovery_canonical_id(organization_value,workspace_value,environment_value,'recovery_restore_outbox',restore_value);payload_value:=jsonb_build_object('environment_id',environment_value,'organization_id',organization_value,'restore_id',restore_value,'workspace_id',workspace_value);
 ...
 result_value:=jsonb_build_object('audit_id',audit_value,'body',jsonb_build_object('id',restore_value,'version',1,'state','queued','target_environment','disposable','created_at',transaction_timestamp()),'correlation_id',correlation_value,'receipt_id',receipt_value,'replayed',false);
...
CREATE FUNCTION public.zasp_recovery_get_restore(organization_value text,workspace_value text,environment_value text,restore_value text) RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO pg_catalog, public AS $get$
 SELECT jsonb_strip_nulls(jsonb_build_object('id',restore_id,'version',version,'state',state,'target_environment',target_environment,'attempt',attempt,'observed_counts',observed_counts,'validation_evidence',validation_evidence,'cleanup_evidence',cleanup_evidence,'error_code',error_code,'created_at',created_at,'started_at',started_at,'completed_at',completed_at)) FROM zasp_recovery_restores WHERE zasp_discovery_principal_ready('zasp_discovery_api') AND (organization_id,workspace_id,environment_id,restore_id)=(organization_value,workspace_value,environment_value,restore_value)
$get$;
...
BEGIN
 ...
 ELSE
  WITH ranked AS (SELECT operation.*,row_number() OVER(PARTITION BY organization_id ORDER BY available_at,created_at,restore_id) AS organization_rank FROM zasp_recovery_restores operation WHERE state IN('queued','retryable','verifying','provisioning','validating','rebuilding','cleanup_required','cleaning') AND available_at<=transaction_timestamp() AND (state NOT IN('verifying','provisioning','validating','rebuilding','cleanup_required','cleaning') OR lease_expires_at<=transaction_timestamp()) AND NOT EXISTS(SELECT 1 FROM zasp_recovery_backups live WHERE live.organization_id=operation.organization_id AND live.state IN('draining','capturing','publishing') AND live.lease_expires_at>transaction_timestamp()) AND NOT EXISTS(SELECT 1 FROM zasp_recovery_restores live WHERE live.organization_id=operation.organization_id AND live.state IN('verifying','provisioning','validating','rebuilding','cleanup_required','cleaning') AND live.lease_expires_at>transaction_timestamp())),
...
BEGIN
 IF NOT zasp_recovery_principal_ready('zasp_recovery_worker') OR jsonb_typeof(manifest_value)<>'object' OR octet_length(manifest_value::text)>65536 OR octet_length(token_value)<>32
  OR (SELECT count(*) FROM jsonb_object_keys(manifest_value))<>4 OR manifest_value->>'schema_version'<>'recovery_signed_manifest_v1'
  OR jsonb_typeof(manifest_value->'signing_key_arn')<>'string' OR manifest_value->>'signing_key_arn'!~'^arn:(aws|aws-us-gov|aws-cn):kms:[a-z0-9-]{3,32}:[0-9]{12}:key/[0-9a-f-]{36}$'
  OR jsonb_typeof(manifest_value->'payload')<>'string' OR length(manifest_value->>'payload') NOT BETWEEN 32 AND 76460 OR jsonb_typeof(manifest_value->'signature')<>'string' OR length(manifest_value->>'signature') NOT BETWEEN 43 AND 684 THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='recovery backup finish rejected';END IF;requested_value:=digest(convert_to(manifest_value::text,'UTF8'),'sha256');
 SELECT * INTO STRICT row_value FROM zasp_recovery_backups WHERE (organization_id,workspace_id,environment_id,backup_id)=(organization_value,workspace_value,environment_value,backup_value) FOR UPDATE;
...
BEGIN
 IF NOT zasp_recovery_principal_ready('zasp_recovery_worker') OR octet_length(token_value)<>32 OR jsonb_typeof(observed_value)<>'object' OR jsonb_typeof(validation_value)<>'object' OR jsonb_typeof(cleanup_value)<>'object' OR cleanup_value->>'state'<>'deleted' THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='recovery restore finish rejected';END IF;
...
BEGIN
 FOREACH function_value IN ARRAY ARRAY[
  ...
  'public.zasp_recovery_create_backup(text,text,text,text,text,text,text,integer,text,text,bytea)'::regprocedure,'public.zasp_recovery_create_restore(text,text,text,text,text,text,text,text,text,bytea,jsonb,bytea)'::regprocedure,'public.zasp_recovery_get_backup(text,text,text,text)'::regprocedure,'public.zasp_recovery_get_restore(text,text,text,text)'::regprocedure,
...
REVOKE ALL ON FUNCTION public.zasp_recovery_execution_security_ready(),public.zasp_recovery_execution_live_fingerprint(),public.zasp_recovery_execution_readiness(text,text) FROM PUBLIC,zasp_discovery_api,zasp_recovery_worker,zasp_recovery_outbox_worker;
GRANT EXECUTE ON FUNCTION public.zasp_recovery_execution_readiness(text,text) TO zasp_discovery_api,zasp_recovery_worker,zasp_recovery_outbox_worker;
...
INSERT INTO public.zasp_schema_metadata(key,value) VALUES('production_recovery_fingerprint', 'a0f8b693d4ee170a9d6fc17b554596debeef7f9c4f88149a2dfcfca32fb8f650') ON CONFLICT(key) DO UPDATE SET value=excluded.value;
