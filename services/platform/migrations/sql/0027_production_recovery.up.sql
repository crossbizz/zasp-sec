...
BEGIN
 ...
 IF kind_value='backup' THEN
  ...
  UPDATE zasp_recovery_backups SET state='draining',attempt=attempt+1,worker_id=worker_value,lease_token=token_value,lease_expires_at=transaction_timestamp()+make_interval(secs=>lease_seconds),started_at=COALESCE(started_at,transaction_timestamp()),updated_at=transaction_timestamp() WHERE (organization_id,workspace_id,environment_id,backup_id)=(organization_value,workspace_value,environment_value,operation_value) RETURNING jsonb_build_object('attempt',attempt,'backup_id',backup_id,'created_at',created_at,'environment_id',environment_id,'lease_expires_at',lease_expires_at,'organization_id',organization_id,'request_digest',request_digest,'retention_days',retention_days,'workspace_id',workspace_id) INTO STRICT operation_json;
 ELSE
  ...
  IF restore_value.attempt>=100 THEN
   UPDATE zasp_recovery_restores SET state=CASE WHEN restore_value.state IN('cleanup_required','cleaning') THEN 'failed_cleanup' ELSE 'failed' END,error_code=CASE WHEN restore_value.state IN('cleanup_required','cleaning') THEN 'cleanup_failed' ELSE 'exhausted' END,completed_at=transaction_timestamp(),worker_id=NULL,lease_token=NULL,lease_expires_at=NULL,updated_at=transaction_timestamp() WHERE (organization_id,workspace_id,environment_id,restore_id)=(organization_value,workspace_value,environment_value,operation_value);
   RETURN jsonb_build_object('disposition','exhausted');
  END IF;
  IF EXISTS(SELECT 1 FROM zasp_recovery_backups live WHERE live.organization_id=organization_value AND live.state IN('draining','capturing','publishing') AND live.lease_expires_at>transaction_timestamp()) OR EXISTS(SELECT 1 FROM zasp_recovery_restores live WHERE live.organization_id=organization_value AND (live.workspace_id,live.environment_id,live.restore_id)<>(workspace_value,environment_value,operation_value) AND live.state IN('verifying','provisioning','validating','rebuilding','cleanup_required','cleaning') AND live.lease_expires_at>transaction_timestamp()) THEN RETURN jsonb_build_object('disposition','retry_later');END IF;
  UPDATE zasp_recovery_restores SET state='verifying',attempt=attempt+1,worker_id=worker_value,lease_token=token_value,lease_expires_at=transaction_timestamp()+make_interval(secs=>lease_seconds),started_at=COALESCE(started_at,transaction_timestamp()),updated_at=transaction_timestamp() WHERE (organization_id,workspace_id,environment_id,restore_id)=(organization_value,workspace_value,environment_value,operation_value) RETURNING jsonb_build_object('attempt',attempt,'created_at',created_at,'environment_id',environment_id,'lease_expires_at',lease_expires_at,'manifest',manifest,'manifest_digest',manifest_digest,'organization_id',organization_id,'request_digest',request_digest,'restore_id',restore_id,'target_environment',target_environment,'workspace_id',workspace_id) INTO STRICT operation_json;
 END IF;
...
INSERT INTO public.zasp_schema_metadata(key,value) VALUES('production_recovery_fingerprint', '09ecdd13131f9b08f4ede532260c9cf7499cf087255d846c8db2bea976b51c62') ON CONFLICT(key) DO UPDATE SET value=excluded.value;
