...
BEGIN
 IF NOT zasp_discovery_principal_ready('zasp_discovery_api') OR NOT zasp_valid_product_id(organization_value) OR NOT zasp_valid_product_id(workspace_value) OR NOT zasp_valid_product_id(environment_value) OR NOT zasp_valid_product_id(actor_value) OR NOT zasp_valid_product_id(restore_value) OR NOT zasp_valid_product_id(correlation_value) OR NOT zasp_valid_product_id(audit_value) OR NOT zasp_valid_product_id(receipt_value) OR length(idempotency_value) NOT BETWEEN 16 AND 128 OR octet_length(request_digest_value)<>32 OR octet_length(manifest_digest_value)<>32 OR jsonb_typeof(manifest_value)<>'object' OR octet_length(manifest_value::text)>8192
  OR (SELECT count(*) FROM jsonb_object_keys(manifest_value))<>8 OR NOT zasp_discovery_s3_object_reference(manifest_value->>'reference') OR length(manifest_value->>'version_id') NOT BETWEEN 1 AND 1024
...
INSERT INTO public.zasp_schema_metadata(key,value) VALUES('production_recovery_fingerprint', 'a96223374b417a3e09f46f9dc1ef20066b4929ffd4c6cb553f8ea96950d64e5c') ON CONFLICT(key) DO UPDATE SET value=excluded.value;
