...
BEGIN
  FOREACH table_name IN ARRAY ARRAY['zasp_identity_administration_state','zasp_identity_provider_connections','zasp_identity_provider_mutations','zasp_identity_secret_reveal_grants','zasp_identity_webhook_events'] LOOP
...
GRANT UPDATE ON public.zasp_product_sessions,public.zasp_product_api_tokens TO zasp_discovery_authority;
...
CREATE FUNCTION public.zasp_identity_admin_reconcile_deprovision(project_value text,event_value text,organization_value text,member_value text,event_digest_value bytea,audit_value text) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO pg_catalog, public AS $deprovision$
DECLARE existing zasp_identity_webhook_events%ROWTYPE;session_count integer;token_count integer;principal_value text;workspace_value text;environment_value text;
BEGIN
  IF length(project_value) NOT BETWEEN 8 AND 128 OR event_value!~'^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' OR length(member_value) NOT BETWEEN 8 AND 128 OR octet_length(event_digest_value)<>32 OR NOT zasp_valid_product_id(audit_value) THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='identity deprovision rejected';END IF;
  ...
  END IF;
  SELECT membership.principal_id INTO principal_value FROM zasp_identity_memberships membership WHERE membership.organization_id=organization_value AND membership.member_reference=member_value AND membership.active FOR UPDATE;
...
CREATE FUNCTION public.zasp_identity_admin_security_ready() RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO pg_catalog, public AS $security$
  SELECT
    ...
    AND has_table_privilege('zasp_discovery_authority','public.zasp_product_sessions','UPDATE')
    AND has_table_privilege('zasp_discovery_authority','public.zasp_product_api_tokens','UPDATE')
    ...
    AND NOT EXISTS(
      ...
      WHERE namespace.nspname='public' AND class.relname IN('zasp_identity_administration_state','zasp_identity_provider_connections','zasp_identity_provider_mutations','zasp_identity_secret_reveal_grants','zasp_identity_webhook_events')
...
CREATE FUNCTION public.zasp_identity_administration_readiness(expected_checksum text,expected_fingerprint text) RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO pg_catalog, public AS $readiness$
  SELECT length(expected_checksum)=64 AND expected_checksum~'^[a-f0-9]{64}$' AND length(expected_fingerprint)=64 AND expected_fingerprint~'^[a-f0-9]{64}$'
    ...
    AND (SELECT count(*) FROM pg_class class JOIN pg_namespace namespace ON namespace.oid=class.relnamespace WHERE namespace.nspname='public' AND class.relname IN('zasp_identity_administration_state','zasp_identity_provider_connections','zasp_identity_provider_mutations','zasp_identity_secret_reveal_grants','zasp_identity_webhook_events') AND class.relkind='r' AND class.relowner=(SELECT oid FROM pg_roles WHERE rolname='zasp_discovery_authority') AND class.relrowsecurity AND class.relforcerowsecurity)=5
