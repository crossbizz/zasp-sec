...
DO $roles$ DECLARE role_name text;BEGIN
  FOREACH role_name IN ARRAY ARRAY['zasp_discovery_scheduler','zasp_projection_risk_worker','zasp_projection_graph_worker','zasp_projection_search_worker'] LOOP
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname=role_name) THEN
      ...
      EXECUTE format('COMMENT ON ROLE %I IS %L',role_name,'zasp-managed:production-discovery-execution-v1');
    END IF;
  END LOOP;
...
CREATE INDEX zasp_execution_projection_claim_idx ON zasp_projection_work(available_at,organization_id,snapshot_id,kind,version) WHERE state IN('pending','retryable','leased') AND attempt<100;
...
BEGIN
 ...
 IF FOUND THEN
   SELECT * INTO STRICT input_row FROM zasp_discovery_snapshot_inputs WHERE (organization_id,workspace_id,environment_id,integration_id,source,snapshot_id)=(organization_value,workspace_value,environment_value,integration_value,snapshot_row.source,snapshot_row.id);
...
BEGIN
 ...
 WITH organizations AS (SELECT projection.organization_id,min(projection.snapshot_id) first_snapshot FROM zasp_projection_work projection WHERE projection.kind=kind_value AND projection.attempt<5 AND projection.available_at<=transaction_timestamp() AND (projection.state IN('pending','retryable') OR projection.state='leased' AND projection.lease_expires_at<=transaction_timestamp()) GROUP BY projection.organization_id ORDER BY first_snapshot,projection.organization_id LIMIT claim_limit),
 picked AS (SELECT chosen.ctid,chosen.organization_id,chosen.snapshot_id,chosen.kind FROM organizations organization CROSS JOIN LATERAL (SELECT projection.ctid,projection.organization_id,projection.snapshot_id,projection.kind FROM zasp_projection_work projection WHERE projection.organization_id=organization.organization_id AND projection.kind=kind_value AND projection.attempt<5 AND projection.available_at<=transaction_timestamp() AND (projection.state IN('pending','retryable') OR projection.state='leased' AND projection.lease_expires_at<=transaction_timestamp()) ORDER BY projection.snapshot_id LIMIT 1 FOR UPDATE SKIP LOCKED) chosen),
...
INSERT INTO zasp_schema_metadata(key,value) VALUES('production_discovery_execution_fingerprint', '96ad9af4cc57b7f30517d70708d3c2675e1089cd99e05dbc50a4318d85637533');
