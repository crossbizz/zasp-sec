...
CREATE TABLE public.zasp_discovery_risk_projection_current (
  organization_id text NOT NULL, workspace_id text NOT NULL, environment_id text NOT NULL,
  integration_id text NOT NULL, source text NOT NULL CHECK(source IN('aws','kubernetes','github','okta')),
  snapshot_id text NOT NULL, generation bigint NOT NULL CHECK(generation>0),
  input_digest bytea NOT NULL CHECK(octet_length(input_digest)=32), content_digest bytea NOT NULL CHECK(octet_length(content_digest)=32),
  item_count integer NOT NULL CHECK(item_count BETWEEN 0 AND 4000), updated_at timestamptz NOT NULL DEFAULT transaction_timestamp(),
  PRIMARY KEY(organization_id,workspace_id,environment_id,integration_id,source),
  FOREIGN KEY(organization_id,workspace_id,environment_id,integration_id,source,snapshot_id)
    REFERENCES zasp_discovery_snapshot_inputs(organization_id,workspace_id,environment_id,integration_id,source,snapshot_id)
);

CREATE TABLE public.zasp_discovery_risk_projection_items (
  organization_id text NOT NULL, workspace_id text NOT NULL, environment_id text NOT NULL,
  integration_id text NOT NULL, source text NOT NULL, snapshot_id text NOT NULL, generation bigint NOT NULL CHECK(generation>0),
  input_digest bytea NOT NULL CHECK(octet_length(input_digest)=32), content_digest bytea NOT NULL CHECK(octet_length(content_digest)=32),
  section text NOT NULL CHECK(section IN('entities','relationships','evidence')), item_id text NOT NULL CHECK(zasp_valid_product_id(item_id)),
  payload jsonb NOT NULL CHECK(jsonb_typeof(payload)='object' AND octet_length(payload::text)<=1048576 AND payload->>'id'=item_id),
  PRIMARY KEY(organization_id,workspace_id,environment_id,integration_id,source,section,item_id),
  FOREIGN KEY(organization_id,workspace_id,environment_id,integration_id,source)
    REFERENCES zasp_discovery_risk_projection_current(organization_id,workspace_id,environment_id,integration_id,source) ON DELETE CASCADE,
  FOREIGN KEY(organization_id,workspace_id,environment_id,integration_id,source,snapshot_id)
    REFERENCES zasp_discovery_snapshot_inputs(organization_id,workspace_id,environment_id,integration_id,source,snapshot_id)
);
...
CREATE FUNCTION public.zasp_execution_apply_risk_projection(organization_value text,workspace_value text,environment_value text,snapshot_value text,version_value text,worker_value text,lease_token_value text,integration_value text,source_value text,generation_value bigint,input_digest_value bytea,items_value jsonb) RETURNS jsonb LANGUAGE plpgsql AS $$
DECLARE input_row zasp_discovery_snapshot_inputs%ROWTYPE;current_row zasp_discovery_risk_projection_current%ROWTYPE;canonical_items jsonb;stored_items jsonb;content_digest_value bytea;item_count_value integer;replayed_value boolean:=false;
BEGIN
...
CREATE FUNCTION public.zasp_execution_security_ready() RETURNS boolean LANGUAGE sql STABLE AS $$
 ...
 AND NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname LIKE 'zasp_execution_%' AND (
   ...
   OR has_function_privilege('zasp_projection_risk_worker',p.oid,'EXECUTE')<>(p.proname=ANY(ARRAY['zasp_execution_claim_projection_work','zasp_execution_heartbeat_projection','zasp_execution_snapshot_projection_page','zasp_execution_projection_status','zasp_execution_apply_risk_projection','zasp_execution_finish_projection','zasp_execution_principal_ready','zasp_execution_readiness','zasp_execution_security_ready']))
   ...
   OR has_function_privilege('zasp_outbox_worker',p.oid,'EXECUTE')<>(p.proname=ANY(ARRAY['zasp_execution_claim_outbox','zasp_execution_heartbeat_outbox','zasp_execution_ack_outbox','zasp_execution_retry_outbox','zasp_execution_readiness']))))
...
GRANT EXECUTE ON FUNCTION zasp_execution_apply_risk_projection(text,text,text,text,text,text,text,text,text,bigint,bytea,jsonb) TO zasp_projection_risk_worker;
GRANT EXECUTE ON FUNCTION zasp_execution_claim_outbox(text,text,text,integer,integer),zasp_execution_heartbeat_outbox(text,text,text,integer,integer),zasp_execution_ack_outbox(text,text,text,text,text,text,text,text),zasp_execution_retry_outbox(text,text,text,text,text,text,text,integer,text),zasp_execution_readiness(text,text) TO zasp_outbox_worker;
