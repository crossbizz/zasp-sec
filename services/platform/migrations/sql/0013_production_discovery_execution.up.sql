...
BEGIN
 ...
 WITH organizations AS (
  SELECT organization_id,min(available_at) due FROM zasp_discovery_outbox
  WHERE topic=topic_value AND attempt<100 AND (state IN('pending','failed') OR state='leased' AND lease_expires_at<=transaction_timestamp()) AND available_at<=transaction_timestamp()
  GROUP BY organization_id ORDER BY due,organization_id LIMIT claim_limit
 ),picked AS (
  SELECT chosen.ctid,chosen.organization_id,chosen.created_at,chosen.id FROM organizations organization
  CROSS JOIN LATERAL (
  ...
  FROM (SELECT * FROM picked ORDER BY created_at,organization_id,id LIMIT claim_limit) candidate WHERE outbox.ctid=candidate.ctid
  RETURNING outbox.organization_id,outbox.workspace_id,outbox.environment_id,outbox.id,outbox.topic,outbox.deterministic_key,outbox.payload_version,outbox.payload,encode(outbox.payload_digest,'base64') payload_digest,outbox.attempt,outbox.lease_expires_at
 )
 SELECT jsonb_build_object('items',COALESCE(jsonb_agg(to_jsonb(claimed) ORDER BY organization_id,id),'[]'::jsonb)) INTO response FROM claimed;
 RETURN response;
END $$;
...
CREATE FUNCTION public.zasp_execution_security_ready() RETURNS boolean LANGUAGE sql STABLE AS $$
 ...
 AND NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname LIKE 'zasp_execution_%' AND (
   ...
   OR has_function_privilege('zasp_outbox_worker',p.oid,'EXECUTE')<>(p.proname='zasp_execution_claim_outbox')))
...
GRANT EXECUTE ON FUNCTION zasp_execution_claim_outbox(text,text,text,integer,integer) TO zasp_outbox_worker;
...
DO $legacy_outbox_split$ DECLARE definition text;BEGIN
 SELECT pg_get_functiondef('public.zasp_discovery_security_ready()'::regprocedure) INTO definition;
 definition:=regexp_replace(definition,',\s*''zasp_discovery_claim_outbox''(?:::text)?','','g');
 EXECUTE definition;
END $legacy_outbox_split$;
REVOKE EXECUTE ON FUNCTION zasp_discovery_claim_outbox(text,text,integer,integer) FROM zasp_outbox_worker;
...
INSERT INTO zasp_schema_metadata(key,value) VALUES('production_discovery_execution_fingerprint', 'c01c2fcd65d68a781ee4e1d54dac5b63698328ea6128e37d4f073ead2b620622');
