DO $rollback_guard$ BEGIN
 IF zasp_execution_live_fingerprint()<>'c01c2fcd65d68a781ee4e1d54dac5b63698328ea6128e37d4f073ead2b620622' OR NOT zasp_execution_security_ready() THEN RAISE EXCEPTION USING ERRCODE='55000',MESSAGE='semantic schema drift blocks rollback';END IF;
END $rollback_guard$;
...
DO $legacy_outbox_restore$ DECLARE definition text;BEGIN
 SELECT pg_get_functiondef('public.zasp_discovery_security_ready()'::regprocedure) INTO definition;
 definition:=replace(definition,'''zasp_discovery_principal_ready'',''zasp_discovery_ack_outbox''','''zasp_discovery_principal_ready'',''zasp_discovery_claim_outbox'',''zasp_discovery_ack_outbox''');
 IF definition NOT LIKE '%''zasp_discovery_claim_outbox''%' THEN RAISE EXCEPTION USING ERRCODE='55000',MESSAGE='execution outbox authority shape changed';END IF;
 EXECUTE definition;
END $legacy_outbox_restore$;
GRANT EXECUTE ON FUNCTION zasp_discovery_claim_outbox(text,text,integer,integer) TO zasp_outbox_worker;
