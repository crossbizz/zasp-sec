DO $rollback_guard$ BEGIN
 IF zasp_execution_live_fingerprint()<>'96ad9af4cc57b7f30517d70708d3c2675e1089cd99e05dbc50a4318d85637533' OR NOT zasp_execution_security_ready() THEN RAISE EXCEPTION USING ERRCODE='55000',MESSAGE='semantic schema drift blocks rollback';END IF;
END $rollback_guard$;
...
DO $managed_role_cleanup$ DECLARE role_name text;role_oid oid;BEGIN
 FOREACH role_name IN ARRAY ARRAY['zasp_discovery_scheduler','zasp_projection_risk_worker','zasp_projection_graph_worker','zasp_projection_search_worker'] LOOP
  SELECT oid INTO role_oid FROM pg_roles WHERE rolname=role_name AND shobj_description(oid,'pg_authid')='zasp-managed:production-discovery-execution-v1';
  IF FOUND AND NOT EXISTS(SELECT 1 FROM pg_auth_members WHERE roleid=role_oid OR member=role_oid) AND NOT EXISTS(SELECT 1 FROM pg_shdepend WHERE refclassid='pg_authid'::regclass AND refobjid=role_oid) THEN EXECUTE format('DROP ROLE %I',role_name);END IF;
 END LOOP;
