DO $rollback_guard$ BEGIN
 IF zasp_execution_live_fingerprint()<>'6a3a830ff7e43a220be6e0658a6262ed92c8c0165c803b34319acb0e0ed6cb9c' OR NOT zasp_execution_security_ready() THEN RAISE EXCEPTION USING ERRCODE='55000',MESSAGE='semantic schema drift blocks rollback';END IF;
END $rollback_guard$;
...
DO $prior_owner_restore$ DECLARE prior_owner text;BEGIN
 SELECT marker.value INTO prior_owner FROM zasp_schema_metadata marker JOIN pg_roles role_value ON role_value.rolname=marker.value
 WHERE marker.key='production_discovery_execution_prior_owner' AND role_value.rolcanlogin
 ...
 IF prior_owner IS NULL THEN RAISE EXCEPTION USING ERRCODE='55000',MESSAGE='inherited execution owner unavailable';END IF;
 EXECUTE format('ALTER TABLE public.zasp_workflow_receipts OWNER TO %I',prior_owner);
 GRANT SELECT,INSERT,UPDATE,DELETE ON TABLE public.zasp_workflow_receipts TO zasp_discovery_authority;
 EXECUTE format('ALTER FUNCTION public.zasp_workflow_mutate(text,text,text,text,text,text,text,text,text,bigint,jsonb,jsonb,text,text,text) OWNER TO %I',prior_owner);
 EXECUTE format('ALTER FUNCTION public.zasp_risk_mutate(text,text,text,text,text,text,text,bigint,text,text,text,text,text) OWNER TO %I',prior_owner);
END $prior_owner_restore$;
...
DROP FUNCTION zasp_execution_apply_risk_projection(text,text,text,text,text,text,text,text,text,bigint,bytea,jsonb);
...
DROP TABLE zasp_discovery_risk_projection_items;
DROP TABLE zasp_discovery_risk_projection_current;
...
DELETE FROM zasp_schema_metadata WHERE key='production_discovery_execution_prior_owner';
