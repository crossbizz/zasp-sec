DO $rollback_guard$
BEGIN
  IF NOT EXISTS(SELECT 1 FROM zasp_schema_metadata WHERE key='security_agent_execution_controls_fingerprint' AND value='fec9bc7992267cd47b6afe90042822413f5cebdc025eae55e64b18c779f96a2e') OR zasp_security_agent_controls_live_fingerprint()<>'fec9bc7992267cd47b6afe90042822413f5cebdc025eae55e64b18c779f96a2e' OR NOT zasp_security_agent_controls_security_ready() OR NOT EXISTS(SELECT 1 FROM zasp_schema_metadata WHERE key='production_core_schema' AND value='security-agent-controls-v1') OR EXISTS(SELECT 1 FROM zasp_schema_versions WHERE version>20) THEN RAISE EXCEPTION USING ERRCODE='55000',MESSAGE='security agent controls rollback rejected';END IF;
