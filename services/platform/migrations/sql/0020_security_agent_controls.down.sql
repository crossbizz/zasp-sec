DO $rollback_guard$
BEGIN
  IF NOT EXISTS(SELECT 1 FROM zasp_schema_metadata WHERE key='security_agent_execution_controls_fingerprint' AND value='3ed37647773abf9c3d3b871e70e627aeb191916062da8881c09e5e867656717a') OR zasp_security_agent_controls_live_fingerprint()<>'3ed37647773abf9c3d3b871e70e627aeb191916062da8881c09e5e867656717a' OR NOT zasp_security_agent_controls_security_ready() OR NOT EXISTS(SELECT 1 FROM zasp_schema_metadata WHERE key='production_core_schema' AND value='security-agent-controls-v1') OR EXISTS(SELECT 1 FROM zasp_schema_versions WHERE version>20) THEN RAISE EXCEPTION USING ERRCODE='55000',MESSAGE='security agent controls rollback rejected';END IF;
