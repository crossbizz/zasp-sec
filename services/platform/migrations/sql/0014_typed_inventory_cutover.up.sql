...
INSERT INTO public.zasp_inventory_legacy_restore(object_kind,object_identity,definition,definition_digest)
SELECT 'function','public.zasp_workflow_mutate(text,text,text,text,text,text,text,text,text,bigint,jsonb,jsonb,text,text,text)',definition,digest(convert_to(definition,'UTF8'),'sha256') FROM (SELECT pg_get_functiondef('public.zasp_workflow_mutate(text,text,text,text,text,text,text,text,text,bigint,jsonb,jsonb,text,text,text)'::regprocedure) definition) captured;
INSERT INTO public.zasp_inventory_legacy_restore(object_kind,object_identity,definition,definition_digest)
...
DO $release_evolution$ DECLARE definition text;original_definition text;BEGIN
 ...
 definition:=replace(definition,'production-discovery-execution-v1','typed-inventory-cutover-v1');
 definition:=replace(definition,'release."version" = 13','release."version" = 14');
 definition:=replace(definition,'release."name" = ''production_discovery_execution''','release."name" = ''typed_inventory_cutover''');
 definition:=replace(definition,'later_release."version" > 13','later_release."version" > 14');
 IF definition=original_definition OR position('typed-inventory-cutover-v1' IN definition)=0 OR position('release."version" = 14' IN definition)=0 OR position('later_release."version" > 14' IN definition)=0 THEN RAISE EXCEPTION USING ERRCODE='55000',MESSAGE='workflow v14 compatibility evolution failed';END IF;
 EXECUTE definition;
 ...
 original_definition:=definition;
 definition:=replace(definition,'production-discovery-execution-v1','typed-inventory-cutover-v1');
 definition:=replace(replace(definition,'release."version"=13','release."version"=14'),'release."version" = 13','release."version" = 14');
 definition:=replace(replace(definition,'release."name"=''production_discovery_execution''','release."name"=''typed_inventory_cutover'''),'release."name" = ''production_discovery_execution''','release."name" = ''typed_inventory_cutover''');
 definition:=replace(replace(definition,'later."version">13','later."version">14'),'later."version" > 13','later."version" > 14');
 IF definition=original_definition OR position('typed-inventory-cutover-v1' IN definition)=0 OR position('typed_inventory_cutover' IN definition)=0 OR position('release."version"=14' IN definition)=0 AND position('release."version" = 14' IN definition)=0 OR position('later."version">14' IN definition)=0 AND position('later."version" > 14' IN definition)=0 THEN RAISE EXCEPTION USING ERRCODE='55000',MESSAGE='risk v14 compatibility evolution failed';END IF;
 EXECUTE definition;
...
CREATE TABLE public.zasp_inventory_capability_evidence (
  organization_id text NOT NULL CHECK(zasp_valid_product_id(organization_id)),
  workspace_id text NOT NULL CHECK(zasp_valid_product_id(workspace_id)),
  environment_id text NOT NULL CHECK(zasp_valid_product_id(environment_id)),
  evidence_id text NOT NULL CHECK(zasp_valid_product_id(evidence_id)),
  agent_id text NOT NULL CHECK(zasp_valid_product_id(agent_id)),
  target_id text NOT NULL CHECK(zasp_valid_product_id(target_id)),
  target_kind text NOT NULL CHECK(target_kind IN('tool','identity','resource','action')),
  category text NOT NULL CHECK(category IN('data_read','data_write','action_execute','identity_assume','network_egress','administration')),
  outcome text NOT NULL CHECK(outcome IN('read','write','execute','assume','connect','administer')),
  evidence_kind text NOT NULL CHECK(evidence_kind IN('attack_lab','runtime_policy')),
  state text NOT NULL CHECK(state IN('verified','blocked')),
  reachability_digest bytea NOT NULL CHECK(octet_length(reachability_digest)=32),
  observed_at timestamptz NOT NULL,
  recorded_at timestamptz NOT NULL DEFAULT transaction_timestamp(),
  PRIMARY KEY(organization_id,workspace_id,environment_id,evidence_id),
  FOREIGN KEY(organization_id,workspace_id,environment_id,agent_id) REFERENCES zasp_inventory_entities(organization_id,workspace_id,environment_id,id) ON DELETE CASCADE,
  FOREIGN KEY(organization_id,workspace_id,environment_id,target_id) REFERENCES zasp_inventory_entities(organization_id,workspace_id,environment_id,id) ON DELETE CASCADE,
  CHECK(agent_id<>target_id),
  CHECK((category,outcome) IN(('data_read','read'),('data_write','write'),('action_execute','execute'),('identity_assume','assume'),('network_egress','connect'),('administration','administer'))),
  CHECK((evidence_kind,state) IN(('attack_lab','verified'),('runtime_policy','blocked')))
);
...
ALTER TABLE public.zasp_inventory_entities
  ...
  ADD COLUMN winning_attributes jsonb CHECK(winning_attributes IS NULL OR jsonb_typeof(winning_attributes)='object' AND octet_length(winning_attributes::text)<=262144),
...
CREATE FUNCTION public.zasp_inventory_validate_typed_entities(provider_value text,entities_value jsonb) RETURNS boolean LANGUAGE sql STABLE AS $$
 SELECT provider_value IN('aws','kubernetes','github','okta') AND NOT EXISTS(
  SELECT 1 FROM jsonb_to_recordset(entities_value) entity_value(kind text,attributes jsonb,identity_namespace text,identity_rule_version integer,identity_priority integer,product_kind text,confidence_basis_points integer,observed_at text,fresh_until text,source_projection_version integer)
  ...
  WHERE rule_value.provider IS NULL OR (entity_value.identity_namespace,entity_value.identity_rule_version,entity_value.identity_priority,entity_value.product_kind,entity_value.confidence_basis_points,entity_value.source_projection_version)<>(rule_value.identity_namespace,rule_value.rule_version,rule_value.priority,rule_value.product_kind,rule_value.confidence_basis_points,rule_value.rule_version)
   ...
   OR entity_value.product_kind='agent' AND (
    jsonb_typeof(entity_value.attributes->'posture')<>'object'
	    OR NOT ((entity_value.attributes->'posture') ?& ARRAY['human_credential','credential_fingerprint','untrusted_input','production_write','shell_execution','production_credential','unrestricted_egress','sensitive_data_reach','unapproved_remote_tool','destructive_tool','runtime_control','production_agent','runtime_policy_supported','host_filesystem','privileged','cicd_write','production_secret_reach','credential_active'])
	    OR (entity_value.attributes->'posture') - ARRAY['human_credential','credential_fingerprint','untrusted_input','production_write','shell_execution','production_credential','unrestricted_egress','sensitive_data_reach','unapproved_remote_tool','destructive_tool','runtime_control','production_agent','runtime_policy_supported','host_filesystem','privileged','cicd_write','production_secret_reach','credential_active']<>'{}'::jsonb
    OR entity_value.attributes->'posture'->>'credential_fingerprint'<>'' AND entity_value.attributes->'posture'->>'credential_fingerprint' !~ '^sha256:[0-9a-f]{64}$'
    OR EXISTS(SELECT 1 FROM jsonb_each(entity_value.attributes->'posture') fact(key,value) WHERE fact.key<>'credential_fingerprint' AND jsonb_typeof(fact.value)<>'boolean')
    OR jsonb_typeof(entity_value.attributes->'posture'->'credential_fingerprint')<>'string'
   )
   OR entity_value.product_kind<>'agent' AND entity_value.attributes ? 'posture'
 )
$$;
...
BEGIN
 ...
 SELECT jsonb_build_object('items',COALESCE(jsonb_agg(jsonb_build_object('id',id,'name',display_name,'kind',product_kind,'owner',annotation_value->>'owner','team',annotation_value->>'team','tags',annotation_value->'tags','evidence_id',winning_evidence_id,'confidence_basis_points',confidence_basis_points,'first_seen',rtrim(rtrim(to_char(first_seen_at AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US'),'0'),'.')||'Z','last_seen',rtrim(rtrim(to_char(last_seen_at AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US'),'0'),'.')||'Z','observed_at',rtrim(rtrim(to_char(observed_at AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US'),'0'),'.')||'Z','fresh_until',rtrim(rtrim(to_char(fresh_until AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US'),'0'),'.')||'Z','freshness_state',CASE WHEN fresh_until>statement_timestamp() THEN 'fresh' ELSE 'stale' END,'version',annotation_version) ORDER BY id),'[]'::jsonb),'next_id',CASE WHEN (SELECT count(*) FROM candidates)>limit_value THEN (SELECT id FROM visible ORDER BY id DESC LIMIT 1) ELSE NULL END) INTO result_value FROM visible;
 RETURN result_value;
END $$;
...
CREATE FUNCTION public.zasp_inventory_update_agent(organization_value text,workspace_value text,environment_value text,principal_value text,agent_value text,idempotency_value text,expected_version_value bigint,owner_value text,team_value text,tags_value jsonb,audit_value text,correlation_value text) RETURNS jsonb LANGUAGE plpgsql AS $$
DECLARE intent_value jsonb;digest_value bytea;prior_digest bytea;prior_response jsonb;entity_record zasp_inventory_entities%ROWTYPE;annotation_record zasp_inventory_annotations%ROWTYPE;result_value jsonb;
BEGIN
 IF NOT zasp_valid_product_id(organization_value) OR NOT zasp_valid_product_id(workspace_value) OR NOT zasp_valid_product_id(environment_value) OR NOT zasp_valid_product_id(principal_value) OR NOT zasp_valid_product_id(agent_value) OR NOT zasp_valid_product_id(audit_value) OR NOT zasp_valid_product_id(correlation_value)
  OR idempotency_value !~ '^[A-Za-z0-9][A-Za-z0-9._:-]{15,127}$' OR expected_version_value NOT BETWEEN 1 AND 999999
  OR length(owner_value) NOT BETWEEN 1 AND 128 OR owner_value<>btrim(owner_value) OR owner_value ~ '[[:cntrl:]]'
  OR length(team_value) NOT BETWEEN 1 AND 128 OR team_value<>btrim(team_value) OR team_value ~ '[[:cntrl:]]'
  OR jsonb_typeof(tags_value)<>'array' OR jsonb_array_length(tags_value)>32 OR octet_length(tags_value::text)>4096
  OR EXISTS(SELECT 1 FROM jsonb_array_elements(tags_value) tag WHERE jsonb_typeof(tag)<>'string' OR length(tag#>>'{}') NOT BETWEEN 1 AND 64 OR tag#>>'{}'<>btrim(tag#>>'{}') OR tag#>>'{}' ~ '[[:cntrl:]]')
  OR EXISTS(SELECT 1 FROM jsonb_array_elements_text(tags_value) WITH ORDINALITY prior(tag,position) JOIN jsonb_array_elements_text(tags_value) WITH ORDINALITY following(tag,position) ON following.position=prior.position+1 WHERE prior.tag>=following.tag)
 THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='invalid agent ownership mutation';END IF;
 intent_value:=jsonb_build_object('resource_id',agent_value,'expected_version',expected_version_value,'body',jsonb_build_object('owner',owner_value,'team',team_value,'tags',tags_value));
 digest_value:=digest(convert_to(intent_value::text,'UTF8'),'sha256');
 PERFORM pg_advisory_xact_lock(hashtextextended(concat_ws(chr(31),organization_value,workspace_value,environment_value,principal_value,'updateAgent',idempotency_value),0));
 SELECT request_digest,response INTO prior_digest,prior_response FROM zasp_workflow_idempotency WHERE (organization_id,workspace_id,environment_id,principal_id,operation,idempotency_key)=(organization_value,workspace_value,environment_value,principal_value,'updateAgent',idempotency_value);
 IF FOUND THEN
  IF prior_digest<>digest_value THEN RAISE EXCEPTION USING ERRCODE='23505',MESSAGE='agent ownership idempotency conflict';END IF;
  RETURN prior_response||jsonb_build_object('replayed',true);
 END IF;
 IF zasp_inventory_scope_state(organization_value,workspace_value,environment_value)->>'phase'<>'cutover' THEN RAISE EXCEPTION USING ERRCODE='55000',MESSAGE='typed inventory cutover required';END IF;
 SELECT * INTO entity_record FROM zasp_inventory_entities entity_value WHERE (entity_value.organization_id,entity_value.workspace_id,entity_value.environment_id,entity_value.id,entity_value.state,entity_value.product_kind)=(organization_value,workspace_value,environment_value,agent_value,'active','agent') FOR UPDATE;
 IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE='P0002',MESSAGE='typed agent missing';END IF;
 IF entity_record.annotation_version<>expected_version_value THEN RAISE EXCEPTION USING ERRCODE='40001',MESSAGE='agent ownership version conflict';END IF;
 SELECT * INTO annotation_record FROM zasp_inventory_annotations annotation WHERE (annotation.organization_id,annotation.workspace_id,annotation.environment_id,annotation.entity_id)=(organization_value,workspace_value,environment_value,agent_value);
 IF FOUND AND annotation_record.version<>entity_record.annotation_version THEN RAISE EXCEPTION USING ERRCODE='55000',MESSAGE='agent annotation authority drift';END IF;
 INSERT INTO zasp_inventory_annotations(organization_id,workspace_id,environment_id,entity_id,owner_value,team_value,tags,version,updated_at)
 VALUES(organization_value,workspace_value,environment_value,agent_value,owner_value,team_value,tags_value,expected_version_value+1,transaction_timestamp())
 ON CONFLICT(organization_id,workspace_id,environment_id,entity_id) DO UPDATE SET owner_value=excluded.owner_value,team_value=excluded.team_value,tags=excluded.tags,version=excluded.version,updated_at=excluded.updated_at;
 UPDATE zasp_inventory_entities entity_value SET annotation_version=expected_version_value+1 WHERE (entity_value.organization_id,entity_value.workspace_id,entity_value.environment_id,entity_value.id,entity_value.annotation_version)=(organization_value,workspace_value,environment_value,agent_value,expected_version_value);
 IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE='40001',MESSAGE='agent ownership version conflict';END IF;
 PERFORM zasp_inventory_refresh_posture_findings(organization_value,workspace_value,environment_value);
 result_value:=jsonb_build_object('agent',zasp_inventory_detail(organization_value,workspace_value,environment_value,agent_value,'agent')->'summary','audit_id',audit_value,'correlation_id',correlation_value,'replayed',false);
 INSERT INTO zasp_workflow_audit(organization_id,workspace_id,environment_id,audit_id,correlation_id,principal_id,operation,resource_kind,resource_id,resource_version) VALUES(organization_value,workspace_value,environment_value,audit_value,correlation_value,principal_value,'updateAgent','agent',agent_value,expected_version_value+1);
 INSERT INTO zasp_workflow_idempotency(organization_id,workspace_id,environment_id,principal_id,operation,idempotency_key,request_digest,response) VALUES(organization_value,workspace_value,environment_value,principal_value,'updateAgent',idempotency_value,digest_value,result_value);
 RETURN result_value;
END $$;

CREATE FUNCTION public.zasp_inventory_agent_capability_edges(organization_value text,workspace_value text,environment_value text,agent_value text) RETURNS TABLE(agent_id text,target_id text,target_kind text,category text,outcome text,evidence_ids text[],reachability_digest bytea) LANGUAGE plpgsql STABLE AS $$
BEGIN
 IF NOT zasp_valid_product_id(organization_value) OR NOT zasp_valid_product_id(workspace_value) OR NOT zasp_valid_product_id(environment_value) OR NOT zasp_valid_product_id(agent_value) OR zasp_inventory_scope_state(organization_value,workspace_value,environment_value)->>'phase'<>'cutover' THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='invalid typed capability authority';END IF;
 ...
 RETURN QUERY WITH identity_paths AS (
  ...
  UNION ALL
  SELECT agent_value,role_categories.role_id,'resource',role_categories.category,role_categories.outcome,evidence_id FROM role_categories CROSS JOIN LATERAL unnest(ARRAY[role_categories.agent_evidence,role_categories.identity_evidence,role_categories.binding_evidence,role_categories.role_evidence]) evidence_id
 ),distinct_evidence AS (
  SELECT DISTINCT raw_capabilities.agent_id,raw_capabilities.target_id,raw_capabilities.target_kind,raw_capabilities.category,raw_capabilities.outcome,raw_capabilities.evidence_id FROM raw_capabilities
 )
 SELECT raw.agent_id,raw.target_id,raw.target_kind,raw.category,raw.outcome,(array_agg(raw.evidence_id ORDER BY raw.evidence_id))[1:63],digest(convert_to(to_jsonb(array_agg(raw.evidence_id ORDER BY raw.evidence_id))::text,'UTF8'),'sha256')
 FROM distinct_evidence raw GROUP BY raw.agent_id,raw.target_id,raw.target_kind,raw.category,raw.outcome;
END $$;

CREATE FUNCTION public.zasp_inventory_record_capability_evidence(organization_value text,workspace_value text,environment_value text,agent_value text,target_value text,category_value text,outcome_value text,evidence_kind_value text,evidence_value text,observed_value timestamptz) RETURNS jsonb LANGUAGE plpgsql AS $$
DECLARE edge_value record;existing_value zasp_inventory_capability_evidence%ROWTYPE;state_value text;
BEGIN
 IF NOT zasp_valid_product_id(organization_value) OR NOT zasp_valid_product_id(workspace_value) OR NOT zasp_valid_product_id(environment_value) OR NOT zasp_valid_product_id(agent_value) OR NOT zasp_valid_product_id(target_value) OR NOT zasp_valid_product_id(evidence_value)
  OR category_value NOT IN('data_read','data_write','action_execute','identity_assume','network_egress','administration') OR outcome_value NOT IN('read','write','execute','assume','connect','administer')
  OR (category_value,outcome_value) NOT IN(('data_read','read'),('data_write','write'),('action_execute','execute'),('identity_assume','assume'),('network_egress','connect'),('administration','administer'))
  OR evidence_kind_value NOT IN('attack_lab','runtime_policy') OR observed_value IS NULL OR observed_value<transaction_timestamp()-interval '365 days' OR observed_value>transaction_timestamp()+interval '5 minutes'
 THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='invalid capability evidence';END IF;
 SELECT * INTO edge_value FROM zasp_inventory_agent_capability_edges(organization_value,workspace_value,environment_value,agent_value) edge
  WHERE (edge.target_id,edge.category,edge.outcome)=(target_value,category_value,outcome_value);
 IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE='P0002',MESSAGE='capability edge missing';END IF;
 state_value:=CASE evidence_kind_value WHEN 'attack_lab' THEN 'verified' ELSE 'blocked' END;
 PERFORM pg_advisory_xact_lock(hashtextextended(concat_ws(chr(31),'zasp-capability-evidence',organization_value,workspace_value,environment_value,evidence_value),0));
 SELECT * INTO existing_value FROM zasp_inventory_capability_evidence evidence
  WHERE (evidence.organization_id,evidence.workspace_id,evidence.environment_id,evidence.evidence_id)=(organization_value,workspace_value,environment_value,evidence_value);
 IF FOUND THEN
  IF (existing_value.agent_id,existing_value.target_id,existing_value.target_kind,existing_value.category,existing_value.outcome,existing_value.evidence_kind,existing_value.state,existing_value.reachability_digest,existing_value.observed_at)
    IS DISTINCT FROM (agent_value,target_value,edge_value.target_kind,category_value,outcome_value,evidence_kind_value,state_value,edge_value.reachability_digest,observed_value)
  THEN RAISE EXCEPTION USING ERRCODE='23505',MESSAGE='capability evidence identity conflict';END IF;
  RETURN jsonb_build_object('agent_id',agent_value,'target_id',target_value,'target_kind',edge_value.target_kind,'category',category_value,'outcome',outcome_value,'state',state_value,'evidence_id',evidence_value,'replayed',true);
 END IF;
 INSERT INTO zasp_inventory_capability_evidence(organization_id,workspace_id,environment_id,evidence_id,agent_id,target_id,target_kind,category,outcome,evidence_kind,state,reachability_digest,observed_at)
 VALUES(organization_value,workspace_value,environment_value,evidence_value,agent_value,target_value,edge_value.target_kind,category_value,outcome_value,evidence_kind_value,state_value,edge_value.reachability_digest,observed_value);
 RETURN jsonb_build_object('agent_id',agent_value,'target_id',target_value,'target_kind',edge_value.target_kind,'category',category_value,'outcome',outcome_value,'state',state_value,'evidence_id',evidence_value,'replayed',false);
END $$;
...
DECLARE result_value jsonb;
BEGIN
 IF limit_value NOT BETWEEN 1 AND 100 OR after_value IS NOT NULL AND (array_length(string_to_array(after_value,'~'),1)<>3 OR NOT zasp_valid_product_id(split_part(after_value,'~',1)) OR split_part(after_value,'~',2) NOT IN('data_read','data_write','action_execute','identity_assume','network_egress','administration') OR split_part(after_value,'~',3) NOT IN('read','write','execute','assume','connect','administer'))
 THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='invalid typed capability page';END IF;
 WITH grouped AS (
  SELECT edge.*,edge.target_id||'~'||edge.category||'~'||edge.outcome key_value FROM zasp_inventory_agent_capability_edges(organization_value,workspace_value,environment_value,agent_value) edge
 ),state_evidence AS (
  SELECT DISTINCT ON(evidence.agent_id,evidence.target_id,evidence.category,evidence.outcome,evidence.reachability_digest)
   evidence.agent_id,evidence.target_id,evidence.category,evidence.outcome,evidence.reachability_digest,evidence.state,evidence.evidence_id
  FROM zasp_inventory_capability_evidence evidence JOIN grouped edge
   ON (evidence.organization_id,evidence.workspace_id,evidence.environment_id,evidence.agent_id,evidence.target_id,evidence.target_kind,evidence.category,evidence.outcome,evidence.reachability_digest)
    =(organization_value,workspace_value,environment_value,edge.agent_id,edge.target_id,edge.target_kind,edge.category,edge.outcome,edge.reachability_digest)
  ORDER BY evidence.agent_id,evidence.target_id,evidence.category,evidence.outcome,evidence.reachability_digest,CASE evidence.state WHEN 'blocked' THEN 2 ELSE 1 END DESC,evidence.observed_at DESC,evidence.evidence_id DESC
 ),enriched AS (
  SELECT edge.*,COALESCE(state_evidence.state,'observed') state_value,
   CASE WHEN state_evidence.evidence_id IS NULL THEN edge.evidence_ids ELSE ARRAY(SELECT proof.evidence_id FROM (SELECT unnest(edge.evidence_ids) evidence_id UNION SELECT state_evidence.evidence_id) proof ORDER BY proof.evidence_id LIMIT 64) END evidence_values
  FROM grouped edge LEFT JOIN state_evidence ON (state_evidence.agent_id,state_evidence.target_id,state_evidence.category,state_evidence.outcome,state_evidence.reachability_digest)=(edge.agent_id,edge.target_id,edge.category,edge.outcome,edge.reachability_digest)
 ),candidates AS (
  SELECT * FROM enriched WHERE after_value IS NULL OR key_value>after_value ORDER BY key_value LIMIT limit_value+1
 ),visible AS (SELECT * FROM candidates ORDER BY key_value LIMIT limit_value)
 SELECT jsonb_build_object('items',COALESCE(jsonb_agg(jsonb_build_object('agent_id',agent_id,'target_id',target_id,'target_kind',target_kind,'category',category,'outcome',outcome,'state',state_value,'reachable',true,'evidence_ids',to_jsonb(evidence_values)) ORDER BY key_value),'[]'::jsonb),'next_key',CASE WHEN (SELECT count(*) FROM candidates)>limit_value THEN (SELECT key_value FROM visible ORDER BY key_value DESC LIMIT 1) ELSE NULL END) INTO result_value FROM visible;
 RETURN result_value;
END $$;
...
BEGIN
 IF source<>cursor_provider OR source NOT IN('aws','kubernetes','github','okta') OR jsonb_typeof(entities)<>'array' OR jsonb_typeof(relationships)<>'array' OR jsonb_typeof(evidence)<>'array' OR jsonb_array_length(entities)>1000 OR jsonb_array_length(relationships)>2000 OR jsonb_array_length(evidence)>1000 OR (SELECT count(*) FROM jsonb_array_elements(evidence) item WHERE NOT item ? 'finding_id')<>jsonb_array_length(entities) OR octet_length(manifest_checksum)<>32 THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='invalid typed complete snapshot';END IF;
 ...
 OR EXISTS(SELECT 1 FROM jsonb_array_elements(evidence) item WHERE jsonb_typeof(item)<>'object' OR NOT ((NOT item ? 'finding_id' AND item ?& ARRAY['id','entity_id','object_reference','artifact_reference','artifact_key','artifact_version_id','checksum_hex','size_bytes','media_type','schema_version','parser_version','tool_version'] AND item-ARRAY['id','entity_id','object_reference','artifact_reference','artifact_key','artifact_version_id','checksum_hex','size_bytes','media_type','schema_version','parser_version','tool_version']='{}'::jsonb) OR (item ?& ARRAY['id','entity_id','finding_id','check_id','severity','status','observed_at','object_reference','artifact_reference','artifact_key','artifact_version_id','checksum_hex','size_bytes','media_type','schema_version','parser_version','tool_version'] AND item-ARRAY['id','entity_id','finding_id','check_id','severity','status','observed_at','object_reference','artifact_reference','artifact_key','artifact_version_id','checksum_hex','size_bytes','media_type','schema_version','parser_version','tool_version']='{}'::jsonb)))
 ...
 IF EXISTS(
  SELECT 1 FROM jsonb_array_elements(evidence) evidence_item CROSS JOIN LATERAL jsonb_to_record(evidence_item) evidence_value(id text,entity_id text,object_reference text,artifact_reference text,artifact_key text,artifact_version_id text,checksum_hex text,size_bytes bigint,media_type text,schema_version text,parser_version text,tool_version text)
 ...
 WITH candidate AS (SELECT * FROM jsonb_to_recordset(entities) entity_value(id text,product_kind text,display_name text,stable_fields jsonb,attributes jsonb,observed_at timestamptz))
 INSERT INTO zasp_inventory_entities(organization_id,workspace_id,environment_id,id,kind,display_name,stable_fields,state,first_seen_at,last_seen_at,product_kind,winning_attributes)
 SELECT $1,$2,$3,candidate.id,candidate.product_kind,candidate.display_name,candidate.stable_fields,'active',candidate.observed_at,candidate.observed_at,candidate.product_kind,candidate.attributes FROM candidate ON CONFLICT(organization_id,workspace_id,environment_id,id) DO NOTHING;
 ...
 FROM jsonb_array_elements(evidence) evidence_item CROSS JOIN LATERAL jsonb_to_record(evidence_item) evidence_value(id text,entity_id text,object_reference text,artifact_reference text,artifact_key text,artifact_version_id text,checksum_hex text,size_bytes bigint,media_type text,schema_version text,parser_version text,tool_version text) WHERE NOT evidence_item ? 'finding_id';
 ...
 UPDATE zasp_inventory_entities entity_value SET
  version=entity_value.version+CASE WHEN (entity_value.kind,entity_value.display_name,entity_value.stable_fields,entity_value.state,entity_value.product_kind,entity_value.confidence_basis_points,entity_value.winning_evidence_id,entity_value.winning_snapshot_id,entity_value.winning_generation,entity_value.observed_at,entity_value.fresh_until,entity_value.projection_version,entity_value.winning_integration_id,entity_value.winning_provider,entity_value.winning_source,entity_value.winning_source_native_id,entity_value.winning_attributes,entity_value.winning_identity_rule,entity_value.winning_source_projection) IS DISTINCT FROM (winner.product_kind,winner.display_name,winner.stable_fields,'active',winner.product_kind,winner.confidence_basis_points,winner.evidence_id,winner.snapshot_id,winner.generation,winner.observed_at,winner.fresh_until,winner.source_projection_version,winner.integration_id,winner.provider,winner.source,winner.source_native_id,winner.attributes,winner.identity_rule_version,winner.source_projection_version) THEN 1 ELSE 0 END,
  kind=winner.product_kind,display_name=winner.display_name,stable_fields=winner.stable_fields,state='active',first_seen_at=winner.minimum_seen,last_seen_at=winner.maximum_seen,tombstoned_at=NULL,product_kind=winner.product_kind,confidence_basis_points=winner.confidence_basis_points,winning_evidence_id=winner.evidence_id,winning_snapshot_id=winner.snapshot_id,winning_generation=winner.generation,observed_at=winner.observed_at,fresh_until=winner.fresh_until,projection_version=winner.source_projection_version,winning_integration_id=winner.integration_id,winning_provider=winner.provider,winning_source=winner.source,winning_source_native_id=winner.source_native_id,winning_attributes=winner.attributes,winning_identity_rule=winner.identity_rule_version,winning_source_projection=winner.source_projection_version
...
CREATE FUNCTION public.zasp_inventory_refresh_posture_findings(organization_value text,workspace_value text,environment_value text) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO pg_catalog, public AS $$
DECLARE candidate_items jsonb;active_count integer;resolved_count integer;
BEGIN
 IF NOT zasp_valid_product_id(organization_value) OR NOT zasp_valid_product_id(workspace_value) OR NOT zasp_valid_product_id(environment_value) THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='invalid posture scope';END IF;
 PERFORM pg_advisory_xact_lock(hashtextextended(concat_ws(chr(31),'zasp-posture',organization_value,workspace_value,environment_value),0));
 WITH agents AS (
  SELECT entity_value.id,entity_value.state,entity_value.winning_evidence_id,entity_value.winning_attributes->'posture' posture,COALESCE(annotation.owner_value,'') owner_value
  FROM zasp_inventory_entities entity_value LEFT JOIN zasp_inventory_annotations annotation
   ON (annotation.organization_id,annotation.workspace_id,annotation.environment_id,annotation.entity_id)=(entity_value.organization_id,entity_value.workspace_id,entity_value.environment_id,entity_value.id)
  WHERE (entity_value.organization_id,entity_value.workspace_id,entity_value.environment_id,entity_value.product_kind)=(organization_value,workspace_value,environment_value,'agent')
   AND entity_value.winning_evidence_id IS NOT NULL
 ),matches AS (
  SELECT agent.id agent_id,agent.winning_evidence_id evidence_id,rule_value.rule,rule_value.title
  FROM agents agent CROSS JOIN LATERAL (VALUES
   ('ownerless_agent','Agent has no owner',agent.state='active' AND agent.owner_value=''),
   ('human_credential','Agent uses a human credential',agent.state='active' AND (agent.posture->>'human_credential')::boolean),
   ('shared_credential','Credential is shared across agents',agent.state='active' AND agent.posture->>'credential_fingerprint' ~ '^sha256:[0-9a-f]{64}$' AND (SELECT count(*) FROM agents shared WHERE shared.state='active' AND (shared.posture->>'credential_active')::boolean AND shared.posture->>'credential_fingerprint'=agent.posture->>'credential_fingerprint')>1),
   ('untrusted_production_write','Untrusted input can write to production',agent.state='active' AND (agent.posture->>'untrusted_input')::boolean AND (agent.posture->>'production_write')::boolean),
   ('shell_credential','Shell execution can use a production credential',agent.state='active' AND (agent.posture->>'shell_execution')::boolean AND (agent.posture->>'production_credential')::boolean),
   ('egress_sensitive','Unrestricted egress can reach sensitive data',agent.state='active' AND (agent.posture->>'unrestricted_egress')::boolean AND (agent.posture->>'sensitive_data_reach')::boolean),
   ('unapproved_tool','Agent can reach an unapproved remote tool',agent.state='active' AND (agent.posture->>'unapproved_remote_tool')::boolean),
   ('destructive_no_control','Destructive action lacks runtime control',agent.state='active' AND (agent.posture->>'destructive_tool')::boolean AND NOT (agent.posture->>'runtime_control')::boolean),
   ('no_runtime_coverage','Production agent lacks runtime policy coverage',agent.state='active' AND (agent.posture->>'production_agent')::boolean AND NOT (agent.posture->>'runtime_policy_supported')::boolean),
   ('weak_runtime_isolation','Runtime has weak isolation',agent.state='active' AND ((agent.posture->>'host_filesystem')::boolean OR (agent.posture->>'privileged')::boolean)),
   ('cicd_production_secret','CI/CD write can reach production secrets',agent.state='active' AND (agent.posture->>'cicd_write')::boolean AND (agent.posture->>'production_secret_reach')::boolean),
   ('zombie_credential','Inactive agent retains an active credential',agent.state='tombstoned' AND (agent.posture->>'credential_active')::boolean)
  ) rule_value(rule,title,matched)
  WHERE rule_value.matched
 ),candidates AS (
  SELECT zasp_discovery_canonical_id(organization_value,workspace_value,environment_value,'posture_finding',matches.agent_id||chr(31)||matches.rule) finding_id,* FROM matches
 )
 SELECT COALESCE(jsonb_agg(jsonb_build_object('finding_id',finding_id,'agent_id',agent_id,'evidence_id',evidence_id,'rule',rule,'title',title) ORDER BY finding_id),'[]'::jsonb) INTO candidate_items FROM candidates;
 IF EXISTS(
  SELECT 1 FROM jsonb_array_elements(candidate_items) candidate JOIN zasp_risk_findings finding
   ON (finding.organization_id,finding.workspace_id,finding.environment_id,finding.id)=(organization_value,workspace_value,environment_value,candidate->>'finding_id')
  WHERE (finding.source,finding.rule,finding.compliance_context) IS DISTINCT FROM ('posture',candidate->>'rule','zasp-posture:'||(candidate->>'rule'))
 ) THEN RAISE EXCEPTION USING ERRCODE='23505',MESSAGE='posture finding ownership conflict';END IF;

 INSERT INTO zasp_risk_findings AS finding(organization_id,workspace_id,environment_id,id,source,rule,title,severity,status,agent_id,compliance_context)
 SELECT organization_value,workspace_value,environment_value,candidate->>'finding_id','posture',candidate->>'rule',candidate->>'title','high','open',candidate->>'agent_id','zasp-posture:'||(candidate->>'rule') FROM jsonb_array_elements(candidate_items) candidate
 ON CONFLICT(organization_id,workspace_id,environment_id,id) DO UPDATE SET
  source='posture',rule=excluded.rule,title=excluded.title,severity='high',status=CASE WHEN finding.status IN('accepted','under_review') THEN finding.status ELSE 'open' END,
  agent_id=excluded.agent_id,path_id=NULL,compliance_context=excluded.compliance_context,acceptance_reason=CASE WHEN finding.status='accepted' THEN finding.acceptance_reason ELSE NULL END,
  version=finding.version+CASE WHEN (finding.source,finding.rule,finding.title,finding.severity,finding.status,finding.agent_id,finding.path_id,finding.compliance_context,finding.acceptance_reason) IS DISTINCT FROM ('posture',excluded.rule,excluded.title,'high',CASE WHEN finding.status IN('accepted','under_review') THEN finding.status ELSE 'open' END,excluded.agent_id,NULL,excluded.compliance_context,CASE WHEN finding.status='accepted' THEN finding.acceptance_reason ELSE NULL END) THEN 1 ELSE 0 END,
  updated_at=CASE WHEN (finding.source,finding.rule,finding.title,finding.severity,finding.status,finding.agent_id,finding.path_id,finding.compliance_context,finding.acceptance_reason) IS DISTINCT FROM ('posture',excluded.rule,excluded.title,'high',CASE WHEN finding.status IN('accepted','under_review') THEN finding.status ELSE 'open' END,excluded.agent_id,NULL,excluded.compliance_context,CASE WHEN finding.status='accepted' THEN finding.acceptance_reason ELSE NULL END) THEN transaction_timestamp() ELSE finding.updated_at END;

 DELETE FROM zasp_risk_finding_evidence evidence_value USING jsonb_array_elements(candidate_items) candidate
 WHERE (evidence_value.organization_id,evidence_value.workspace_id,evidence_value.environment_id,evidence_value.finding_id)=(organization_value,workspace_value,environment_value,candidate->>'finding_id');
 INSERT INTO zasp_risk_finding_evidence(organization_id,workspace_id,environment_id,finding_id,position,evidence_id)
 SELECT organization_value,workspace_value,environment_value,candidate->>'finding_id',1,candidate->>'evidence_id' FROM jsonb_array_elements(candidate_items) candidate;

 DELETE FROM zasp_risk_finding_factors factor_value USING jsonb_array_elements(candidate_items) candidate
 WHERE (factor_value.organization_id,factor_value.workspace_id,factor_value.environment_id,factor_value.finding_id)=(organization_value,workspace_value,environment_value,candidate->>'finding_id');
 INSERT INTO zasp_risk_finding_factors(organization_id,workspace_id,environment_id,finding_id,position,name,evidence_id)
 SELECT organization_value,workspace_value,environment_value,candidate->>'finding_id',1,candidate->>'title',candidate->>'evidence_id' FROM jsonb_array_elements(candidate_items) candidate;

 UPDATE zasp_risk_findings finding SET status='resolved',path_id=NULL,acceptance_reason=NULL,version=finding.version+1,updated_at=transaction_timestamp()
 WHERE (finding.organization_id,finding.workspace_id,finding.environment_id,finding.source)=(organization_value,workspace_value,environment_value,'posture')
  AND finding.compliance_context LIKE 'zasp-posture:%' AND finding.status IN('open','under_review')
  AND NOT EXISTS(SELECT 1 FROM jsonb_array_elements(candidate_items) candidate WHERE candidate->>'finding_id'=finding.id);
 GET DIAGNOSTICS resolved_count=ROW_COUNT;
 SELECT count(*) INTO active_count FROM zasp_risk_findings finding WHERE (finding.organization_id,finding.workspace_id,finding.environment_id,finding.source)=(organization_value,workspace_value,environment_value,'posture') AND finding.compliance_context LIKE 'zasp-posture:%' AND finding.status IN('open','under_review','accepted');
 RETURN jsonb_build_object('active_count',active_count,'resolved_count',resolved_count);
END $$;
...
BEGIN
 ...
 ELSE
   ...
   WITH ranked_findings AS (
    SELECT item.payload->>'finding_id' finding_id,item.payload->>'entity_id' entity_id,item.payload->>'check_id' check_id,item.payload->>'id' evidence_id,item.source,
    ...
    WHERE (item.organization_id,item.workspace_id,item.environment_id,item.section)=(organization_value,workspace_value,environment_value,'evidence')
      AND item.payload ?& ARRAY['finding_id','entity_id','check_id','severity','status'] AND item.payload->>'severity'='high' AND item.payload->>'status'='FAIL'
      AND zasp_valid_product_id(item.payload->>'finding_id') AND zasp_valid_product_id(item.payload->>'entity_id') AND zasp_valid_product_id(item.payload->>'id') AND item.payload->>'check_id' ~ '^[a-z][a-z0-9_]{0,63}$'
   )
   SELECT COALESCE(jsonb_agg(jsonb_build_object('finding_id',finding_id,'entity_id',entity_id,'check_id',check_id,'evidence_id',evidence_id,'source',source) ORDER BY finding_id),'[]'::jsonb) INTO finding_items FROM ranked_findings WHERE rank_value=1;
...
DECLARE old_path_ids text[];path_items jsonb;
BEGIN
 IF jsonb_typeof(items_value)<>'array' OR jsonb_array_length(items_value)>1000 OR octet_length(items_value::text)>1048576 OR EXISTS(
  SELECT 1 FROM jsonb_array_elements(items_value) item
  WHERE jsonb_typeof(item)<>'object' OR NOT item ?& ARRAY['finding_id','entity_id','check_id','evidence_id','source'] OR item-ARRAY['finding_id','entity_id','check_id','evidence_id','source']<>'{}'::jsonb
    OR NOT zasp_valid_product_id(item->>'finding_id') OR NOT zasp_valid_product_id(item->>'entity_id') OR NOT zasp_valid_product_id(item->>'evidence_id') OR item->>'check_id' !~ '^[a-z][a-z0-9_]{0,63}$' OR item->>'source' NOT IN('aws','kubernetes','github','okta')
 ...
 SELECT COALESCE(array_agg(DISTINCT path.id),'{}'::text[]) INTO old_path_ids FROM zasp_risk_findings finding
 JOIN zasp_risk_attack_paths path ON (path.organization_id,path.workspace_id,path.environment_id,path.id)=(finding.organization_id,finding.workspace_id,finding.environment_id,finding.path_id)
 WHERE (finding.organization_id,finding.workspace_id,finding.environment_id,finding.source)=(organization_value,workspace_value,environment_value,'prowler')
   AND finding.compliance_context LIKE 'zasp-discovery:%'
   AND path.id=zasp_discovery_canonical_id(organization_value,workspace_value,environment_value,'attack_path',finding.id||chr(31)||(SELECT string_agg(node.node_id,chr(31) ORDER BY node.position) FROM zasp_risk_attack_path_nodes node WHERE (node.organization_id,node.workspace_id,node.environment_id,node.path_id)=(organization_value,workspace_value,environment_value,path.id)));
 ...
 WITH RECURSIVE input_items AS (
  SELECT item->>'finding_id' finding_id,item->>'entity_id' entity_id,item->>'evidence_id' evidence_id FROM jsonb_array_elements(items_value) item
 ),edges AS (
  SELECT CASE WHEN relationship.kind='trusts' THEN relationship.to_entity_id ELSE relationship.from_entity_id END from_id,
         CASE WHEN relationship.kind='trusts' THEN relationship.from_entity_id ELSE relationship.to_entity_id END to_id
  FROM zasp_inventory_relationships relationship
  JOIN zasp_inventory_entities from_entity ON (from_entity.organization_id,from_entity.workspace_id,from_entity.environment_id,from_entity.id,from_entity.state)=(relationship.organization_id,relationship.workspace_id,relationship.environment_id,relationship.from_entity_id,'active')
  JOIN zasp_inventory_entities to_entity ON (to_entity.organization_id,to_entity.workspace_id,to_entity.environment_id,to_entity.id,to_entity.state)=(relationship.organization_id,relationship.workspace_id,relationship.environment_id,relationship.to_entity_id,'active')
  WHERE (relationship.organization_id,relationship.workspace_id,relationship.environment_id,relationship.state)=(organization_value,workspace_value,environment_value,'present')
    AND relationship.kind IN('assigned_to','binds','uses_policy','uses_identity','has_permission','depends_on','member_of','trusts')
 ),walk AS (
  SELECT input.finding_id,input.evidence_id,ARRAY[edge.from_id,edge.to_id]::text[] node_ids
  FROM input_items input JOIN edges edge ON edge.to_id=input.entity_id
  UNION ALL
  SELECT walk.finding_id,walk.evidence_id,ARRAY[edge.from_id]::text[]||walk.node_ids
  FROM walk JOIN edges edge ON edge.to_id=walk.node_ids[1]
  WHERE cardinality(walk.node_ids)<8 AND NOT edge.from_id=ANY(walk.node_ids)
 ),ranked AS (
  SELECT walk.*,row_number() OVER(PARTITION BY walk.finding_id ORDER BY cardinality(walk.node_ids) DESC,array_to_string(walk.node_ids,chr(31))) rank_value
  FROM walk JOIN zasp_inventory_entities entry ON (entry.organization_id,entry.workspace_id,entry.environment_id,entry.id,entry.state)=(organization_value,workspace_value,environment_value,walk.node_ids[1],'active')
  WHERE entry.product_kind IN('identity','agent','tool','runtime') OR EXISTS(
   SELECT 1 FROM zasp_inventory_source_observations observation
   WHERE (observation.organization_id,observation.workspace_id,observation.environment_id,observation.entity_id,observation.source_state,observation.source_kind)=(organization_value,workspace_value,environment_value,entry.id,'present','aws_role')
  )
 ),selected AS (
  SELECT finding_id,evidence_id,node_ids,zasp_discovery_canonical_id(organization_value,workspace_value,environment_value,'attack_path',finding_id||chr(31)||array_to_string(node_ids,chr(31))) path_id
  FROM ranked WHERE rank_value=1
 )
 SELECT COALESCE(jsonb_agg(jsonb_build_object('finding_id',finding_id,'evidence_id',evidence_id,'path_id',path_id,'entry_id',node_ids[1],'sink_id',node_ids[cardinality(node_ids)],'node_ids',to_jsonb(node_ids)) ORDER BY finding_id),'[]'::jsonb) INTO path_items FROM selected;

 IF EXISTS(
  SELECT 1 FROM jsonb_array_elements(path_items) candidate JOIN zasp_risk_attack_paths path
    ON (path.organization_id,path.workspace_id,path.environment_id,path.id)=(organization_value,workspace_value,environment_value,candidate->>'path_id')
  WHERE NOT EXISTS(SELECT 1 FROM zasp_risk_findings finding WHERE (finding.organization_id,finding.workspace_id,finding.environment_id,finding.id,finding.path_id,finding.source)=(organization_value,workspace_value,environment_value,candidate->>'finding_id',path.id,'prowler') AND finding.compliance_context LIKE 'zasp-discovery:%')
    OR (path.entry_id,path.sink_id) IS DISTINCT FROM (candidate->>'entry_id',candidate->>'sink_id')
    OR (SELECT COALESCE(jsonb_agg(node.node_id ORDER BY node.position),'[]'::jsonb) FROM zasp_risk_attack_path_nodes node WHERE (node.organization_id,node.workspace_id,node.environment_id,node.path_id)=(organization_value,workspace_value,environment_value,path.id)) IS DISTINCT FROM candidate->'node_ids'
 ) THEN RAISE EXCEPTION USING ERRCODE='23505',MESSAGE='risk attack path ownership conflict';END IF;

 WITH candidates AS (SELECT candidate FROM jsonb_array_elements(path_items) candidate)
 UPDATE zasp_risk_attack_paths path SET
  state=CASE WHEN path.state='blocked' THEN 'blocked' ELSE 'observed' END,
  blocked_edge=CASE WHEN path.state='blocked' THEN path.blocked_edge ELSE -1 END,
  version=path.version+CASE WHEN path.state NOT IN('blocked','observed') OR (SELECT COALESCE(jsonb_agg(evidence.evidence_id ORDER BY evidence.position),'[]'::jsonb) FROM zasp_risk_attack_path_evidence evidence WHERE (evidence.organization_id,evidence.workspace_id,evidence.environment_id,evidence.path_id)=(organization_value,workspace_value,environment_value,path.id)) IS DISTINCT FROM jsonb_build_array(candidates.candidate->>'evidence_id') THEN 1 ELSE 0 END,
  updated_at=CASE WHEN path.state NOT IN('blocked','observed') OR (SELECT COALESCE(jsonb_agg(evidence.evidence_id ORDER BY evidence.position),'[]'::jsonb) FROM zasp_risk_attack_path_evidence evidence WHERE (evidence.organization_id,evidence.workspace_id,evidence.environment_id,evidence.path_id)=(organization_value,workspace_value,environment_value,path.id)) IS DISTINCT FROM jsonb_build_array(candidates.candidate->>'evidence_id') THEN transaction_timestamp() ELSE path.updated_at END
 FROM candidates WHERE (path.organization_id,path.workspace_id,path.environment_id,path.id)=(organization_value,workspace_value,environment_value,candidates.candidate->>'path_id');

 INSERT INTO zasp_risk_attack_paths(organization_id,workspace_id,environment_id,id,entry_id,sink_id,state)
 SELECT organization_value,workspace_value,environment_value,candidate->>'path_id',candidate->>'entry_id',candidate->>'sink_id','observed' FROM jsonb_array_elements(path_items) candidate
 ON CONFLICT(organization_id,workspace_id,environment_id,id) DO NOTHING;

 DELETE FROM zasp_risk_break_options option_value USING jsonb_array_elements(path_items) candidate WHERE (option_value.organization_id,option_value.workspace_id,option_value.environment_id,option_value.path_id)=(organization_value,workspace_value,environment_value,candidate->>'path_id');
 DELETE FROM zasp_risk_attack_path_evidence evidence_value USING jsonb_array_elements(path_items) candidate WHERE (evidence_value.organization_id,evidence_value.workspace_id,evidence_value.environment_id,evidence_value.path_id)=(organization_value,workspace_value,environment_value,candidate->>'path_id');
 DELETE FROM zasp_risk_attack_path_nodes node_value USING jsonb_array_elements(path_items) candidate WHERE (node_value.organization_id,node_value.workspace_id,node_value.environment_id,node_value.path_id)=(organization_value,workspace_value,environment_value,candidate->>'path_id');

 INSERT INTO zasp_risk_attack_path_nodes(organization_id,workspace_id,environment_id,path_id,position,node_id)
 SELECT organization_value,workspace_value,environment_value,candidate->>'path_id',node.ordinality::smallint,node.node_id
 FROM jsonb_array_elements(path_items) candidate CROSS JOIN LATERAL jsonb_array_elements_text(candidate->'node_ids') WITH ORDINALITY node(node_id,ordinality);
 INSERT INTO zasp_risk_attack_path_evidence(organization_id,workspace_id,environment_id,path_id,position,evidence_id)
 SELECT organization_value,workspace_value,environment_value,candidate->>'path_id',1,candidate->>'evidence_id' FROM jsonb_array_elements(path_items) candidate;
 INSERT INTO zasp_risk_break_options(organization_id,workspace_id,environment_id,path_id,rank,target_id,evidence_id,kind)
 SELECT organization_value,workspace_value,environment_value,candidate->>'path_id',node.ordinality::smallint,node.node_id,candidate->>'evidence_id',CASE WHEN node.ordinality=1 THEN 'remove_node' ELSE 'enforce_policy' END
 FROM jsonb_array_elements(path_items) candidate CROSS JOIN LATERAL jsonb_array_elements_text(candidate->'node_ids') WITH ORDINALITY node(node_id,ordinality);

 WITH candidates AS (SELECT candidate FROM jsonb_array_elements(path_items) candidate)
 UPDATE zasp_risk_findings finding SET path_id=candidates.candidate->>'path_id',version=finding.version+CASE WHEN finding.path_id IS DISTINCT FROM candidates.candidate->>'path_id' AND finding.updated_at<>transaction_timestamp() THEN 1 ELSE 0 END,updated_at=CASE WHEN finding.path_id IS DISTINCT FROM candidates.candidate->>'path_id' THEN transaction_timestamp() ELSE finding.updated_at END
 FROM candidates WHERE (finding.organization_id,finding.workspace_id,finding.environment_id,finding.id)=(organization_value,workspace_value,environment_value,candidates.candidate->>'finding_id');
 UPDATE zasp_risk_findings finding SET path_id=NULL,version=finding.version+1,updated_at=transaction_timestamp()
 WHERE (finding.organization_id,finding.workspace_id,finding.environment_id)=(organization_value,workspace_value,environment_value) AND finding.path_id IS NOT NULL
   AND EXISTS(SELECT 1 FROM jsonb_array_elements(items_value) item WHERE item->>'finding_id'=finding.id)
   AND NOT EXISTS(SELECT 1 FROM jsonb_array_elements(path_items) candidate WHERE candidate->>'finding_id'=finding.id);
 ...
 DELETE FROM zasp_risk_attack_paths path WHERE (path.organization_id,path.workspace_id,path.environment_id)=(organization_value,workspace_value,environment_value)
   AND path.id=ANY(old_path_ids) AND NOT EXISTS(SELECT 1 FROM zasp_risk_findings finding WHERE (finding.organization_id,finding.workspace_id,finding.environment_id,finding.path_id)=(organization_value,workspace_value,environment_value,path.id));
END $$;
...
CREATE INDEX zasp_inventory_capability_evidence_edge_v14_idx ON public.zasp_inventory_capability_evidence(organization_id,workspace_id,environment_id,agent_id,target_id,category,outcome,reachability_digest,state,observed_at DESC,evidence_id DESC);
...
ALTER TABLE public.zasp_inventory_capability_evidence OWNER TO zasp_inventory_authority;
GRANT SELECT ON public.zasp_inventory_cutover_state,public.zasp_inventory_legacy_restore,public.zasp_inventory_identity_rules,public.zasp_inventory_identity_bindings,public.zasp_inventory_annotations,public.zasp_inventory_capability_evidence TO CURRENT_USER;
DO $typed_tables$ DECLARE table_name text;BEGIN
 FOREACH table_name IN ARRAY ARRAY['zasp_inventory_cutover_state','zasp_inventory_legacy_restore','zasp_inventory_identity_rules','zasp_inventory_identity_bindings','zasp_inventory_annotations','zasp_inventory_capability_evidence'] LOOP
 ...
 END LOOP;
END $typed_tables$;
CREATE POLICY zasp_inventory_annotations_mutation ON public.zasp_inventory_annotations TO zasp_discovery_authority USING(true) WITH CHECK(true);
GRANT SELECT,INSERT,UPDATE ON public.zasp_inventory_annotations TO zasp_discovery_authority;
CREATE POLICY zasp_inventory_capability_evidence_mutation ON public.zasp_inventory_capability_evidence TO zasp_discovery_authority USING(true) WITH CHECK(true);
GRANT SELECT,INSERT ON public.zasp_inventory_capability_evidence TO zasp_discovery_authority;
...
CREATE FUNCTION public.zasp_inventory_security_ready() RETURNS boolean LANGUAGE sql STABLE AS $$
 ...
 AND (SELECT count(*) FROM aclexplode(COALESCE((SELECT relacl FROM pg_class WHERE oid='public.zasp_risk_attack_paths'::regclass),acldefault('r',(SELECT relowner FROM pg_class WHERE oid='public.zasp_risk_attack_paths'::regclass)))) acl WHERE acl.grantee=(SELECT oid FROM pg_roles WHERE rolname='zasp_discovery_authority') AND acl.privilege_type=ANY(ARRAY['SELECT','INSERT','UPDATE','DELETE']) AND NOT acl.is_grantable)=4
 AND NOT EXISTS(SELECT 1 FROM aclexplode(COALESCE((SELECT relacl FROM pg_class WHERE oid='public.zasp_risk_attack_paths'::regclass),acldefault('r',(SELECT relowner FROM pg_class WHERE oid='public.zasp_risk_attack_paths'::regclass)))) acl WHERE acl.grantee=(SELECT oid FROM pg_roles WHERE rolname='zasp_discovery_authority') AND (acl.privilege_type<>ALL(ARRAY['SELECT','INSERT','UPDATE','DELETE']) OR acl.is_grantable))
 AND (SELECT count(*) FROM pg_class class JOIN pg_namespace namespace ON namespace.oid=class.relnamespace WHERE namespace.nspname='public' AND class.relname=ANY(ARRAY['zasp_inventory_cutover_state','zasp_inventory_legacy_restore','zasp_inventory_identity_rules','zasp_inventory_identity_bindings','zasp_inventory_annotations','zasp_inventory_capability_evidence']) AND class.relowner=(SELECT oid FROM pg_roles WHERE rolname='zasp_inventory_authority') AND class.relrowsecurity AND class.relforcerowsecurity)=6
 AND NOT EXISTS(SELECT 1 FROM pg_class class JOIN pg_namespace namespace ON namespace.oid=class.relnamespace WHERE namespace.nspname='public' AND class.relname=ANY(ARRAY['zasp_inventory_cutover_state','zasp_inventory_legacy_restore','zasp_inventory_identity_rules','zasp_inventory_identity_bindings','zasp_inventory_annotations','zasp_inventory_capability_evidence']) AND ((SELECT count(*) FROM pg_policy policy WHERE policy.polrelid=class.oid)<>CASE WHEN class.relname IN('zasp_inventory_annotations','zasp_inventory_capability_evidence') THEN 2 ELSE 1 END OR NOT EXISTS(SELECT 1 FROM pg_policy policy WHERE policy.polrelid=class.oid AND policy.polname=class.relname||'_authority' AND policy.polpermissive AND policy.polcmd='*' AND policy.polroles=ARRAY[(SELECT oid FROM pg_roles WHERE rolname='zasp_inventory_authority')] AND pg_get_expr(policy.polqual,policy.polrelid)='true' AND pg_get_expr(policy.polwithcheck,policy.polrelid)='true') OR class.relname='zasp_inventory_annotations' AND NOT EXISTS(SELECT 1 FROM pg_policy policy WHERE policy.polrelid=class.oid AND policy.polname='zasp_inventory_annotations_mutation' AND policy.polpermissive AND policy.polcmd='*' AND policy.polroles=ARRAY[(SELECT oid FROM pg_roles WHERE rolname='zasp_discovery_authority')] AND pg_get_expr(policy.polqual,policy.polrelid)='true' AND pg_get_expr(policy.polwithcheck,policy.polrelid)='true') OR class.relname='zasp_inventory_capability_evidence' AND NOT EXISTS(SELECT 1 FROM pg_policy policy WHERE policy.polrelid=class.oid AND policy.polname='zasp_inventory_capability_evidence_mutation' AND policy.polpermissive AND policy.polcmd='*' AND policy.polroles=ARRAY[(SELECT oid FROM pg_roles WHERE rolname='zasp_discovery_authority')] AND pg_get_expr(policy.polqual,policy.polrelid)='true' AND pg_get_expr(policy.polwithcheck,policy.polrelid)='true') OR EXISTS(SELECT 1 FROM aclexplode(COALESCE(class.relacl,acldefault('r',class.relowner))) acl WHERE acl.grantee<>class.relowner AND NOT (class.relname='zasp_inventory_annotations' AND acl.grantee=(SELECT oid FROM pg_roles WHERE rolname='zasp_discovery_authority') AND acl.privilege_type=ANY(ARRAY['SELECT','INSERT','UPDATE']) AND NOT acl.is_grantable) AND NOT (class.relname='zasp_inventory_capability_evidence' AND acl.grantee=(SELECT oid FROM pg_roles WHERE rolname='zasp_discovery_authority') AND acl.privilege_type=ANY(ARRAY['SELECT','INSERT']) AND NOT acl.is_grantable))))
 AND (SELECT count(*) FROM zasp_inventory_identity_rules)=30
 AND (SELECT count(*) FROM pg_proc procedure JOIN pg_namespace namespace ON namespace.oid=procedure.pronamespace WHERE namespace.nspname='public' AND procedure.proname LIKE 'zasp_inventory_%')=24
 AND NOT EXISTS(SELECT 1 FROM pg_proc procedure JOIN pg_namespace namespace ON namespace.oid=procedure.pronamespace WHERE namespace.nspname='public' AND procedure.proname LIKE 'zasp_inventory_%' AND procedure.proname NOT IN('zasp_inventory_readiness','zasp_inventory_backfill_scope','zasp_inventory_compat_read','zasp_inventory_equivalence_scope','zasp_inventory_cutover_scope','zasp_inventory_page','zasp_inventory_detail','zasp_inventory_update_agent','zasp_inventory_agent_capability_edges','zasp_inventory_record_capability_evidence','zasp_inventory_agent_capabilities_page','zasp_inventory_agent_relationships_page','zasp_inventory_agent_sessions_page','zasp_inventory_home_summary','zasp_inventory_apply_findings','zasp_inventory_refresh_posture_findings') AND (procedure.proowner<>(SELECT oid FROM pg_roles WHERE rolname='zasp_inventory_authority') OR NOT procedure.prosecdef OR NOT COALESCE(procedure.proconfig,'{}') @> ARRAY['search_path=pg_catalog, public'] OR EXISTS(SELECT 1 FROM aclexplode(COALESCE(procedure.proacl,acldefault('f',procedure.proowner))) acl WHERE acl.privilege_type='EXECUTE' AND acl.grantee NOT IN(procedure.proowner,(SELECT oid FROM pg_roles WHERE rolname='zasp_discovery_authority')))))
 AND (SELECT count(*) FROM pg_proc procedure JOIN pg_namespace namespace ON namespace.oid=procedure.pronamespace WHERE namespace.nspname='public' AND procedure.proname IN('zasp_inventory_readiness','zasp_inventory_backfill_scope','zasp_inventory_compat_read','zasp_inventory_equivalence_scope','zasp_inventory_cutover_scope','zasp_inventory_page','zasp_inventory_detail','zasp_inventory_update_agent','zasp_inventory_agent_capability_edges','zasp_inventory_record_capability_evidence','zasp_inventory_agent_capabilities_page','zasp_inventory_agent_relationships_page','zasp_inventory_agent_sessions_page','zasp_inventory_home_summary','zasp_inventory_apply_findings','zasp_inventory_refresh_posture_findings') AND procedure.proowner=(SELECT oid FROM pg_roles WHERE rolname='zasp_discovery_authority') AND procedure.prosecdef AND COALESCE(procedure.proconfig,'{}') @> ARRAY['search_path=pg_catalog, public'] AND NOT EXISTS(SELECT 1 FROM aclexplode(COALESCE(procedure.proacl,acldefault('f',procedure.proowner))) acl WHERE acl.privilege_type='EXECUTE' AND acl.grantee<>procedure.proowner AND NOT (procedure.proname IN('zasp_inventory_page','zasp_inventory_detail','zasp_inventory_update_agent','zasp_inventory_agent_capabilities_page','zasp_inventory_agent_relationships_page','zasp_inventory_agent_sessions_page','zasp_inventory_home_summary') AND acl.grantee=(SELECT oid FROM pg_roles WHERE rolname='zasp_discovery_api')) AND NOT (procedure.proname='zasp_inventory_readiness' AND acl.grantee IN(SELECT oid FROM pg_roles WHERE rolname=ANY(ARRAY['zasp_discovery_api','zasp_discovery_worker','zasp_runtime_ingest','zasp_runtime_worker','zasp_outbox_worker','zasp_runtime_gateway','zasp_discovery_scheduler','zasp_projection_risk_worker','zasp_projection_graph_worker','zasp_projection_search_worker'])))))=16
 ...
 AND (SELECT count(*) FROM pg_class class CROSS JOIN LATERAL aclexplode(COALESCE(class.relacl,acldefault('r',class.relowner))) acl WHERE class.oid IN('zasp_risk_findings'::regclass,'zasp_risk_finding_evidence'::regclass,'zasp_risk_finding_factors'::regclass,'zasp_risk_attack_paths'::regclass,'zasp_risk_attack_path_nodes'::regclass,'zasp_risk_attack_path_evidence'::regclass,'zasp_risk_break_options'::regclass) AND acl.grantee=(SELECT oid FROM pg_roles WHERE rolname='zasp_discovery_authority') AND acl.privilege_type=ANY(ARRAY['SELECT','INSERT','UPDATE','DELETE']) AND NOT acl.is_grantable)=28
 AND NOT EXISTS(SELECT 1 FROM pg_class class CROSS JOIN LATERAL aclexplode(COALESCE(class.relacl,acldefault('r',class.relowner))) acl WHERE class.oid IN('zasp_risk_findings'::regclass,'zasp_risk_finding_evidence'::regclass,'zasp_risk_finding_factors'::regclass,'zasp_risk_attack_paths'::regclass,'zasp_risk_attack_path_nodes'::regclass,'zasp_risk_attack_path_evidence'::regclass,'zasp_risk_break_options'::regclass) AND acl.grantee=(SELECT oid FROM pg_roles WHERE rolname='zasp_discovery_authority') AND (acl.privilege_type<>ALL(ARRAY['SELECT','INSERT','UPDATE','DELETE']) OR acl.is_grantable))
...
DO $authority$ DECLARE procedure_oid oid;BEGIN
 FOR procedure_oid IN SELECT procedure.oid FROM pg_proc procedure JOIN pg_namespace namespace ON namespace.oid=procedure.pronamespace WHERE namespace.nspname='public' AND procedure.proname LIKE 'zasp_inventory_%' LOOP
  ...
  IF procedure_oid NOT IN('public.zasp_inventory_readiness(text,text)'::regprocedure,'public.zasp_inventory_backfill_scope(text,text,text)'::regprocedure,'public.zasp_inventory_compat_read(text,text,text,text)'::regprocedure,'public.zasp_inventory_equivalence_scope(text,text,text)'::regprocedure,'public.zasp_inventory_cutover_scope(text,text,text)'::regprocedure,'public.zasp_inventory_page(text,text,text,text,text,integer)'::regprocedure,'public.zasp_inventory_detail(text,text,text,text,text)'::regprocedure,'public.zasp_inventory_update_agent(text,text,text,text,text,text,bigint,text,text,jsonb,text,text)'::regprocedure,'public.zasp_inventory_agent_capability_edges(text,text,text,text)'::regprocedure,'public.zasp_inventory_record_capability_evidence(text,text,text,text,text,text,text,text,text,timestamptz)'::regprocedure,'public.zasp_inventory_agent_capabilities_page(text,text,text,text,text,integer)'::regprocedure,'public.zasp_inventory_agent_relationships_page(text,text,text,text,text,integer)'::regprocedure,'public.zasp_inventory_agent_sessions_page(text,text,text,text,text,integer)'::regprocedure,'public.zasp_inventory_home_summary(text,text,text)'::regprocedure,'public.zasp_inventory_apply_findings(text,text,text,jsonb)'::regprocedure,'public.zasp_inventory_refresh_posture_findings(text,text,text)'::regprocedure) THEN
  ...
  ELSE
   ...
   IF procedure_oid IN('public.zasp_inventory_page(text,text,text,text,text,integer)'::regprocedure,'public.zasp_inventory_detail(text,text,text,text,text)'::regprocedure,'public.zasp_inventory_update_agent(text,text,text,text,text,text,bigint,text,text,jsonb,text,text)'::regprocedure,'public.zasp_inventory_agent_capabilities_page(text,text,text,text,text,integer)'::regprocedure,'public.zasp_inventory_agent_relationships_page(text,text,text,text,text,integer)'::regprocedure,'public.zasp_inventory_agent_sessions_page(text,text,text,text,text,integer)'::regprocedure,'public.zasp_inventory_home_summary(text,text,text)'::regprocedure) THEN
    EXECUTE format('GRANT EXECUTE ON FUNCTION %s TO zasp_discovery_api',procedure_oid::regprocedure);
   ELSIF procedure_oid='public.zasp_inventory_readiness(text,text)'::regprocedure THEN
    EXECUTE format('GRANT EXECUTE ON FUNCTION %s TO zasp_discovery_api,zasp_discovery_worker,zasp_runtime_ingest,zasp_runtime_worker,zasp_outbox_worker,zasp_runtime_gateway,zasp_discovery_scheduler,zasp_projection_risk_worker,zasp_projection_graph_worker,zasp_projection_search_worker',procedure_oid::regprocedure);
   END IF;
  END IF;
 END LOOP;
END $authority$;

GRANT SELECT ON public.zasp_risk_attack_paths TO zasp_discovery_authority;
...
REVOKE ALL ON public.zasp_inventory_cutover_state,public.zasp_inventory_legacy_restore,public.zasp_inventory_identity_rules,public.zasp_inventory_identity_bindings,public.zasp_inventory_annotations,public.zasp_inventory_capability_evidence FROM CURRENT_USER;
...
INSERT INTO zasp_schema_metadata(key,value) VALUES
 ...
 ('typed_inventory_cutover_fingerprint', 'f2e8b9fec3df4a18b15b2be0c1843bc294626902eab1ccbb18b7fa72f8157a27');
