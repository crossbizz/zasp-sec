...
const productionRuntimeCorrelationRoutingReadinessSQL = `SELECT zasp_production_runtime_correlation_routing_readiness($1,$2)`

//go:embed sql/0049_production_runtime_correlation_routing.up.sql
var productionRuntimeCorrelationRoutingUpSQL string

//go:embed sql/0049_production_runtime_correlation_routing.down.sql
var productionRuntimeCorrelationRoutingDownSQL string

func ProductionRuntimeCorrelationRouting() Metadata {
	up, down := strings.TrimSpace(productionRuntimeCorrelationRoutingUpSQL), strings.TrimSpace(productionRuntimeCorrelationRoutingDownSQL)
	digest := sha256.Sum256([]byte(up + "\x00" + down))
	return Metadata{version: 49, name: "production_runtime_correlation_routing", checksum: hex.EncodeToString(digest[:]), up: up, down: down}
}

func ProductionRuntimeCorrelationRoutingSemanticFingerprint() string {
	return semanticFingerprint(productionRuntimeCorrelationRoutingUpSQL, "production_runtime_correlation_routing_fingerprint")
}

func (runner *Runner) UpProductionRuntimeCorrelationRouting(ctx context.Context) error {
	...
	return runner.withTransaction(ctx, func(ctx context.Context, transaction Transaction) error {
		...
		prior := ProductionRuntimeAcceptance()
		if err := requireMigrationReadiness(ctx, transaction, productionRuntimeAcceptanceReadinessSQL, prior.Checksum(), ProductionRuntimeAcceptanceSemanticFingerprint()); err != nil {
			return err
		}
		metadata := ProductionRuntimeCorrelationRouting()
		...
		if err := transaction.Exec(ctx, `INSERT INTO public.zasp_schema_metadata(key,value) VALUES('production_runtime_correlation_routing_checksum',$1)`, metadata.Checksum()); err != nil {
			return fixedDatabaseError(ctx, err)
		}
		...
		return requireMigrationReadiness(ctx, transaction, productionRuntimeCorrelationRoutingReadinessSQL, metadata.Checksum(), ProductionRuntimeCorrelationRoutingSemanticFingerprint())
	})
}

func (runner *Runner) DownProductionRuntimeCorrelationRouting(ctx context.Context) error {
	if runner == nil || nilInterface(runner.database) {
		return ErrInvalidRunner
	}
	return runner.withTransaction(ctx, func(ctx context.Context, transaction Transaction) error {
		...
		metadata := ProductionRuntimeCorrelationRouting()
		if err := requireMigrationReadiness(ctx, transaction, productionRuntimeCorrelationRoutingReadinessSQL, metadata.Checksum(), ProductionRuntimeCorrelationRoutingSemanticFingerprint()); err != nil {
			return err
		}
