...
func TestRunnerVersionDistinguishesEmptyBaselineCoreWorkflowsReceiptsAndDrift(t *testing.T) {
	...
	reveal, risk, discovery, connector, reference, execution, typed, runtime := APITokenRevealGrants(), ProductionRiskProjection(), ProductionDiscovery(), ConnectorAuthorization(), ReferenceAuthorization(), ProductionDiscoveryExecution(), ProductionTypedInventoryCutover(), ProductionRuntimeDataPlane()
	for _, test := range []struct {
		...
		{name: "drift", rows: []Row{fakeRow{values: []any{true}}, fakeRow{values: []any{int64(16)}}}, wantErr: ErrInvalidState},
	} {
