...
func TestRunnerVersionDistinguishesEmptyBaselineCoreWorkflowsReceiptsAndDrift(t *testing.T) {
	...
	reveal, risk, discovery, connector, reference, execution, typed := APITokenRevealGrants(), ProductionRiskProjection(), ProductionDiscovery(), ConnectorAuthorization(), ReferenceAuthorization(), ProductionDiscoveryExecution(), ProductionTypedInventoryCutover()
	for _, test := range []struct {
		...
		{name: "drift", rows: []Row{fakeRow{values: []any{true}}, fakeRow{values: []any{int64(15)}}}, wantErr: ErrInvalidState},
	} {
