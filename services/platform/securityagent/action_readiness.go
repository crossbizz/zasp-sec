...
var productionActionReadiness = []ActionReadiness{
	...
	{Key: "create_temporary_policy", ProductionState: actionProductionAvailable, MaximumAutonomy: string(AutonomySupervised), Provider: "runtime_policy", Reversible: true, ApprovalFloor: "operator", VerificationKind: "policy_state", CleanupKind: "ttl_disable", CurrentEvidence: "v22 tenant-scoped supervised execution with signed gateway readback, credential-rotation reconciliation, TTL cleanup, and PostgreSQL E2E"},
	{Key: "isolate_session", ProductionState: actionProductionAvailable, MaximumAutonomy: string(AutonomySupervised), Provider: "runtime_gateway", Reversible: true, ApprovalFloor: "operator", VerificationKind: "gateway_decision", CleanupKind: "ttl_release", CurrentEvidence: "v24 tenant-scoped supervised execution with exact session policy enforcement, unrelated-session allowance, durable gateway readback, and TTL cleanup"},
	...
	{Key: "revoke_integration_connection", ProductionState: actionProductionAvailable, MaximumAutonomy: string(AutonomySupervised), Provider: "connector_broker", Reversible: false, ApprovalFloor: "admin", VerificationKind: "connection_state", CleanupKind: "provider_reconciliation", CurrentEvidence: "v23 tenant-scoped supervised execution through the existing connector reconciler with durable connection-state verification"},
