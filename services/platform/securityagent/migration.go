...
CREATE POLICY security_agents_tenant ON security_agents USING (organization_id = current_setting('zasp.organization_id', true));
CREATE POLICY security_agent_runs_tenant ON security_agent_runs USING (organization_id = current_setting('zasp.organization_id', true));
CREATE POLICY security_agent_steps_tenant ON security_agent_steps USING (organization_id = current_setting('zasp.organization_id', true));
CREATE POLICY security_agent_approvals_tenant ON security_agent_approvals USING (organization_id = current_setting('zasp.organization_id', true));
CREATE POLICY security_action_idempotency_tenant ON security_action_idempotency USING (organization_id = current_setting('zasp.organization_id', true));
`
...
func ValidateMigration(source string) error {
	...
	for _, table := range []string{"security_agents", "security_agent_runs", "security_agent_steps", "security_agent_approvals", "security_action_idempotency"} {
		if strings.Count(source, "CREATE TABLE IF NOT EXISTS "+table+" ") != 1 || strings.Count(source, "ALTER TABLE "+table+" ENABLE ROW LEVEL SECURITY;") != 1 || !strings.Contains(source, "CREATE POLICY "+table+"_tenant ON "+table+" USING (organization_id = current_setting('zasp.organization_id', true));") {
			return ErrRejected
		}
	}
	return nil
