...
const CoreSchemaVersion = "production-workflow-receipt-provenance-v3"
...
const (
	...
	postgresAuthenticatePATSQL     = `SELECT jsonb_build_object('principal_id', token.principal_id, 'organization_id', token.organization_id, 'workspace_id', token.workspace_id, 'environment_id', token.environment_id, 'permissions', zasp_effective_scope_permissions(scope.permissions, membership.role)) FROM zasp_product_api_tokens AS token JOIN zasp_identity_memberships AS membership ON membership.principal_id = token.principal_id AND membership.organization_id = token.organization_id AND membership.active JOIN zasp_authorized_scopes AS scope ON scope.principal_id = token.principal_id AND scope.organization_id = token.organization_id AND scope.workspace_id = token.workspace_id AND scope.environment_id = token.environment_id WHERE token.token_digest = digest($1, 'sha256') AND token.revoked_at IS NULL AND token.expires_at > now()`
