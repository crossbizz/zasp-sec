...
func newProductionHandlers(repository, securityAgentRepository *PostgresRepository, provider CallbackProvider, connector http.Handler, cookie CookiePolicy) (Dependencies, Authenticator, error) {
	...
	if securityAgentSchema {
		securityAgentDefinitions, securityAgentErr := newWorkflowHTTPHandler(securityAgentRepository, cookie.WorkflowSigningKey, cookie.Clock, workflowCapabilities)
	...
	if stringIn(repository.schema, RedTeamExecutionSchemaVersion, AttackLabExecutionSchemaVersion, ProductionRecoverySchemaVersion) {
		redTeamHandler, redTeamErr := NewRedTeamPublicHTTPHandler(securityAgentRepository, cookie.WorkflowSigningKey)
		if redTeamErr != nil {
			return Dependencies{}, nil, ErrRepositoryConfiguration
		}
...
func capabilitiesForPermissions(permissions []string) []string {
	...
	for _, permission := range permissions {
		...
		case "run_tests":
			capabilities = append(capabilities, "red-team.write")
		}
	}
	return capabilities
}
