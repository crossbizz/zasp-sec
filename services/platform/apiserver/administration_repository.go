...
const (
	...
	postgresRevealGrantCleanupSQL        = `WITH candidates AS MATERIALIZED (SELECT reveal.organization_id,reveal.workspace_id,reveal.environment_id,reveal.principal_id,reveal.grant_id,reveal.token_id,reveal.expires_at FROM zasp_api_token_reveal_grants AS reveal WHERE reveal.acknowledged_at IS NULL ORDER BY reveal.organization_id,reveal.workspace_id,reveal.environment_id,reveal.principal_id,reveal.grant_id FOR UPDATE OF reveal SKIP LOCKED LIMIT $1), cleaned AS (UPDATE zasp_api_token_reveal_grants AS reveal SET acknowledged_at=transaction_timestamp(),ciphertext=NULL,nonce=NULL,authentication_tag=NULL FROM candidates LEFT JOIN zasp_product_api_tokens AS token ON token.organization_id=candidates.organization_id AND token.id=candidates.token_id WHERE reveal.organization_id=candidates.organization_id AND reveal.grant_id=candidates.grant_id AND reveal.acknowledged_at IS NULL AND (candidates.expires_at<=transaction_timestamp() OR token.id IS NULL OR token.revoked_at IS NOT NULL OR token.expires_at<=transaction_timestamp()) RETURNING 1) SELECT jsonb_build_object('cleaned',count(*)) FROM cleaned`
...
func (repository *PostgresRepository) ReadAdministration(ctx context.Context, identity RequestIdentity, operation string, parameters map[string]string) (json.RawMessage, error) {
	...
	case "listEnvironments":
		return repository.database.QueryJSON(ctx, postgresListEnvironmentsSQL, identity.Scope.OrganizationID().String(), parameters["workspace_id"], identity.PrincipalID.String(), parameters["after_id"], adminLimit(parameters)+1)
	case "getEnvironment":
