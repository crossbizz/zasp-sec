...
const (
	...
	postgresListWorkspacesSQL            = `SELECT jsonb_build_object('items', COALESCE(jsonb_agg(item ORDER BY item->>'id'), '[]'::jsonb)) FROM (SELECT jsonb_build_object('id', id, 'organization_id', organization_id, 'name', name, 'version', version) AS item FROM zasp_workspaces WHERE organization_id=$1 AND ($2='' OR id>$2) ORDER BY id LIMIT $3) AS page`
	...
	postgresListSessionEventsSQL         = `SELECT jsonb_build_object('items', COALESCE(jsonb_agg(item ORDER BY item->>'at',item->>'id'), '[]'::jsonb)) FROM (SELECT jsonb_build_object('id', id, 'session_id', session_id, 'class', class, 'label', label, 'evidence_id', evidence_id, 'source', source, 'confidence', confidence, 'at', at) AS item FROM zasp_session_events WHERE organization_id=$1 AND session_id=$2 AND EXISTS (SELECT 1 FROM zasp_product_sessions WHERE organization_id=$1 AND workspace_id=$3 AND environment_id=$4 AND session_id=$2) AND ($5::timestamptz IS NULL OR (at,id)>($5,$6)) ORDER BY at,id LIMIT $7) page`
	postgresListComplianceControlsSQL    = `SELECT jsonb_build_object('items', COALESCE(jsonb_agg(item ORDER BY item->>'id'), '[]'::jsonb)) FROM (SELECT jsonb_build_object('id', control.id, 'framework', control.framework, 'name', control.name, 'evidence_ids', COALESCE((SELECT jsonb_agg(evidence.id ORDER BY evidence.id) FROM zasp_compliance_evidence AS evidence WHERE evidence.organization_id=control.organization_id AND evidence.control_id=control.id), '[]'::jsonb), 'fresh_until', control.fresh_until) AS item FROM zasp_compliance_controls AS control WHERE organization_id=$1 AND ($2='' OR control.id>$2) ORDER BY control.id LIMIT $3) page`
	postgresListComplianceEvidenceSQL    = `SELECT jsonb_build_object('items', COALESCE(jsonb_agg(item ORDER BY item#>>'{control,id}'), '[]'::jsonb)) FROM (SELECT jsonb_build_object('control', jsonb_build_object('id', control.id, 'framework', control.framework, 'name', control.name, 'evidence_ids', COALESCE((SELECT jsonb_agg(evidence.id ORDER BY evidence.id) FROM zasp_compliance_evidence AS evidence WHERE evidence.organization_id=control.organization_id AND evidence.control_id=control.id), '[]'::jsonb), 'fresh_until', control.fresh_until), 'freshness', CASE WHEN NOT EXISTS (SELECT 1 FROM zasp_compliance_evidence AS evidence WHERE evidence.organization_id=control.organization_id AND evidence.control_id=control.id) THEN 'missing' WHEN control.fresh_until > now() THEN 'fresh' ELSE 'stale' END, 'evidence', COALESCE((SELECT jsonb_agg(jsonb_build_object('id', evidence.id, 'asset_id', evidence.asset_id, 'source', evidence.source, 'at', evidence.at) ORDER BY evidence.at,evidence.id) FROM zasp_compliance_evidence AS evidence WHERE evidence.organization_id=control.organization_id AND evidence.control_id=control.id), '[]'::jsonb)) AS item FROM zasp_compliance_controls AS control WHERE organization_id=$1 AND ($2='' OR control.id>$2) ORDER BY control.id LIMIT $3) page`
...
func (repository *PostgresRepository) ReadAdministration(ctx context.Context, identity RequestIdentity, operation string, parameters map[string]string) (json.RawMessage, error) {
	...
	case "listWorkspaces":
		return repository.database.QueryJSON(ctx, postgresListWorkspacesSQL, identity.Scope.OrganizationID().String(), parameters["after_id"], adminLimit(parameters)+1)
	case "getWorkspace":
	...
	case "listComplianceEvidence":
		return repository.database.QueryJSON(ctx, postgresListComplianceEvidenceSQL, identity.Scope.OrganizationID().String(), parameters["after_id"], adminLimit(parameters)+1)
...
func (repository *PostgresRepository) MutateAdministration(ctx context.Context, identity RequestIdentity, mutation administrationMutation) (json.RawMessage, error) {
	...
	case "createEnvironment":
		...
		return repository.database.QueryJSON(ctx, postgresCreateEnvironmentSQL, mutation.ID, organization, mutation.WorkspaceID, mutation.Name, mutation.AuditID, actor, workspace, environment)
