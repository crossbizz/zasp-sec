...
func TestWorkflowHandlerHidesAndRejectsConnectorRevocationWithoutV23Authority(t *testing.T) {
	identity := fixtureRequestIdentity(t)
	available := false
	repository := &workflowRepositoryStub{connectorRevocationAvailable: &available}
	handler, err := newWorkflowHTTPHandler(repository, []byte("0123456789abcdef0123456789abcdef"), time.Now)
	if err != nil {
		t.Fatal(err)
	}
	...
	response := httptest.NewRecorder()
	handler.ServeHTTP(response, request)
	if response.Code != http.StatusOK || strings.Contains(response.Body.String(), `"key":"revoke_integration_connection"`) {
		t.Fatalf("legacy action catalog = %d %s", response.Code, response.Body.String())
	}

	body := `{"name":"Revoke compromised integration","trigger_kind":"finding","trigger_source":"credential","environment_ids":["pid_10000003-0000-4000-8000-000000000003"],"autonomy":"supervised","max_steps":1,"max_duration_seconds":900,"temporary_policy_seconds":3600,"ai_token_budget":4000,"concurrency_limit":1,"allowed_actions":["revoke_integration_connection"],"verification_kind":"connection_state","definition_version":1,"enabled":false}`
	request = workflowRequest(t, identity, testCorrelationID, "createSecurityAgent", nil, http.MethodPost, "/api/v1/security-agents", body)
	request.Header.Set("Idempotency-Key", "idem-reject-v23-action-on-v22")
	response = httptest.NewRecorder()
	handler.ServeHTTP(response, request)
	if response.Code != http.StatusBadRequest || repository.mutationCalls != 0 {
		t.Fatalf("legacy definition response = %d %s mutations=%d", response.Code, response.Body.String(), repository.mutationCalls)
	}
}
