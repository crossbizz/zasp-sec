...
const postgresSecurityAgentAutonomousSchemaVersionSQL = `SELECT metadata.value
FROM zasp_schema_metadata AS metadata
JOIN zasp_schema_versions AS release ON release.version = 21 AND release.name = 'security_agent_autonomous_response'
WHERE metadata.key = 'production_core_schema' AND metadata.value = 'security-agent-autonomous-v1'
  AND zasp_security_agent_autonomous_readiness($1, $2)
  AND NOT EXISTS (SELECT 1 FROM zasp_schema_versions newer WHERE newer.version > 21)`

const postgresSecurityAgentTemporaryPolicySchemaVersionSQL = `SELECT metadata.value
FROM zasp_schema_metadata AS metadata
JOIN zasp_schema_versions AS release ON release.version = 22 AND release.name = 'security_agent_temporary_policy'
WHERE metadata.key = 'production_core_schema' AND metadata.value = 'security-agent-temporary-policy-v1'
  AND zasp_security_agent_temporary_policy_readiness($1, $2)
  AND NOT EXISTS (SELECT 1 FROM zasp_schema_versions newer WHERE newer.version > 22)`

const postgresSecurityAgentConnectorRevocationSchemaVersionSQL = `SELECT metadata.value
FROM zasp_schema_metadata AS metadata
JOIN zasp_schema_versions AS release ON release.version = 23 AND release.name = 'security_agent_connector_revocation'
WHERE metadata.key = 'production_core_schema' AND metadata.value = 'security-agent-connector-revocation-v1'
...
const postgresAttackLabExecutionSchemaVersionSQL = `SELECT metadata.value
FROM zasp_schema_metadata AS metadata
JOIN zasp_schema_versions AS release ON release.version = 26 AND release.name = 'attack_lab_execution'
WHERE metadata.key = 'production_core_schema' AND metadata.value = 'attack-lab-execution-v1'
...
WHERE metadata.key = 'production_core_schema' AND metadata.value = 'production-recovery-v1'
  ...
  AND NOT EXISTS (SELECT 1 FROM zasp_schema_versions newer WHERE newer.version > 43)`
...
func expectedSecurityAgentAutonomousSchemaChecksum() string {
	return migrations.ProductionSecurityAgentAutonomousResponse().Checksum()
}
func expectedSecurityAgentAutonomousSchemaFingerprint() string {
	return migrations.ProductionSecurityAgentAutonomousResponseSemanticFingerprint()
}
func expectedSecurityAgentTemporaryPolicySchemaChecksum() string {
	return migrations.ProductionSecurityAgentTemporaryPolicy().Checksum()
}
func expectedSecurityAgentTemporaryPolicySchemaFingerprint() string {
	return migrations.ProductionSecurityAgentTemporaryPolicySemanticFingerprint()
}
func expectedSecurityAgentConnectorRevocationSchemaChecksum() string {
	return migrations.ProductionSecurityAgentConnectorRevocation().Checksum()
}
...
func expectedAttackLabExecutionSchemaChecksum() string {
	return migrations.ProductionAttackLabExecution().Checksum()
}
...
func (database *PostgresJSONDatabase) SchemaVersion(ctx context.Context) (string, error) {
	...
	} else if marker == RedTeamExecutionSchemaVersion {
	...
	} else if marker == SecurityAgentTemporaryPolicySchemaVersion {
		if err := database.driver.QueryRow(ctx, postgresSecurityAgentTemporaryPolicySchemaVersionSQL, expectedSecurityAgentTemporaryPolicySchemaChecksum(), expectedSecurityAgentTemporaryPolicySchemaFingerprint()).Scan(&version); err != nil {
			return "", classifyPostgresError(err)
		}
	} else if marker == SecurityAgentAutonomousSchemaVersion {
		if err := database.driver.QueryRow(ctx, postgresSecurityAgentAutonomousSchemaVersionSQL, expectedSecurityAgentAutonomousSchemaChecksum(), expectedSecurityAgentAutonomousSchemaFingerprint()).Scan(&version); err != nil {
			return "", classifyPostgresError(err)
		}
	} else if marker == SecurityAgentControlsSchemaVersion {
