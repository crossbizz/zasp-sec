...
func TestRuntimeSessionReadProjectionBackfillAuthorityAndReplay(t *testing.T) {
	...
	if err := admin.QueryRow(ctx, postgresProductionRecoverySchemaVersionSQL, expectedProductionRecoverySchemaChecksum(), expectedProductionRecoverySchemaFingerprint()).Scan(&apiSchema); err != nil || apiSchema != ProductionRecoverySchemaVersion {
