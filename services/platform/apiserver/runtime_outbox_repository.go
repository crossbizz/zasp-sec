...
const (
	postgresRuntimeOutboxReadySQL     = `SELECT jsonb_build_object('ready',zasp_runtime_data_plane_readiness($1,$2) AND zasp_discovery_principal_ready($3))`
...
func (repository *RuntimeOutboxRepository) Ready(ctx context.Context) error {
	...
	payload, err := repository.database.QueryJSON(ctx, postgresRuntimeOutboxReadySQL, migrations.ProductionRuntimeDataPlane().Checksum(), migrations.ProductionRuntimeDataPlaneSemanticFingerprint(), DiscoveryDatabaseAuthorityOutbox)
	var result struct {
