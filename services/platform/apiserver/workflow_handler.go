...
type workflowSecurityAgentCapabilities interface {
	...
	SecurityAgentSessionIsolationAvailable() bool
}
...
func workflowMutationTarget(operation string) (kind, action string, status int, ok bool) {
	...
	case "completeIntegrationOAuth", "remediateIntegrationAuthorization":
		return "integration", "update", http.StatusOK, true
...
func servedWorkflowActions(values []string) bool {
	return servedWorkflowActionsWithCapabilities(values, true, true)
}

func servedWorkflowActionsWithCapabilities(values []string, connectorRevocationAvailable, sessionIsolationAvailable bool) bool {
	return servedWorkflowActionsAtAutonomyWithCapabilities(values, string(securityagent.AutonomySupervised), connectorRevocationAvailable, sessionIsolationAvailable)
}

func servedWorkflowActionsAtAutonomy(values []string, autonomy string) bool {
	return servedWorkflowActionsAtAutonomyWithCapabilities(values, autonomy, true, true)
}

func servedWorkflowActionsAtAutonomyWithCapabilities(values []string, autonomy string, connectorRevocationAvailable, sessionIsolationAvailable bool) bool {
	requested := securityagent.Autonomy(autonomy)
	for _, value := range values {
		if value == "revoke_integration_connection" && !connectorRevocationAvailable {
			return false
		}
		if value == "isolate_session" && !sessionIsolationAvailable {
			return false
		}
