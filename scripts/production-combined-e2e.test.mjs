...
test("combined production E2E owns every local boundary and fixed assertion", async () => {
  ...
  for (const value of [
    ...
    "Task6 reload and deletion left no enrollment credential in persistent browser state", "zasp_runtime_sensor_heartbeat",
		...
		"agentsecctl", "schema 27 production_recovery verified", "schema 28 production_policy_deployment verified", "schema 29 production_home_attention verified", "schema 30 production_approval_notification verified", "ZASP_POLICY_DEPLOYMENT_DB_PRINCIPAL", "ZASP_RECOVERY_WORKER_DB_PRINCIPAL", "ZASP_RECOVERY_OUTBOX_DB_PRINCIPAL",
