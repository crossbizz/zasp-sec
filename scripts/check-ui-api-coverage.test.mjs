...
test("all three future public operations resolve and deliberate removal fails", async () => {
...
test("API-available operations require OpenAPI but do not claim a wired UI", async () => {
  ...
  assert.deepEqual(result, { planned: 2, apiAvailable: 48, available: 20, public: 68, internal: 0 });
...
test("strict YAML parsing rejects representation and map-schema ambiguity", async () => {
  ...
  for (const hostile of [
    ...
    map.replace("availability: planned", "availability: active"),
  ]) {
...
test("CLI emits only fixed success or rejection lines", async () => {
  ...
  assert.equal(successOut.value(), "UI/API coverage passed: planned=3 api_available=47 available=20 public=67 internal=0.\n");
